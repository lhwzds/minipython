...
| Product embedding API | Exercise Rust and pure Python structured results, inert value transport, authorized external functions, and persistent isolated sessions through real workers | `cargo test --test product` | An embedding surface bypassed the process boundary or broke its lifecycle contract |
...
| Pure Python binding | yes | `proven` | `tests/product.rs` runs the real CPython 3.14.6 client through the private JSON broker and isolated worker, covering structured values, exceptions, explicit callbacks, persistent sessions, and timeout closure; process tests reject direct broker invocation | Keep the package dependency-free and prevent any in-process VM, C ABI, shell, or worker-bypass path |
