...
fn collections_sandbox_subset_keeps_export_surface_explicit() {
    assert_eq!(
        ...
        Ok(output_lines(&[
            ...
            "abc ValuesView True",
            "abc __all__ True",
            "['AsyncGenerator', 'AsyncIterable', 'AsyncIterator', 'Awaitable', 'Buffer', 'ByteString', 'Callable', 'Collection', 'Container', 'Coroutine', 'Generator', 'Hashable', 'ItemsView', 'Iterable', 'Iterator', 'KeysView', 'Mapping', 'MappingView', 'MutableMapping', 'MutableSequence', 'MutableSet', 'Reversible', 'Sequence', 'Set', 'Sized', 'ValuesView', '__all__', '__doc__', '__name__', '__package__']",
        ]))
    );
}
...
fn sys_sandbox_subset_keeps_export_surface_explicit() {
    assert_eq!(
        ...
        Ok(output_lines(&[
            ...
            "__all__ False",
            "__file__ False",
            "['__breakpointhook__', '__doc__', '__name__', '__package__', '_getframe', 'argv', 'breakpointhook', 'builtin_module_names', 'byteorder', 'dont_write_bytecode', 'exc_info', 'flags', 'float_info', 'float_repr_style', 'get_int_max_str_digits', 'getdefaultencoding', 'hash_info', 'hexversion', 'implementation', 'is_finalizing', 'maxsize', 'modules', 'path', 'set_int_max_str_digits', 'stderr', 'stdin', 'stdout', 'version', 'version_info', 'warnoptions']",
            "list []",
            "list ['']",
            "list True",
            "bool False",
...
fn reports_unsupported_match_patterns() {
    ...
    assert_eq!(
        run_source("match 1:\n    case 1 as 2:\n        print(\"bad\")"),
        Err("parse error: cannot use literal as pattern target".to_string())
    );
    assert_eq!(
        run_source("match 1:\n    case 1 as None:\n        print(\"bad\")"),
    ...
    assert_eq!(
        run_source("match 1:\n    case 1 as False:\n        print(\"bad\")"),
        Err("parse error: cannot use False as pattern target".to_string())
    );
    assert_eq!(
        run_source("match 1:\n    case 1 as ...:\n        print(\"bad\")"),
        Err("parse error: cannot use ellipsis as pattern target".to_string())
    );
    assert_eq!(
        run_source("match 1:\n    case 1 as f\"x\":\n        print(\"bad\")"),
        Err("parse error: cannot use f-string expression as pattern target".to_string())
    );
    assert_eq!(
        run_source("match 1:\n    case 1 as t\"x\":\n        print(\"bad\")"),
        Err("parse error: cannot use t-string expression as pattern target".to_string())
    );
    assert_eq!(
        run_source("match 1:\n    case 1 as -1:\n        print(\"bad\")"),
        Err("parse error: cannot use expression as pattern target".to_string())
    );
    assert_eq!(
        run_source("match [1, 2]:\n    case x, x:\n        print(x)"),
