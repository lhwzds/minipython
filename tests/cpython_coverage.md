...
| `expression` | supported | The `invalid_if_expression`, `invalid_expression`, `invalid_legacy_expression`, `if_expression`, `disjunction`, and `lambdef` alternatives are covered by `cpython_invalid_expression_rules_subset`, `cpython_grammar_conditional_expression_subset`, `cpython_expression_without_invalid_subset`, `cpython_grammar_boolean_operations_subset`, `cpython_grammar_lambda_subset`, `runs_boolean_operators`, `evaluates_eval_input_expression`, and `cpython_compile_expression_stack_size_shapes_subset` long-expression compile-shape checks; standalone bang syntax forms are rejected with CPython's `invalid syntax` message |
...
| `invalid_expression` | supported | Missing commas, string-adjacent expressions, incomplete conditional expressions, CPython `test_syntax.py` conditional-expression branches that accidentally contain statements, statement-in-expression positions, standalone bang syntax forms, and unparenthesized f-string/t-string lambda expressions are rejected by `cpython_invalid_expression_rules_subset` |
