...
| `invalid_case_block` | supported | Missing case-block colons across literal, guarded, wildcard, capture, sequence, mapping, class, OR, and as-pattern cases are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `expected ':'` messages; missing case indentation with and without guards plus missing case patterns are also rejected by `cpython_invalid_match_pattern_subset` |
