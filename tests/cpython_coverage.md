...
- `cpython_collections_userstring_add_method_subset`, backed by
  ...
  diagnostics, without implementing `UserString.__radd__`, `UserString.__mul__`,
  without preserving UserString subclass result types, full UserString
  string-method proxying, or widening host IO, network, process, C ABI, or full
  stdlib scope.
...
- `CONTAINER_RUNTIME` also includes
  ...
  without implementing `UserString.__radd__`, `UserString.__mul__`, without
  preserving UserString subclass result types, or full UserString string-method
  proxying.
- `CONTAINER_RUNTIME` also includes
