...
| `invalid_as_pattern` | supported | `_`, literal expression targets with `cannot use literal as pattern target`, `None` targets with `cannot use None as pattern target`, `True` targets with `cannot use True as pattern target`, CPython `test_syntax.py` attribute targets after `case ... as`, and call-form as-pattern targets with `cannot use function call as pattern target` are rejected by `cpython_invalid_match_pattern_subset` |
