...
- `cpython_runtime_exception_capture_subset`, backed by
  ...
  catchable in-place left-shift `TypeError` behavior, catchable in-place
  right-shift `TypeError` behavior, catchable in-place bit-or `TypeError`
  behavior, catchable in-place bit-xor `TypeError` behavior, and catchable
  in-place bit-and `TypeError` behavior with unsupported
  `+=` / `-=` / `/=` / `//=` / `%=` / `**=` / `<<=` / `>>=` / `|=` / `^=` / `&=` text,
