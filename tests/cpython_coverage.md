...
| `wildcard_pattern` | supported | Wildcard cases and grouped wildcards are covered by `cpython_match_capture_wildcard_group_helper_rules_subset` / `cpython_match_capture_wildcard_group_helper_rules_diff_subset`, `cpython_grammar_match_stmt_subset`, and `runs_match_wildcard_case`; wildcard arithmetic match patterns such as `case _ + y`, wildcard bitwise match patterns such as `case _ & y`, wildcard comparison match patterns such as `case _ == y`, wildcard boolean match patterns such as `case _ and y`, wildcard assignment-form match patterns such as `case _ := y`, and wildcard reserved-keyword match patterns such as `case _ lambda y` are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `invalid syntax` messages |
