...
- `cpython_runtime_exception_capture_subset`, backed by
  ...
  catchable binary add `TypeError` behavior, catchable in-place add
  `TypeError` behavior, catchable in-place subtract `TypeError` behavior,
  catchable in-place multiply `TypeError` behavior, catchable matrix multiply
  `TypeError` behavior,
