...
- `cpython_int_type_not_subscriptable_subset`, backed by
  `cpython_int_type_not_subscriptable_diff_subset`, now pins `int[int]`
  rejection with `type 'int' is not subscriptable`, absent type and instance
...
- `cpython_bool_type_not_subscriptable_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_float_type_not_subscriptable_subset`, backed by
  `cpython_float_type_not_subscriptable_diff_subset`, now pins `float[int]`
  rejection with `type 'float' is not subscriptable`, absent type and instance
  `__class_getitem__` visibility, and CPython AttributeError shape for direct
...
- `cpython_bytes_type_not_subscriptable_subset`, backed by
  ...
  `__class_getitem__` visibility, and CPython AttributeError shape for direct
  calls, without adding `bytes.__class_getitem__` or GenericAlias support,
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_bytearray_type_not_subscriptable_subset`, backed by
  `cpython_bytearray_type_not_subscriptable_diff_subset`, now pins
  `bytearray[int]` rejection with `type 'bytearray' is not subscriptable`,
  absent type and instance `__class_getitem__` visibility, and CPython
...
- `cpython_range_type_not_subscriptable_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_slice_type_not_subscriptable_subset`, backed by
  `cpython_slice_type_not_subscriptable_diff_subset`, now pins `slice[int]`
  rejection with `type 'slice' is not subscriptable`, absent type and instance
  `__class_getitem__` visibility, and CPython AttributeError shape for direct
...
- `cpython_module_type_not_subscriptable_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_super_type_not_subscriptable_subset`, backed by
  `cpython_super_type_not_subscriptable_diff_subset`, now pins `super[int]`
  rejection with `type 'super' is not subscriptable`, absent type and instance
  `__class_getitem__` visibility, and CPython AttributeError shape for direct
  calls, without adding `super.__class_getitem__` or GenericAlias support,
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_property_type_not_subscriptable_subset`, backed by
  `cpython_property_type_not_subscriptable_diff_subset`, now pins
  `property[int]` rejection with `type 'property' is not subscriptable`, absent
  type and instance `__class_getitem__` visibility, and CPython AttributeError
...
- `cpython_collections_userdict_slots_metadata_subset`, backed by
  ...
  surface.
- `cpython_collections_userdict_name_dir_surface_subset`, backed by
  `cpython_collections_userdict_name_dir_surface_diff_subset`, now pins
  `UserDict.__name__` lookup while keeping `__name__` out of
...
- `cpython_collections_userlist_name_dir_surface_subset`, backed by
  `cpython_collections_userlist_name_dir_surface_diff_subset`, now pins
  `UserList.__name__` lookup while keeping `__name__` out of `dir(UserList)`
  and `dir(UserList(...))`, without changing `UserList.__name__` lookup,
  without changing direct base metadata lookup, or adding new UserList
  sequence-method surface.
- `cpython_collections_userlist_module_metadata_subset`, backed by
  `cpython_collections_userlist_module_metadata_diff_subset`, now pins
  `UserList.__module__`, `UserList(...).__module__`, and `dir(UserList)` and
  `dir(UserList(...))` visibility to `collections`, without changing direct
  base metadata lookup or adding new UserList sequence-method surface.
- `cpython_collections_userlist_abstractmethods_metadata_subset`, backed by
  `cpython_collections_userlist_abstractmethods_metadata_diff_subset`, now pins
  `UserList.__abstractmethods__`, `UserList(...).__abstractmethods__`, and
...
- `cpython_collections_userlist_reversed_method_subset`, backed by
  `cpython_collections_userlist_reversed_method_diff_subset`, now pins
  `UserList.__reversed__`, `UserList(...).__reversed__`,
...
- `cpython_collections_userlist_inherited_setattr_method_subset`, backed by
  `cpython_collections_userlist_inherited_setattr_method_diff_subset`, now
  pins `UserList.__setattr__`, `UserList(...).__setattr__`,
  `UserList.__setattr__ is object.__setattr__`, and `dir(UserList)` and
  `dir(UserList(...))` visibility for pure in-memory attribute assignment,
  without adding the full inherited object method matrix and without changing UserList deletion or getattribute behavior.
...
- `cpython_collections_userstring_len_method_subset`, backed by
  ...
  type-object dispatch, `self=` keyword binding, empty strings, bad receiver
  errors, and UserString-specific arity/keyword TypeErrors, without
  implementing full UserString string-method proxying or widening host IO,
  network, process, C ABI, or full stdlib scope.
- `cpython_collections_userstring_display_methods_subset`, backed by
  `cpython_collections_userstring_display_methods_diff_subset`, now pins
  `UserString.__str__`, `UserString.__repr__`, `str()`, and `repr()` display
  behavior, including type-object dispatch, `self=` keyword binding, empty
  strings, bad receiver errors, and UserString-specific arity/keyword
  TypeErrors, without implementing full UserString string-method proxying or
  widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_collections_userstring_hash_method_subset`, backed by
  `cpython_collections_userstring_hash_method_diff_subset`, now pins
  `UserString.__hash__` and `hash()` behavior, including `.data` hash
  equivalence, type-object dispatch, `self=` keyword binding, empty strings,
  bad receiver errors, UserString-specific arity/keyword TypeErrors, and
...
- `cpython_collections_userstring_float_method_subset`, backed by
  `cpython_collections_userstring_float_method_diff_subset`, now pins
  `UserString.__float__` and `float(UserString(...))` behavior, including type and instance visibility,
  `dir(UserString)` discoverability, direct and bound method calls, signed, whitespace-padded, nan, and infinity strings, and CPython `UserString.__float__` TypeError/ValueError text, without broadening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_collections_userstring_complex_method_subset`, backed by
  `cpython_collections_userstring_complex_method_diff_subset`, now pins
...
- `cpython_collections_userstring_dunder_format_method_subset`, backed by
  `cpython_collections_userstring_dunder_format_method_diff_subset`, now pins
  `UserString.__format__` as inherited `object.__format__`, including
  type and instance visibility, `dir(UserString)` discoverability,
  `UserString.__format__ is object.__format__`, `method_descriptor` metadata,
  direct and bound method calls, bad receiver support, non-empty format-spec
...
- `cpython_collections_userstring_case_transform_methods_subset`, backed by
  ...
  host IO, network, process, C ABI, or full stdlib scope.
- `cpython_collections_userstring_predicate_methods_subset`, backed by
  `cpython_collections_userstring_predicate_methods_diff_subset`, now pins
  `UserString.islower`, `UserString.isupper`, `UserString.istitle`,
  ...
  implementing full UserString string-method proxying or widening host IO,
  network, process, C ABI, or full stdlib scope.
- `cpython_collections_userstring_search_methods_subset`, backed by
  `cpython_collections_userstring_search_methods_diff_subset`, now pins
  `UserString.count`, `UserString.find`, `UserString.rfind`,
  `UserString.index`, and `UserString.rindex` search/count methods, including
  `sub/start/end` keyword binding, empty substring bounds, `UserString` needle
...
- `cpython_collections_userstring_split_method_subset`, backed by
  ...
  UserString string-method proxying or widening host IO, network, process, C
  ABI, or full stdlib scope.
- `cpython_collections_userstring_rsplit_method_subset`, backed by
  `cpython_collections_userstring_rsplit_method_diff_subset`, now pins
  `UserString.rsplit`, including ordinary `list[str]` result values, right-side whitespace and explicit separator splitting, `sep=` and `maxsplit=` keyword binding,
  bool and negative maxsplit handling, `__index__` dispatch and bad-result
  TypeErrors, C-ssize_t overflow `OverflowError`, `str` subclass separator
  handling, UserString separator rejection, bad receiver errors, and
  UserString-specific arity/keyword TypeErrors, without implementing full
...
- `cpython_collections_userstring_zfill_method_subset`, backed by
  ...
  padding, `width=` keyword binding, bool and negative width handling,
  `__index__` dispatch and bad-result TypeErrors, oversized integer
  `OverflowError`, bad receiver errors, and UserString-specific arity/keyword
  TypeErrors, without implementing full UserString alignment methods or
  widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_collections_userstring_splitlines_method_subset`, backed by
  `cpython_collections_userstring_splitlines_method_diff_subset`, now pins
  `UserString.splitlines`, including ordinary `list[str]` result values,
  ASCII and Unicode line-boundary handling, `keepends` truthiness and keyword
  binding, empty and trailing-line behavior, bad receiver errors, and
  UserString-specific arity/keyword TypeErrors, without implementing full
  UserString string-method proxying or widening host IO, network, process, C
  ABI, or full stdlib scope.
...
- `cpython_collections_userstring_replace_method_subset`, backed by
  ...
  UserString string-method proxying or widening host IO, network, process, C
  ABI, or full stdlib scope.
- `cpython_collections_userstring_center_method_subset`, backed by
  `cpython_collections_userstring_center_method_diff_subset`, now pins
  `UserString.center`, including UserString result wrapping, default and custom
  fill characters, str-subclass fill characters, UserString fill rejection,
  positional-only fill character rejection, `width=` keyword binding, bool and
  negative width handling, `__index__` dispatch and bad-result TypeErrors,
  C-ssize_t overflow `OverflowError`, bad receiver errors, and
  UserString-specific arity/keyword TypeErrors, without implementing full
  UserString alignment proxying or widening host IO, network, process, C ABI,
  or full stdlib scope.
- `cpython_collections_userstring_ljust_method_subset`, backed by
  `cpython_collections_userstring_ljust_method_diff_subset`, now pins
  `UserString.ljust`, including UserString result wrapping, right-side padding,
...
- `cpython_tuple_subclass_repr_direct_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_tuple_hash_direct_subset`, backed by
  `cpython_tuple_hash_direct_diff_subset`, now pins tuple direct `__hash__`,
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_tuple_inherited_sizeof_direct_subset`, backed by
  `cpython_tuple_inherited_sizeof_direct_diff_subset`, now pins tuple inherited `__sizeof__`,
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_tuple_inherited_getattribute_direct_subset`, backed by
  `cpython_tuple_inherited_getattribute_direct_diff_subset`, now pins tuple inherited `__getattribute__`,
...
- `cpython_list_type_metadata_dir_surface_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_tuple_type_metadata_dir_surface_subset`, backed by
  `cpython_tuple_type_metadata_dir_surface_diff_subset`, now pins `dir(tuple)` and
  `dir(())` hiding `__base__`, `__bases__`, and `__name__` while keeping
...
- `cpython_set_type_metadata_dir_surface_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_frozenset_type_metadata_dir_surface_subset`, backed by
  `cpython_frozenset_type_metadata_dir_surface_diff_subset`, now pins
  `dir(frozenset)` and `dir(frozenset())` hiding `__base__`, `__bases__`, and
  `__name__` while keeping `frozenset.__base__`, `frozenset.__bases__`, and
  `frozenset.__name__` directly readable, without hiding direct frozenset type metadata lookup,
  without changing frozenset method visibility, without widening host IO, network,
  process, C ABI, or full stdlib scope.
- `cpython_str_type_metadata_dir_surface_subset`, backed by
  `cpython_str_type_metadata_dir_surface_diff_subset`, now pins `dir(str)` and
  `dir("")` hiding `__base__`, `__bases__`, and `__name__` while keeping
...
- `cpython_float_type_metadata_dir_surface_subset`, backed by
  ...
  host IO, network, process, C ABI, or full stdlib scope.
- `cpython_int_type_metadata_dir_surface_subset`, backed by
  `cpython_int_type_metadata_dir_surface_diff_subset`, now pins `dir(int)`
  and `dir(0)` hiding `__base__`, `__bases__`, and `__name__` while keeping
  `int.__base__`, `int.__bases__`, and `int.__name__` directly readable,
...
- `cpython_bool_type_metadata_dir_surface_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_object_type_metadata_dir_surface_subset`, backed by
  `cpython_object_type_metadata_dir_surface_diff_subset`, now pins
  `dir(object)`, `dir(object())`, and `object.__dir__(object())` hiding
  `__base__`, `__bases__`, and `__name__` while keeping `object.__base__`,
  `object.__bases__`, and `object.__name__` directly readable,
...
- `cpython_set_family_class_getitem_generic_alias_subset`, backed by
  ...
  `frozenset.__class_getitem__(int) == frozenset[int]`, exact set and frozenset instance lookup,
  set and frozenset subclass class and instance origin binding,
  `GenericAlias` origin/args, pair argument normalization
...
- `cpython_memoryview_class_getitem_generic_alias_subset`, backed by
  ...
  without adding full GenericAlias repr parity or full classmethod_descriptor metadata,
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_enumerate_class_getitem_generic_alias_subset`, backed by
  `cpython_enumerate_class_getitem_generic_alias_diff_subset`, now pins
...
- `cpython_enumerate_type_metadata_dir_surface_subset`, backed by
  ...
  and without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_enumerate_reduce_type_dict_subset`, backed by
  `cpython_enumerate_reduce_type_dict_diff_subset`, now pins enumerate
  `__reduce__` returning the underlying iterator plus the current enumerate
  index, enumerate type `__dict__` mappingproxy entries, and `__reduce__`
  visibility in `dir(enumerate)` and `dir(enumerate(...))`, without widening
  host IO, network, process, C ABI, or full stdlib scope.
...
- `cpython_filter_type_metadata_dir_surface_subset`, backed by
  ...
  and without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_filter_reduce_type_dict_subset`, backed by
  `cpython_filter_reduce_type_dict_diff_subset`, now pins filter `__reduce__`
  ...
  full stdlib scope.
- `cpython_callable_iterator_reduce_type_dict_subset`, backed by
  `cpython_callable_iterator_reduce_type_dict_diff_subset`, now pins
  callable_iterator `__reduce__` for `iter(make, 2)` returning the
...
- `cpython_builtin_iterator_dir_protocol_methods_subset`, backed by
  ...
  network, process, C ABI, or full stdlib scope.
- `cpython_builtin_iterator_type_dict_mappingproxy_surface_subset`, backed by
  `cpython_builtin_iterator_type_dict_mappingproxy_surface_diff_subset`, now
  pins the builtin iterator type `__dict__` mappingproxy surface for sequence,
...
- `cpython_range_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  `dir(type(reversed(range(...))))`, and `dir(reversed(range(...)))` for
  `range_iterator.__base__`, `range_iterator.__bases__`,
  ...
  `inst.__reduce__()`. It also pins range-iterator reduce forms
...
- `cpython_list_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  `dir(iter([...]))` for `list_iterator.__base__`,
  `list_iterator.__bases__`, `list_iterator.__module__`,
  `list_iterator.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, `hasattr(typ, '__reduce__')`, direct
  ...
  or full stdlib scope.
- `cpython_list_iterator_setstate_subset`, backed by
  `cpython_list_iterator_setstate_diff_subset`, now pins
  `list_iterator.__setstate__`, `__setstate__` visibility on the type and instance,
  negative state exhaustion, oversized non-overflowing state exhaustion, bool and int-subclass state handling,
...
- `cpython_reversed_tuple_type_metadata_dir_surface_subset`, backed by
  ...
  `dir(type(reversed(tuple(...))))`, and `dir(reversed(tuple(...)))` for
  `reversed.__base__`, `reversed.__bases__`, `reversed.__module__`,
  `reversed.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, direct `typ.__length_hint__(inst)` /
...
- `cpython_reversed_str_type_metadata_dir_surface_subset`, backed by
  ...
  `exhausted-reduce 2 True 1 True tuple True`, without widening host IO,
  network, process, C ABI, or full stdlib scope.
- `cpython_reversed_bytes_type_metadata_dir_surface_subset`, backed by
  `cpython_reversed_bytes_type_metadata_dir_surface_diff_subset`, now pins
  `reversed(bytes)`, `type(inst)`, `dir(type(reversed(bytes)))`, and
  `dir(reversed(bytes))` for `reversed.__base__`, `reversed.__bases__`,
  `reversed.__module__`, `reversed.__qualname__`, `__iter__`, `__next__`,
  `__length_hint__`, `__reduce__`, direct `typ.__length_hint__(inst)` /
  `typ.__reduce__(inst)` dispatch, `inst.__length_hint__()`,
  `fresh-reduce 3 True 1 True 2 True`,
  ...
  `exhausted-reduce 2 True 1 True tuple True`, without widening host IO,
  network, process, C ABI, or full stdlib scope.
- `cpython_reversed_bytearray_type_metadata_dir_surface_subset`, backed by
  `cpython_reversed_bytearray_type_metadata_dir_surface_diff_subset`, now pins
  `reversed(bytearray)`, `type(inst)`, `dir(type(reversed(bytearray)))`, and
  `dir(reversed(bytearray))` for `reversed.__base__`,
  `reversed.__bases__`, `reversed.__module__`,
  `reversed.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, direct `typ.__length_hint__(inst)` /
  `typ.__reduce__(inst)` dispatch, `inst.__length_hint__()`,
  ...
  `exhausted-reduce 2 True 1 True tuple True`, without widening host IO,
  network, process, C ABI, or full stdlib scope.
- `cpython_reversed_memoryview_type_metadata_dir_surface_subset`, backed by
  `cpython_reversed_memoryview_type_metadata_dir_surface_diff_subset`, now
  pins `reversed(memoryview)`, `type(inst)`,
  `dir(type(reversed(memoryview)))`, and `dir(reversed(memoryview))` for
  `reversed.__base__`, `reversed.__bases__`, `reversed.__module__`,
  `reversed.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, direct `typ.__length_hint__(inst)` /
  `typ.__reduce__(inst)` dispatch, `inst.__length_hint__()`,
  `bytes(fresh[1][0]) == bytes(source)`,
  `fresh-reduce 3 True 1 memoryview True 2 True`,
  `iter-next-hint True 0 2 2`,
  `reduce-shape 3 True 1 memoryview True 1 True`,
  `late-reduce 3 True 1 memoryview True 0 True`,
  `empty-reduce 2 True 1 True tuple True`, and
  `exhausted-reduce 2 True 1 True tuple True`, without widening host IO,
  network, process, C ABI, or full stdlib scope.
- `cpython_reversed_array_type_metadata_dir_surface_subset`, backed by
  `cpython_reversed_array_type_metadata_dir_surface_diff_subset`, now pins
  `reversed(array.array)`, `type(inst)`,
  `dir(type(reversed(array.array)))`, and `dir(reversed(array.array))` for
  `reversed.__base__`, `reversed.__bases__`, `reversed.__module__`,
  `reversed.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, direct `typ.__length_hint__(inst)` /
  `typ.__reduce__(inst)` dispatch, `inst.__length_hint__()`,
...
- `cpython_tuple_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  `iter((1, 2, 3))`, `type(inst)`, `dir(type(iter((...))))`, and
  `dir(iter((...)))` for `tuple_iterator.__base__`,
  `tuple_iterator.__bases__`, `tuple_iterator.__module__`,
  `tuple_iterator.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, `hasattr(typ, '__reduce__')`, direct
  `typ.__length_hint__(inst)` / `typ.__next__(inst)` dispatch, and
  `inst.__length_hint__()`, without widening host IO, network, process, C ABI,
  or full stdlib scope.
- `cpython_tuple_iterator_setstate_subset`, backed by
  `cpython_tuple_iterator_setstate_diff_subset`, now pins
  `tuple_iterator.__setstate__`, `__setstate__` visibility on the type and instance,
  ...
  process, C ABI, or full stdlib scope.
- `cpython_str_iterator_type_metadata_dir_surface_subset`, backed by
  `cpython_str_iterator_type_metadata_dir_surface_diff_subset`, now pins
  `iter(non-ASCII str)`, `type(inst)`, `dir(type(iter(non-ASCII str)))`, and
  `dir(iter(non-ASCII str))` for `str_iterator.__base__`,
  `str_iterator.__bases__`, `str_iterator.__module__`,
  `str_iterator.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  ...
  C ABI, or full stdlib scope.
- `cpython_str_iterator_setstate_subset`, backed by
  `cpython_str_iterator_setstate_diff_subset`, now pins
  `str_iterator.__setstate__`, `__setstate__` visibility on the type and instance,
  negative state clamping to zero, oversized non-overflowing state exhaustion,
  bool and int-subclass state handling, ASCII `str_ascii_iterator` receiver rejection,
  `TypeError: an integer is required`,
  `OverflowError: Python int too large to convert to C ssize_t`, and
  descriptor receiver error behavior, without widening host IO, network,
  process, C ABI, or full stdlib scope.
- `cpython_str_ascii_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  host IO, network, process, C ABI, or full stdlib scope.
- `cpython_str_ascii_iterator_setstate_subset`, backed by
  `cpython_str_ascii_iterator_setstate_diff_subset`, now pins
  `str_ascii_iterator.__setstate__`, `__setstate__` visibility on the type and instance,
  negative state clamping to zero, oversized non-overflowing state exhaustion,
  bool and int-subclass state handling, non-ASCII `str_iterator` receiver rejection,
  `TypeError: an integer is required`,
  `OverflowError: Python int too large to convert to C ssize_t`, and
  descriptor receiver error behavior, without widening host IO, network,
  process, C ABI, or full stdlib scope.
- `cpython_bytes_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  `dir(iter(b'...'))` for `bytes_iterator.__base__`,
  `bytes_iterator.__bases__`, `bytes_iterator.__module__`,
  `bytes_iterator.__qualname__`, `__iter__`, `__next__`, `__length_hint__`,
  `__reduce__`, direct `typ.__length_hint__(inst)` / `typ.__next__(inst)` /
  `typ.__reduce__(inst)` dispatch, `inst.__length_hint__()`, and
...
- `cpython_bytearray_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  and `dir(iter(bytearray(...)))` for `bytearray_iterator.__base__`,
  `bytearray_iterator.__bases__`, `bytearray_iterator.__module__`,
  `bytearray_iterator.__qualname__`, `__iter__`, `__next__`,
  `__length_hint__`, `__reduce__`, direct `typ.__length_hint__(inst)` /
  `typ.__next__(inst)` / `typ.__reduce__(inst)` dispatch,
  `inst.__length_hint__()`, and `reduce-shape 3 True True 2`, without widening
  host IO, network, process, C ABI, or full stdlib scope.
- `cpython_bytearray_iterator_setstate_subset`, backed by
  `cpython_bytearray_iterator_setstate_diff_subset`, now pins
  `bytearray_iterator.__setstate__`, `__setstate__` visibility on the type and instance,
  negative state exhaustion, oversized non-overflowing state exhaustion,
  bool and int-subclass state handling,
  `TypeError: an integer is required`,
  `OverflowError: Python int too large to convert to C ssize_t`, and
  descriptor receiver error behavior, without widening host IO, network,
...
- `cpython_set_iterator_type_metadata_dir_surface_subset`, backed by
  ...
  network, process, C ABI, or full stdlib scope.
- `cpython_dict_keyiterator_type_metadata_dir_surface_subset`, backed by
  `cpython_dict_keyiterator_type_metadata_dir_surface_diff_subset`, now pins
  `iter({'a': 1, 'b': 2, 'c': 3})`, `type(inst)`,
  `dir(type(iter(dict(...))))`, and `dir(iter(dict(...)))` for
  `dict_keyiterator.__base__`, `dict_keyiterator.__bases__`,
  `dict_keyiterator.__module__`, `dict_keyiterator.__qualname__`,
  `__iter__`, `__next__`, `__length_hint__`, `__reduce__`, direct
  `typ.__length_hint__(inst)` / `typ.__next__(inst)` /
  `typ.__reduce__(inst)` dispatch, `inst.__length_hint__()`,
  `reduce-shape 2 True 1 list True 2 True`, and size-change
...
- `cpython_tuple_inherited_getstate_direct_subset`, backed by
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_tuple_inherited_dir_direct_subset`, backed by
  `cpython_tuple_inherited_dir_direct_diff_subset`, now pins tuple inherited `__dir__`,
  exact and tuple-subclass default dir-list shape checks, direct `tuple.__dir__`
  dispatch for tuple and non-tuple receivers, non-tuple direct receiver support
  ...
  without widening host IO, network, process, C ABI, or full stdlib scope.
- `cpython_tuple_inherited_init_direct_subset`, backed by
  `cpython_tuple_inherited_init_direct_diff_subset`, now pins tuple inherited `__init__`,
...
- `cpython_set_new_direct_allocation_subset`, backed by
  ...
  C-layout behavior.
- `cpython_frozenset_new_direct_allocation_subset`, backed by
  `cpython_frozenset_new_direct_allocation_diff_subset`, now pins `frozenset.__new__` direct allocation,
...
- `cpython_range_unacceptable_base_type_subset`, backed by
...
- `cpython_tuple_index_missing_valueerror_message_subset`, backed by
  ...
  and tuple-subclass `index()` calls.
- `cpython_immutable_sequence_index_rich_compare_subset`, backed by
  `cpython_immutable_sequence_index_rich_compare_diff_subset`, now pins
  tuple and range `index()` rich comparison, propagated equality exceptions,
...
- `cpython_str_instance_doc_attribute_subset`, backed by
  ...
  `str.__doc__` and `dir(value)` without adding writable instance dictionaries.
...
- `cpython_bytearray_attribute_assignment_errors_subset`, backed by
...
- `cpython_bytes_instance_doc_attribute_subset`, backed by
  ...
  without adding writable instance dictionaries.
- `cpython_bytearray_instance_doc_attribute_subset`, backed by
  `cpython_bytearray_instance_doc_attribute_diff_subset`, now pins bytearray instance `__doc__`
  lookup for empty and populated bytearrays, matching `bytearray.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- `cpython_range_instance_doc_attribute_subset`, backed by
...
- `cpython_range_public_attributes_subset`, backed by
  ...
  default, explicit, negative-step, and large-integer ranges, range data attributes in dir()
  for both instances and the builtin type, plus CPython's read-only range method attributes
  (`count` and `index`), readonly, and no-`__dict__` assignment/deletion errors without adding range instance dictionaries.
...
- `cpython_super_object_dir_supported_attributes_subset`, backed by
...
- `cpython_super_getattribute_wrapper_descriptor_subset`, backed by
  `cpython_super_getattribute_wrapper_descriptor_diff_subset`, now pins the super getattribute wrapper descriptor,
  including `dir(super)`, `super.__dict__`, descriptor metadata, direct
  `super.__getattribute__()` access for supported super attributes, and no-receiver,
  missing-name, extra-arg, keyword, wrong-receiver, and non-string-name errors without
...
- `cpython_json_loads_invalid_utf8_error_detail_subset`, backed by
  ...
  `json.loads()` invalid UTF-8 byte input diagnostics aligned with CPython for
  `bytes`, UTF-8-BOM-stripped bytes, and `bytearray`: invalid start bytes,
...
- `cpython_json_loads_invalid_utf32_error_detail_subset`, backed by
  ...
  aligned with CPython for UTF-32 BOM-stripped bytes and heuristic UTF-32 byte
  detection: truncated data and out-of-range code point cases preserve
  CPython-style codec/endian byte position/reason text without adding unpaired
  surrogate storage or full `JSONDecodeError` compatibility.
...
- `cpython_runtime_exception_capture_subset`, backed by
  ...
  behavior, object.__getattribute__ UserList type-object __base__ metadata
  behavior, object.__getattribute__ UserList type-object __bases__ metadata
  ...
  behavior, object.__getattribute__ namedtuple builtin __qualname__ metadata
  behavior, object.__getattribute__ namedtuple builtin __defaults__ metadata
  behavior, object.__getattribute__ namedtuple builtin __kwdefaults__ metadata
  behavior, object.__getattribute__ namedtuple builtin __annotations__ metadata
  ...
  behavior, object.__getattribute__ _count_elements builtin __text_signature__ metadata
  behavior, object.__getattribute__ Hashable ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Hashable ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Iterable ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Iterable ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Iterator ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Iterator ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Sized ABC type-object __doc__ metadata
  ...
  behavior, object.__getattribute__ Callable ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Collection ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Collection ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Reversible ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Reversible ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Sequence ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Sequence ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ MutableSequence ABC type-object __doc__ metadata
  behavior, object.__getattribute__ MutableSequence ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ ByteString ABC type-object __doc__ metadata
  behavior, object.__getattribute__ ByteString ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Buffer ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Buffer ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Awaitable ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Awaitable ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Coroutine ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Coroutine ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ AsyncIterable ABC type-object __doc__ metadata
  behavior, object.__getattribute__ AsyncIterable ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ AsyncIterator ABC type-object __doc__ metadata
  behavior, object.__getattribute__ AsyncIterator ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ AsyncGenerator ABC type-object __doc__ metadata
  behavior, object.__getattribute__ AsyncGenerator ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Generator ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Generator ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Mapping ABC type-object __doc__ metadata
  behavior, object.__getattribute__ Mapping ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ MutableMapping ABC type-object __doc__ metadata
  behavior, object.__getattribute__ MutableMapping ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ MappingView ABC type-object __doc__ metadata
  behavior, object.__getattribute__ MappingView ABC type-object __text_signature__ metadata
  ...
  behavior, object.__getattribute__ ValuesView ABC type-object __doc__ metadata
  behavior, object.__getattribute__ ValuesView ABC type-object __text_signature__ metadata
  behavior, object.__getattribute__ Set ABC type-object __doc__ metadata
  ...
  behavior, object.__getattribute__ UserString type-object __qualname__ metadata
  ...
  behavior, object.__getattribute__ UserString type-object __text_signature__ metadata
  ...
  behavior, object.__getattribute__ frozenset type-object __text_signature__ metadata
  behavior, object.__getattribute__ memoryview type-object __text_signature__ metadata
  ...
  behavior, object.__getattribute__ dict type-object __doc__ metadata
  behavior, object.__getattribute__ range type-object __doc__ metadata
  ...
  behavior, object.__getattribute__ set type-object __doc__ metadata
  ...
  behavior, object.__getattribute__ bytearray type-object __doc__ metadata
  behavior, object.__getattribute__ OrderedDict type-object __doc__ metadata
  ...
  catchable in-place subtract `TypeError` behavior, catchable in-place multiply
  `TypeError` behavior, catchable matrix multiply `TypeError` behavior,
...
- `cpython_type_params_dynamic_new_class_subset`, backed by gated direct
  ...
  `Generic[T]` with and without callback-provided `__type_params__`.
...
- Bytes literal runtime behavior has direct CPython output parity evidence in
  ...
  `cpython_mixed_bytes_nonbytes_literal_rejection_diff_subset`, while exact
  MiniPython parse-error wording remains local subset evidence.
...
- `cpython_base_exception_method_descriptor_metadata_subset`, backed by
  `cpython_base_exception_method_descriptor_metadata_diff_subset`, keeps
  BaseException helper method descriptor metadata aligned with CPython:
  `BaseException.add_note`, `Exception.add_note`,
...
- `cpython_base_exception_bound_method_dir_metadata_subset`, backed by
  `cpython_base_exception_bound_method_dir_metadata_diff_subset`, keeps
  BaseException helper bound method dir metadata aligned with CPython: bound
  `add_note` and `with_traceback` methods on `BaseException`, `Exception`, and
  ...
  `IndexError.with_traceback`.
- `cpython_base_exception_bound_method_call_wrapper_subset`, backed by
  `cpython_base_exception_bound_method_call_wrapper_diff_subset`, keeps the
  BaseException helper bound method `__call__` wrapper surface aligned with
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose a `method-wrapper` `__call__` through
...
- `cpython_base_exception_bound_method_repr_wrapper_subset`, backed by
  ...
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose a `method-wrapper` `__repr__` through
  `dir()`; the wrapper has CPython `builtin_function_or_method.__repr__`
...
- `cpython_base_exception_bound_method_ne_wrapper_subset`, backed by
  `cpython_base_exception_bound_method_ne_wrapper_diff_subset`, keeps the
  BaseException helper bound method `__ne__` wrapper surface aligned with
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose a `method-wrapper` `__ne__` through
  `dir()`; the wrapper has CPython `builtin_function_or_method.__ne__`
  metadata, keeps `__module__` absent, compares helper bound method identity,
  returns `NotImplemented` for non-method values, and preserves CPython
  argument errors, including `IndexError.with_traceback`.
- `cpython_base_exception_bound_method_order_wrapper_subset`, backed by
  `cpython_base_exception_bound_method_order_wrapper_diff_subset`, keeps the
  BaseException helper bound method ordering wrapper surface aligned with
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose `method-wrapper` `__lt__`, `__le__`,
  `__gt__`, and `__ge__` entries through `dir()`; the wrappers have CPython
...
- `cpython_base_exception_bound_method_setattr_wrapper_subset`, backed by
  `cpython_base_exception_bound_method_setattr_wrapper_diff_subset`, keeps the
  BaseException helper bound method `__setattr__` wrapper surface aligned with
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose a `method-wrapper` `__setattr__`
  through `dir()`; the wrapper has CPython `object.__setattr__` metadata,
  keeps `__module__` absent, rejects missing, partial, extra, and keyword
...
- `cpython_base_exception_bound_method_func_absent_subset`, backed by
  ...
  `IndexError.with_traceback`.
- `cpython_base_exception_bound_method_get_absent_subset`, backed by
  `cpython_base_exception_bound_method_get_absent_diff_subset`, keeps
  BaseException helper bound method `__get__` absence aligned with CPython:
  bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` keep `__get__` out of `dir()` and raise the
  CPython `builtin_function_or_method` `AttributeError`, including
  `IndexError.with_traceback`.
- `cpython_base_exception_bound_method_missing_attribute_subset`, backed by
  `cpython_base_exception_bound_method_missing_attribute_diff_subset`, keeps
  the BaseException helper bound method missing-attribute error shape aligned
  with CPython: bound `add_note` and `with_traceback` methods on
  `BaseException`, `Exception`, and `IndexError` report
  `builtin_function_or_method` as the owner for absent attributes such as
...
- `cpython_base_exception_bound_method_module_mutation_subset`, backed by
  `cpython_base_exception_bound_method_module_mutation_diff_subset`, keeps
  BaseException helper bound method `__module__` mutation behavior aligned with
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose an initial `None` module, accept direct
  and wrapper assignment/deletion, return the assigned value through normal
  attribute lookup and `__getattribute__`, and keep the override isolated per
  bound method object so fresh bound method objects still report `None`.
...
- `cpython_base_exception_bound_method_init_subclass_subset`, backed by
  `cpython_base_exception_bound_method_init_subclass_diff_subset`, keeps
  BaseException helper bound method `__init_subclass__` wrapper surface aligned
  with CPython: bound `add_note` and `with_traceback` methods on
  `BaseException`, `Exception`, and `IndexError` expose a
  `builtin_function_or_method` `__init_subclass__` through `dir()`; the wrapper
  is bound to the shared `builtin_function_or_method` type, carries CPython
  metadata, returns `None` with no arguments, and rejects extra positional and
  keyword arguments with CPython messages, including
  `IndexError.with_traceback`.
- `cpython_base_exception_bound_method_new_subset`, backed by
  `cpython_base_exception_bound_method_new_diff_subset`, keeps the
  BaseException helper bound method `__new__` wrapper surface aligned with
  CPython: bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose a `builtin_function_or_method`
  `__new__` through `dir()`; the wrapper is bound to `object`, carries CPython
  `object.__new__` metadata, creates a plain `object`, and rejects missing,
  keyword-only, and non-type string calls with CPython messages, including
  `IndexError.with_traceback`.
...
- `cpython_base_exception_bound_method_reduce_subset`, backed by
  ...
  bound `add_note` and `with_traceback` methods on `BaseException`,
  `Exception`, and `IndexError` expose a `builtin_function_or_method`
  `__reduce__` wrapper through `dir()`; the wrapper has CPython metadata,
...
- Direct sandbox stdlib `cpython_diff` evidence names are also mirrored here:
  ...
  `cpython_itertools_cycle_diff_subset` and
  ...
  `cpython_itertools_takewhile_diff_subset` and
  ...
  `cpython_itertools_accumulate_diff_subset` and
  `cpython_itertools_accumulate_subset`,
  `cpython_itertools_zip_longest_diff_subset` and
  ...
  `cpython_itertools_count_error_diff_subset` and
  `cpython_itertools_count_error_subset`,
...
- `NUMBER` also includes `cpython_numeric_from_number_classmethod_metadata_subset`,
  backed by `cpython_numeric_from_number_classmethod_metadata_diff_subset`, for
  `float.from_number` / `complex.from_number` classmethod metadata on exact
  types, instances, and subclasses, including `__self__`, `__qualname__`,
  `__module__`, `__text_signature__`, absence of `__func__`, exact complex
  identity preservation, and CPython arity/keyword diagnostics.
- `NUMBER` also includes CPython
...
- `NUMBER` also includes CPython
  ...
  (`sys.__package__` is `''`).
- `NUMBER` also includes CPython
  `cpython_sys_module_doc_metadata_subset`, backed by
  `cpython_sys_module_doc_metadata_diff_subset`, covering sys module `__doc__` metadata
  (`sys.__doc__` is CPython's public module doc string and is visible through
  `object.__getattribute__`, `sys.__dict__`, and `dir(sys)`).
- `NUMBER` also includes CPython
...
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_counter_type_base_metadata_diff_subset` and
...
- The bundled `collections` module also includes
  ...
  pure-memory `defaultdict` construction, `repr(defaultdict)` /
...
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_defaultdict_type_base_metadata_diff_subset` and
...
- The bundled `collections` module also includes
  ...
  method-descriptor error behavior: `defaultdict.copy` and
  `defaultdict.__copy__` as `method_descriptor` values,
  `defaultdict.__dict__['copy']` and `defaultdict.__dict__['__copy__']`
  exposure through the type mappingproxy, unbound method text,
  method-descriptor self validation, no-extra-argument text, and keyword
  rejection without promoting full `defaultdict.__dict__` method-table parity,
...
- The bundled `collections` module also includes
  ...
  descriptor access through the class and instance, including modern
...
- The bundled `json` module includes `cpython_json_loads_dumps_diff_subset` /
  ...
  empty-dict-subclass `{}` fast path, `Counter`, Counter subclass, and
  `OrderedDict` mapping containers, and namedtuples, standard string escapes,
...
`cpython_json_dumps_counter_subclass_subset`, backed by
`cpython_json_dumps_counter_subclass_diff_subset`, keeps `dumps()` treating
supported `collections.Counter` subclasses as dict-style mapping containers,
including normal count output, nested values, subclass `items()` dispatch for
non-empty Counters, the empty fast path, circular-reference detection, and
CPython's malformed `items()` result errors, without expanding into arbitrary
mapping-protocol or custom encoder support.
...
`cpython_json_dumps_default_hook_super_identity_subset`, backed by
`cpython_json_dumps_default_hook_super_identity_diff_subset`, keeps shared unsupported super replacement circular detection without treating fresh super objects as circular.

`cpython_json_dumps_default_hook_type_identity_subset`, backed by
`cpython_json_dumps_default_hook_type_identity_diff_subset`, keeps shared unsupported class/module/builtin replacement circular detection without treating fresh classes as circular.

`cpython_json_dumps_default_hook_exception_identity_subset`, backed by
`cpython_json_dumps_default_hook_exception_identity_diff_subset`, keeps shared unsupported exception replacement circular detection without treating fresh exceptions as circular.
...
`cpython_json_dumps_default_hook_frame_locals_proxy_identity_subset`, backed by
`cpython_json_dumps_default_hook_frame_locals_proxy_identity_diff_subset`, keeps shared unsupported frame-locals proxy replacement circular detection without treating fresh proxies as circular.
...
`cpython_json_dumps_default_hook_custom_mappingproxy_identity_subset`, backed by
`cpython_json_dumps_default_hook_custom_mappingproxy_identity_diff_subset`, keeps shared unsupported custom mappingproxy replacement circular detection without treating fresh proxies over the same mapping object as circular.
...
`cpython_json_dumps_default_hook_singledispatch_register_identity_subset`, backed by
`cpython_json_dumps_default_hook_singledispatch_register_identity_diff_subset`, keeps shared unsupported functools.singledispatch register decorator replacement circular detection without treating fresh register decorators as circular.
...
`cpython_json_dumps_default_hook_singledispatchmethod_bound_identity_subset`, backed by
`cpython_json_dumps_default_hook_singledispatchmethod_bound_identity_diff_subset`, keeps shared unsupported functools.singledispatchmethod bound callable replacement circular detection without treating fresh bound callables as circular.
...
falsey parse-hook fallback through public truthiness, non-callable hook `TypeError`
text when the hook is used, hook exception propagation, and no-op behavior when a hook is unused,
...
`cpython_json_loads_object_pairs_hook_subset`, backed by
`cpython_json_loads_object_pairs_hook_diff_subset`, keeps `loads()` `object_pairs_hook` pair-list post-processing for
decoded object pairs including nested object post-order traversal, duplicate-key preservation and input order, and
`object_pairs_hook` precedence over `object_hook` while arrays and scalar values bypassing `object_pairs_hook`.
It covers arbitrary returned values including `None`, lists, and derived key lists,
non-callable hook `TypeError` text when the hook is used, and hook exception propagation,
without adding unsupported decoder hooks or `JSONDecoder` subclassing.
...
`cpython_json_loads_int_digit_limit_diff_subset`, keeps `loads()` integer digit-limit enforcement through
`sys.get_int_max_str_digits()` / `sys.set_int_max_str_digits()` for positive and negative integers at the active limit,
top-level, array, and object integer overflows, CPython `ValueError` text classification, and disabling the limit with `0`,
...
`cpython_json_loads_invalid_utf8_error_detail_subset`, backed by
`cpython_json_loads_invalid_utf8_error_detail_diff_subset`, keeps `json.loads()` invalid UTF-8 byte input diagnostics aligned with CPython
...
truncated data and out-of-range code point cases preserve CPython-style codec/endian byte position/reason text
...
`cpython_json_loads_expecting_value_message_subset`, backed by
`cpython_json_loads_expecting_value_message_diff_subset`, pins those public `str(error)` / `args` values
without adding full `JSONDecodeError` compatibility.
...
with CPython through `json.__author__`, `object.__getattribute__`, `dir()`,
and `__dict__`, without adding file APIs, module `__all__`, or
encoder/decoder classes.

`cpython_json_module_version_metadata_subset`, backed by
`cpython_json_module_version_metadata_diff_subset`, keeps json module `__version__` metadata aligned
with CPython through `json.__version__`, `object.__getattribute__`, `dir()`,
and `__dict__`, without adding file APIs, module `__all__`, or
encoder/decoder classes.

`cpython_json_function_module_identity_metadata_subset`, backed by
`cpython_json_function_module_identity_metadata_diff_subset`, keeps json public function `__module__` identity
stable across repeated `loads` / `dumps` lookups and through bound-method
`__module__` / `__getattribute__('__module__')` delegation, without adding
full module globals, function `__code__`, file APIs, module `__all__`, or
encoder/decoder classes.

`cpython_json_function_name_qualname_identity_metadata_subset`, backed by
`cpython_json_function_name_qualname_identity_metadata_diff_subset`, keeps json public function `__name__` / `__qualname__` identity
stable across repeated `loads` / `dumps` lookups, shared per-function
...
aligned with CPython for `loads` / `dumps`, including `repr()`, `str()`,
...
`cpython_json_function_subclasshook_wrapper_metadata_subset`, backed by
`cpython_json_function_subclasshook_wrapper_metadata_diff_subset`, keeps json public function `__subclasshook__` wrapper metadata aligned with CPython for `loads` / `dumps`, including `dir()`
visibility, `builtin_function_or_method` type shape, binding to the shared
`function` type, `__name__` / `__qualname__` / `__doc__` / `__module__` /
`__text_signature__` metadata, missing-argument and keyword `TypeError` text,
and the one-argument `NotImplemented` return, without adding function
`__new__`, function `__code__`, `__sizeof__`, `__getstate__`, pickle helpers,
...
`cpython_json_function_rich_compare_wrapper_metadata_subset`, backed by
`cpython_json_function_rich_compare_wrapper_metadata_diff_subset`, keeps json
public function `__eq__` / `__ne__` wrapper metadata aligned with CPython for
`loads` / `dumps`, including `dir()` visibility, `method-wrapper` type shape,
`__self__` / `__name__` / `__qualname__` / `__doc__` / missing `__module__` /
`__text_signature__` metadata, same-function bool results, different-function
and non-function `NotImplemented` results, and arity / keyword error text,
without adding function `__code__`, pickle helpers, file APIs, module
`__all__`, or encoder/decoder classes.

`cpython_json_function_order_wrapper_metadata_subset`, backed by
`cpython_json_function_order_wrapper_metadata_diff_subset`, keeps json public
function ordering wrapper metadata aligned with CPython for `loads` / `dumps`,
covering `__lt__` / `__le__` / `__gt__` / `__ge__` `dir()` visibility,
`method-wrapper` type shape, `__self__` / `__name__` / `__qualname__` /
`__doc__` / missing `__module__` / `__text_signature__` metadata,
`NotImplemented` results, and arity / keyword error text, without adding
...
`cpython_json_function_doc_module_assignment_metadata_subset`, backed by
`cpython_json_function_doc_module_assignment_metadata_diff_subset`, keeps json public function `__doc__` / `__module__` assignment and deletion aligned with
CPython for `loads` / `dumps`, including bound-method delegation, no
`__dict__` pollution for those controlled attributes, repeated deletion to
...
`cpython_json_function_annotate_assignment_metadata_subset`, backed by
`cpython_json_function_annotate_assignment_metadata_diff_subset`, keeps json public function `__annotate__` assignment
and deletion errors aligned with CPython for `loads` / `dumps`, including
callable replacement identity, `None` reset, non-callable assignment
`TypeError`, and deletion `TypeError`, without adding lazy annotation
evaluation, function `__code__`, file APIs, module `__all__`, or
encoder/decoder classes.
...
`cpython_json_function_builtins_identity_metadata_subset`, backed by
`cpython_json_function_builtins_identity_metadata_diff_subset`, keeps json public function `__builtins__` identity
and mutation visibility aligned with CPython for the supported builtins dict
...
empty function metadata dict surface without adding arbitrary function
attribute assignment, function `__code__`, file APIs, module `__all__`, or
encoder/decoder classes.

`cpython_json_function_annotations_identity_metadata_subset`, backed by
`cpython_json_function_annotations_identity_metadata_diff_subset`, keeps json public function `__annotations__` identity
and per-function mutation visibility aligned with CPython for the supported
...
`cpython_json_function_defaults_assignment_metadata_subset`, backed by
`cpython_json_function_defaults_assignment_metadata_diff_subset`, keeps json public function `__defaults__` assignment
and deletion aligned with CPython for `loads` / `dumps`, including tuple and
tuple-subclass replacement identity, empty tuple assignment, `None` reset,
...
`cpython_json_function_dir_metadata_subset`, backed by
...
`bound.__name__` / `bound.__qualname__` / `bound.__module__` lookup while
...
`cpython_json_function_bound_method_repr_str_wrapper_subset`, backed by
`cpython_json_function_bound_method_repr_str_wrapper_diff_subset`, keeps json public function bound method `__repr__`
and `__str__` wrapper access aligned with CPython for `loads.__get__(...)` and
...
`cpython_json_function_bound_method_get_wrapper_subset`, backed by
`cpython_json_function_bound_method_get_wrapper_diff_subset`, keeps json public function bound method `__get__`
wrapper access aligned with CPython for `loads.__get__(...)` and
`dumps.__get__(...)`, including `dir()` visibility, `method-wrapper`
classification, `__self__` / `__name__` / `__qualname__` / `__doc__` metadata,
descriptor rebinding identity, missing / extra / keyword / invalid-`None`
argument errors, and wrapper repr arity / keyword `TypeError` text, without
adding `__reduce__`, `__reduce_ex__`, file APIs, module `__all__`, or
encoder/decoder classes.

`cpython_json_function_bound_method_dir_wrapper_subset`, backed by
`cpython_json_function_bound_method_dir_wrapper_diff_subset`, keeps json public function bound method `__dir__`
wrapper access aligned with CPython for `loads.__get__(...)` and
...
`cpython_json_function_bound_method_rich_compare_wrapper_subset`, backed by
`cpython_json_function_bound_method_rich_compare_wrapper_diff_subset`, keeps json public function bound method `__eq__`
and `__ne__` wrapper access aligned with CPython for `loads.__get__(...)` and
`dumps.__get__(...)`, including `dir()` visibility, `method-wrapper`
classification, `__self__` / `__name__` / `__qualname__` / `__doc__` /
`__text_signature__` metadata, same-receiver identity comparison, equal-but-not
identical receiver rejection, different-function rejection, non-method
...
`dumps.__get__(...)`, including `dir()` visibility, `method-wrapper`
classification, `__self__` / `__name__` / `__qualname__` / `__doc__` /
`__text_signature__` metadata, integer return shape, equality with
`hash(bound)`, stable repeated calls, and arity / keyword `TypeError` text,
without adding `__reduce__`, `__reduce_ex__`, `__sizeof__`, file APIs, module
`__all__`, or encoder/decoder classes.
...
the underlying function `__annotate__` value and the fact that `__annotate__`
stays out of `dir(bound)`, without adding function `__code__`, file APIs,
module `__all__`, or encoder/decoder classes.

`cpython_json_function_bound_method_type_params_metadata_subset`, backed by
`cpython_json_function_bound_method_type_params_metadata_diff_subset`, keeps json public function bound method `__type_params__` metadata
and `__getattribute__('__type_params__')` lookup aligned with CPython for
`loads.__get__(...)` and `dumps.__get__(...)`, including shared identity with
the underlying function `__type_params__` empty tuple and shared identity
between `json.loads.__type_params__` and `json.dumps.__type_params__`, while
keeping `__type_params__` out of `dir(bound)` and without adding function
`__code__`, file APIs, module `__all__`, or encoder/decoder classes.
...
`loads.__get__(...)` and `dumps.__get__(...)`, including
`object.__getattribute__(bound, '__defaults__')` missing-attribute text and
the fact that `__defaults__` stays out of `dir(bound)`, without adding file
APIs, module `__all__`, or encoder/decoder classes.
...
`dir(bound)`, without adding file APIs, module `__all__`, or encoder/decoder
classes.

`cpython_json_function_bound_method_globals_metadata_subset`, backed by
`cpython_json_function_bound_method_globals_metadata_diff_subset`, keeps json public function bound method `__globals__` metadata
...
- The bundled `itertools` module includes
  ...
  `cpython_itertools_count_error_diff_subset`,
  ...
  `accumulate()` and `zip_longest()`, modern unexpected-keyword diagnostics for
  `count()` and `repeat()`, and exact no-keyword TypeError text for
...
- The bundled `functools` module also includes
  ...
  `cpython_functools_lru_cache_subset`,
  `cpython_functools_lru_cache_exception_diff_subset` and
  ...
  attributes, public wrapper `repr()` / `str()` shape, direct `@lru_cache`
  decoration, zero, negative, and bool `maxsize` behavior including
  `cache_parameters()` metadata,
...
- The bundled `functools` module also includes
  ...
  missing-positional diagnostics, current no-keyword arity diagnostics, and unsupported-keyword text,
...
- The bundled `types` module also includes
  ...
  _types module `__package__` metadata (`_types.__package__` is `''`).
- The bundled `types` module also includes
...
- The bundled `types` module also includes
  `cpython_types_nonetype_unacceptable_base_type_subset`, backed by
  `cpython_types_nonetype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'NoneType' is not an acceptable base type` rejection for
...
- The bundled `types` module also includes
  `cpython_types_ellipsistype_unacceptable_base_type_subset`, backed by
  `cpython_types_ellipsistype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'ellipsis' is not an acceptable base type` rejection for
  `types.EllipsisType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime Ellipsis `__class__` bases while
  preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_capsuletype_unacceptable_base_type_subset`, backed by
  `cpython_types_capsuletype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'PyCapsule' is not an acceptable base type` rejection for
...
- The bundled `types` module also includes
  `cpython_types_capsuletype_qualname_metadata_subset`, backed by
  `cpython_types_capsuletype_qualname_metadata_diff_subset`, covering CPython's
  public `types.CapsuleType.__qualname__` metadata returning `PyCapsule`
  through direct lookup and `object.__getattribute__`. The slice keeps
...
- The bundled `types` module also includes
  ...
  CPython's public `types.CapsuleType.__text_signature__` metadata returning
  `None` through direct lookup and `object.__getattribute__`. The slice keeps
  `__text_signature__` absent from `dir(types.CapsuleType)` and the fallback
  `__dict__` probe, so MiniPython does not promote capsule C API behavior,
  constructor-signature support, or a writable type dictionary into the
  sandbox contract.
...
- The bundled `types` module also includes
  `cpython_types_functiontype_unacceptable_base_type_subset`, backed by
  `cpython_types_functiontype_unacceptable_base_type_diff_subset`, covering
...
- The bundled `types` module also includes
  ...
  preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_tracebacktype_unacceptable_base_type_subset`, backed by
  `cpython_types_tracebacktype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'traceback' is not an acceptable base type` rejection for
  `types.TracebackType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime traceback `__class__` bases while
  preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_frametype_unacceptable_base_type_subset`, backed by
  `cpython_types_frametype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'frame' is not an acceptable base type` rejection for
...
- The bundled `types` module also includes
  ...
  preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_coroutinetype_unacceptable_base_type_subset`, backed by
  `cpython_types_coroutinetype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'coroutine' is not an acceptable base type` rejection for
  `types.CoroutineType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime coroutine `__class__` bases while
  preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_asyncgeneratortype_unacceptable_base_type_subset`, backed by
  `cpython_types_asyncgeneratortype_unacceptable_base_type_diff_subset`,
...
- The bundled `types` module also includes
  ...
  `__class__` bases while preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_methodtype_unacceptable_base_type_subset`, backed by
  `cpython_types_methodtype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'method' is not an acceptable base type` rejection for
...
- The bundled `types` module also includes
  ...
  `cpython_types_classmethoddescriptortype_unacceptable_base_type_diff_subset`,
  covering CPython's
  `type 'classmethod_descriptor' is not an acceptable base type` rejection for
  `types.ClassMethodDescriptorType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime classmethod descriptor
  `__class__` bases while preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_wrapperdescriptortype_unacceptable_base_type_subset`, backed by
  `cpython_types_wrapperdescriptortype_unacceptable_base_type_diff_subset`,
  covering CPython's `type 'wrapper_descriptor' is not an acceptable base type`
  rejection for `types.WrapperDescriptorType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime wrapper descriptor
  `__class__` bases while preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_methodwrappertype_unacceptable_base_type_subset`, backed by
  `cpython_types_methodwrappertype_unacceptable_base_type_diff_subset`,
  covering CPython's `type 'method-wrapper' is not an acceptable base type`
...
- The bundled `types` module also includes
  ...
  bases while preserving supported `ModuleType` subclass creation.
- The bundled `types` module also includes
  `cpython_types_memberdescriptortype_unacceptable_base_type_subset`, backed by
  `cpython_types_memberdescriptortype_unacceptable_base_type_diff_subset`,
  covering CPython's `type 'member_descriptor' is not an acceptable base type`
...
- The bundled `types` module also includes
  ...
  outside the sandbox contract.
- The bundled `types` module also includes
  `cpython_types_celltype_base_metadata_subset`, backed by
  `cpython_types_celltype_base_metadata_diff_subset`, covering CPython's public
  `types.CellType.__base__` and `types.CellType.__bases__` hierarchy metadata
  returning `object` and `(object,)` through direct lookup and
  `object.__getattribute__`. The slice keeps `__base__` and `__bases__` absent
  from `dir(types.CellType)` and the fallback `__dict__` probe while leaving
...
- The bundled `types` module also includes
  ...
  `types.CellType.__name__` metadata returning `cell` through direct lookup and
  `object.__getattribute__`. The slice also keeps `__name__` absent from
  `dir(types.CellType)` and the fallback `__dict__` probe while leaving broader
  CellType constructor behavior, CPython implementation fields such as
  `__basicsize__`, and a writable type dictionary outside this sandbox contract
  slice.
- The bundled `types` module also includes
  `cpython_types_celltype_qualname_metadata_subset`, backed by
  `cpython_types_celltype_qualname_metadata_diff_subset`, covering CPython's
  public `types.CellType.__qualname__` metadata returning `cell` through direct
  lookup and `object.__getattribute__`. The slice keeps `__qualname__` absent
  from `dir(types.CellType)` and the fallback `__dict__` probe, so broader
  `CellType` type-object metadata and a writable type dictionary remain outside
  this sandbox contract slice.
- The bundled `types` module also includes
  `cpython_types_celltype_text_signature_metadata_subset`, backed by
  `cpython_types_celltype_text_signature_metadata_diff_subset`, covering
  CPython's public `types.CellType.__text_signature__` metadata returning
  `([contents])` through direct lookup and `object.__getattribute__`. The slice
  keeps `__text_signature__` absent from `dir(types.CellType)` and the fallback
  `__dict__` probe, so broader CellType constructor behavior and a writable
  type dictionary remain outside this sandbox contract slice.
- The bundled `types` module also includes
...
- The bundled `types` module also includes
  `cpython_types_celltype_unacceptable_base_type_subset`, backed by
  `cpython_types_celltype_unacceptable_base_type_diff_subset`, covering
  CPython's `type 'cell' is not an acceptable base type` rejection for
  `types.CellType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and closure cell `__class__` bases while preserving
  supported `ModuleType` subclass creation.
...
- The bundled `copy` module includes
  ...
  copy public function `__dict__` empty metadata (`copy.copy.__dict__`,
  ...
  `TypeError` text for bad classmethod hook signatures, and staticmethod
  `__replace__()` arity `TypeError` text for class-defined and external
  staticmethod hooks. Full pickle dispatch-table contents,
...
- The bundled `io.BytesIO` subset includes
  ...
  method TypeError/ValueError/OSError classification. `cpython_io_bytesio_getstate_diff_subset` /
  `cpython_io_bytesio_getstate_subset` covers `BytesIO.__getstate__()` state
  tuples for untouched empty, materialized-empty, seeked, and custom-attribute
  streams, object-level `__getattribute__` access, closed-stream `ValueError`,
  and exact no-argument / no-keyword diagnostics. `cpython_io_bytesio_setstate_diff_subset` /
  `cpython_io_bytesio_setstate_subset` covers `BytesIO.__setstate__()` buffer,
  ...
  diagnostics. `cpython_io_bytesio_read_method_descriptor_diff_subset` /
  `cpython_io_bytesio_read_method_descriptor_subset` covers the
  `io.BytesIO.read` method descriptor, including descriptor type visibility,
  bytes-returning descriptor calls with omitted, explicit, `None`, and negative
  sizes, wrong-receiver descriptor `TypeError`, missing-receiver keyword behavior,
  and exact extra-argument / keyword diagnostics.
  `cpython_io_bytesio_read1_method_descriptor_diff_subset` /
  `cpython_io_bytesio_read1_method_descriptor_subset` covers the
  `io.BytesIO.read1` method descriptor, including descriptor type visibility,
  read1 bytes-returning descriptor calls with omitted, explicit, `None`, and
  negative sizes, wrong-receiver descriptor `TypeError`, missing-receiver keyword
  behavior, and exact extra-argument / keyword diagnostics.
  `cpython_io_bytesio_readline_method_descriptor_diff_subset` /
  `cpython_io_bytesio_readline_method_descriptor_subset` covers the
  `io.BytesIO.readline` method descriptor, including descriptor type visibility,
  ...
  behavior, and exact extra-argument / keyword diagnostics.
  `cpython_io_bytesio_write_method_descriptor_diff_subset` /
  `cpython_io_bytesio_write_method_descriptor_subset` covers the
  `io.BytesIO.write` method descriptor, including descriptor type visibility,
  ...
  and missing-receiver keyword behavior. `cpython_io_bytesio_seekable_method_descriptor_diff_subset` /
  ...
  `cpython_io_bytesio_check_readable_method_descriptor_diff_subset` /
  `cpython_io_bytesio_check_readable_method_descriptor_subset` covers the
  `io.BytesIO._checkReadable` method descriptor, including descriptor type
  visibility, readability guard descriptor calls for open-stream bound and
  ...
  descriptor `TypeError`, and missing-receiver keyword behavior.
  `cpython_io_bytesio_enter_method_descriptor_diff_subset` /
  `cpython_io_bytesio_enter_method_descriptor_subset` covers the
  `io.BytesIO.__enter__` method descriptor, including descriptor type visibility,
  ...
  `cpython_io_bytesio_iter_wrapper_descriptor_diff_subset` /
  `cpython_io_bytesio_iter_wrapper_descriptor_subset` covers the
  `io.BytesIO.__iter__` wrapper descriptor, including descriptor type visibility,
  line-iteration descriptor calls returning the stream itself, first-line
...
- `RUNTIME_BUILTINS` also includes
  ...
  unsupported-operand text for unsupported `matmul`, addition,
  multiplication, bitwise, subtraction, division, floor-division, modulo, and
  shift operands, bool-preserving `+`, `*`, `&`, `|`, `^`, `-`, `/`, `//`,
  `%`, and `<<` type names, CPython-style builtin sequence concatenation and
  repeat errors for `str`, `bytes`, `bytearray`, `list`, and `tuple`, complex
  division/modulo unsupported-operand text, and CPython public unary
  bad-operand text for direct `+`/`-`/`~` syntax plus the operator aliases.
...
- `RUNTIME_BUILTINS` also includes `cpython_operator_sequence_member_subset`,
  ...
  representative TypeError/ZeroDivisionError propagation. `contains` now also
...
- `RUNTIME_BUILTINS` also includes
  ...
  module metadata: operator module `__package__` metadata
  (`operator.__package__` is `''`), the exported `operator.__all__` names,
  `operator.*` callable `__module__` / `__name__` / `__qualname__` /
  `__doc__` introspection, helper instance `attrgetter` / `itemgetter` /
  `methodcaller` type names plus `__doc__` metadata and gated `__module__`
  metadata, and dunder aliases such as
  ...
  `cpython_operator_helper_instance_module_metadata_diff_subset`.
- `RUNTIME_BUILTINS` also includes
  `cpython_operator_helper_instance_text_signature_metadata_subset`, covering
  helper instance `__text_signature__` metadata for `attrgetter`, `itemgetter`,
  and `methodcaller`. Direct attribute access and `object.__getattribute__`
...
- `RUNTIME_BUILTINS` also includes
  ...
  compatibility.
- `RUNTIME_BUILTINS` also includes
  `cpython_operator_helper_type_classinfo_metadata_subset`, covering helper type classinfo metadata for
  `type(operator.attrgetter(...))`, `type(operator.itemgetter(...))`, and
  `type(operator.methodcaller(...))`: `isinstance(helper, type(helper))`,
  `isinstance(type(helper), type)`, `issubclass(type(helper), object)`,
  self-`issubclass` checks, empty `__type_params__`, and CPython's
  `type 'operator.attrgetter' is not an acceptable base type` style rejection
  for class and `type(...)` construction attempts. Direct CPython diff evidence
  is in `cpython_operator_helper_type_classinfo_metadata_diff_subset`. This
...
- `RUNTIME_BUILTINS` also includes
  ...
  `type(operator.attrgetter(...))`, `type(operator.itemgetter(...))`, and
  `type(operator.methodcaller(...))`. The descriptor exposes CPython-aligned
  public metadata, direct-call `repr(self)` behavior for helper instances, and
...
- `RUNTIME_BUILTINS` also includes
  ...
  `__text_signature__` for predicate/sequence/call helpers, and dunder alias
...
- `STRING_RUNTIME` also includes
  ...
  `maketrans()`, `removeprefix()`, and `removesuffix()`, while accepting
  contiguous sliced views. It also covers the
...
- `CONTAINER_RUNTIME` also includes
  ...
  boundaries.
- `CONTAINER_RUNTIME` also includes
  `cpython_dict_view_direct_display_methods_subset`, backed by
...
- `CONTAINER_RUNTIME` also includes
  `cpython_dict_view_type_metadata_subset`, backed by
...
- `CONTAINER_RUNTIME` also includes
  `cpython_dict_view_dir_type_lookup_surface_subset`, backed by
...
- `CONTAINER_RUNTIME` also includes
  `cpython_dict_view_type_constructor_rejection_subset`, backed by
...
- `CONTAINER_RUNTIME` also includes
  ...
  coverage.
- `CONTAINER_RUNTIME` also includes
  `cpython_dict_view_type_subclasses_subset`, backed by
...
- `CONTAINER_RUNTIME` also includes
  ...
  metadata for `__base__` and `__bases__` without broadening into CPython
  object-layout internals.
- `CONTAINER_RUNTIME` also includes
  `cpython_types_mappingproxy_type_subclasses_subset`, backed by
  `cpython_types_mappingproxy_type_subclasses_diff_subset`, covering mappingproxy type subclasses
  introspection with an empty direct-subclass list without broadening into
  arbitrary builtin subclass registries.
- `CONTAINER_RUNTIME` also includes
  `cpython_types_mappingproxy_type_dict_subset`, backed by
...
- `CONTAINER_RUNTIME` also includes
  ...
  `MappingProxyType(mapping={'a': 1})` keyword construction and CPython's
  public argument-binding errors for mixed positional/keyword calls, missing
  `mapping`, and too many keywords.
- `CONTAINER_RUNTIME` also includes
...
- `CONTAINER_RUNTIME` also includes
  ...
  surface.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userdict_name_dir_surface_diff_subset` and
  `cpython_collections_userdict_name_dir_surface_subset`, covering
  `UserDict.__name__` lookup while keeping `__name__` out of `dir(UserDict)`
...
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userlist_type_base_metadata_diff_subset` and
...
- `CONTAINER_RUNTIME` also includes
  ...
  surface.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userlist_name_dir_surface_diff_subset` and
  `cpython_collections_userlist_name_dir_surface_subset`, covering
  `UserList.__name__` lookup while keeping `__name__` out of `dir(UserList)`
  and `dir(UserList(...))`, without changing `UserList.__name__` lookup,
  without changing direct base metadata lookup, or adding new UserList
  sequence-method surface.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userlist_module_metadata_diff_subset` and
  `cpython_collections_userlist_module_metadata_subset`, covering
  `UserList.__module__`, `UserList(...).__module__`, and `dir(UserList)` and
  `dir(UserList(...))` visibility to `collections`, without changing direct
  base metadata lookup or adding new UserList sequence-method surface.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userlist_abstractmethods_metadata_diff_subset` and
  `cpython_collections_userlist_abstractmethods_metadata_subset`, covering
  `UserList.__abstractmethods__`, `UserList(...).__abstractmethods__`, and
  `dir(UserList)` and `dir(UserList(...))` visibility to an empty
  `frozenset()`, without adding `__slots__`, full abstractmethod machinery, or
  new UserList sequence-method surface.
...
- `CONTAINER_RUNTIME` also includes
  ...
  or adding new UserList sequence-method surface.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userlist_reversed_method_diff_subset` and
  `cpython_collections_userlist_reversed_method_subset`, covering
  `UserList.__reversed__`, `UserList(...).__reversed__`,
  `reversed(UserList(...))`, and `dir(UserList)` and `dir(UserList(...))`
  visibility through `Sequence.__reversed__`, without pinning CPython's generator object implementation shape
  and without adding new UserList mutation APIs.
- `CONTAINER_RUNTIME` also includes
  ...
  without adding more constructor error-message parity and without resetting UserList instance attributes.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userlist_inherited_setattr_method_diff_subset` and
  `cpython_collections_userlist_inherited_setattr_method_subset`, covering
  `UserList.__setattr__`, `UserList(...).__setattr__`,
  `UserList.__setattr__ is object.__setattr__`, and `dir(UserList)` and
  `dir(UserList(...))` visibility for pure in-memory attribute assignment,
  without adding the full inherited object method matrix and without changing UserList deletion or getattribute behavior.
- `CONTAINER_RUNTIME` also includes
...
- `CONTAINER_RUNTIME` also includes
  ...
  `UserString.__len__` and `len()` Unicode length behavior, type-object
  dispatch, `self=` keyword binding, empty strings, bad receiver errors, and
  UserString-specific arity/keyword TypeErrors, without implementing full
  UserString string-method proxying.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_display_methods_diff_subset` and
  `cpython_collections_userstring_display_methods_subset`, covering
  `UserString.__str__`, `UserString.__repr__`, `str()`, and `repr()` display
  behavior, type-object dispatch, `self=` keyword binding, empty strings, bad
  receiver errors, and UserString-specific arity/keyword TypeErrors, without
  implementing full UserString string-method proxying.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_hash_method_diff_subset` and
  `cpython_collections_userstring_hash_method_subset`, covering
  `UserString.__hash__` and `hash()` behavior, `.data` hash equivalence,
  type-object dispatch, `self=` keyword binding, empty strings, bad receiver
...
- `CONTAINER_RUNTIME` also includes
  ...
  explicit-base rejection, and CPython `UserString.__int__` TypeError/ValueError text, without broadening host IO, network, process, C ABI, or full stdlib scope.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_float_method_diff_subset` and
  `cpython_collections_userstring_float_method_subset`, covering
  `UserString.__float__` and `float(UserString(...))` behavior, type and instance visibility,
  `dir(UserString)` discoverability, direct and bound method calls, signed, whitespace-padded, nan, and infinity strings, and CPython `UserString.__float__` TypeError/ValueError text, without broadening host IO, network, process, C ABI, or full stdlib scope.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_complex_method_diff_subset` and
  `cpython_collections_userstring_complex_method_subset`, covering
...
- `CONTAINER_RUNTIME` also includes
  ...
  without implementing pickle support or widening host IO, network, process, C ABI, or full stdlib scope.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_dunder_format_method_diff_subset` and
  `cpython_collections_userstring_dunder_format_method_subset`, covering
  `UserString.__format__` as inherited `object.__format__`, type and instance
  visibility, `dir(UserString)` discoverability, `UserString.__format__ is
  object.__format__`, `method_descriptor` metadata, direct and bound method
...
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_predicate_methods_diff_subset` and
  `cpython_collections_userstring_predicate_methods_subset`, covering
  `UserString.islower`, `UserString.isupper`, `UserString.istitle`,
  `UserString.isspace`, `UserString.isalpha`, `UserString.isalnum`,
  `UserString.isdigit`, `UserString.isdecimal`, `UserString.isnumeric`,
  `UserString.isascii`, `UserString.isidentifier`, and
  `UserString.isprintable` no-argument predicate methods, Unicode predicate
  behavior, type-object dispatch, `self=` keyword binding, bad receiver errors,
  and UserString-specific arity/keyword TypeErrors, without implementing full
  UserString string-method proxying.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_search_methods_diff_subset` and
  `cpython_collections_userstring_search_methods_subset`, covering
  `UserString.count`, `UserString.find`, `UserString.rfind`,
  `UserString.index`, and `UserString.rindex` search/count methods,
  `sub/start/end` keyword binding, empty substring bounds, `UserString` needle
  values, missing-substring `ValueError` for index/rindex, bad receiver errors,
  and UserString-specific arity/keyword TypeErrors, without implementing full
  UserString string-method proxying.
...
- `CONTAINER_RUNTIME` also includes
  ...
  UserString string-method proxying.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_rsplit_method_diff_subset` and
  `cpython_collections_userstring_rsplit_method_subset`, covering
  `UserString.rsplit`, ordinary `list[str]` result values, right-side whitespace and explicit separator splitting, `sep=` and `maxsplit=` keyword binding, bool
  and negative maxsplit handling, `__index__` dispatch and bad-result
  TypeErrors, C-ssize_t overflow `OverflowError`, `str` subclass separator
  handling, UserString separator rejection, bad receiver errors, and
  UserString-specific arity/keyword TypeErrors, without implementing full
...
- `CONTAINER_RUNTIME` also includes
  ...
  `UserString.zfill`, UserString result wrapping, sign-aware zero padding,
  `width=` keyword binding, bool and negative width handling, `__index__`
  dispatch and bad-result TypeErrors, oversized integer `OverflowError`, bad
  receiver errors, and UserString-specific arity/keyword TypeErrors, without
  implementing full UserString alignment methods.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_splitlines_method_diff_subset` and
  `cpython_collections_userstring_splitlines_method_subset`, covering
...
- `CONTAINER_RUNTIME` also includes
  ...
  UserString string-method proxying.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_userstring_center_method_diff_subset` and
  `cpython_collections_userstring_center_method_subset`, covering
  `UserString.center`, UserString result wrapping, default and custom fill
  characters, str-subclass fill characters, UserString fill rejection,
  positional-only fill character rejection, `width=` keyword binding, bool and
  negative width handling, `__index__` dispatch and bad-result TypeErrors,
  C-ssize_t overflow `OverflowError`, bad receiver errors, and
  UserString-specific arity/keyword TypeErrors, without implementing full
...
- `CONTAINER_RUNTIME` also includes
  ...
  ChainMap constructor source length for mapping, list, tuple, string, integer,
  and `None` sources, including direct `ChainMap.__len__()` dispatch and
  CPython's non-iterable source TypeError text.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_chainmap_subclass_source_len_subset`, backed by
  `cpython_collections_chainmap_subclass_source_len_diff_subset`, covering
  ChainMap subclass source length for mapping, list, tuple, string, integer,
  and `None` sources without eager mapping validation.
- `CONTAINER_RUNTIME` also includes
  `cpython_collections_chainmap_constructor_source_iter_subset`, backed by
  `cpython_collections_chainmap_constructor_source_iter_diff_subset`, covering
  ChainMap constructor source iteration for dict, `OrderedDict`, `UserDict`
...
- `CONTAINER_RUNTIME` also includes
  ...
  self validation, `defaultdict.copy` and `defaultdict.__copy__` as
  `method_descriptor` values, `defaultdict.__dict__['copy']` and
  `defaultdict.__dict__['__copy__']` exposure through the type mappingproxy,
...
- `CONTAINER_RUNTIME` also includes
  ...
  empty `popitem()` errors preserving CPython's `KeyError(message)` string
  display, unexpected-keyword errors, and duplicate-argument errors without
  promoting the full OrderedDict runtime surface.
- `CONTAINER_RUNTIME` also includes
...
- `CONTAINER_RUNTIME` also includes
  ...
  covering missing required arguments in user-defined `__init_subclass__`
  hooks: the public `TypeError` reports the hook's defining class and CPython's
  positional-argument wording, including inherited hooks, such as
...
- `CONTAINER_RUNTIME` also includes
  ...
  `cpython_copy_replace_unsupported_type_error_diff_subset`.
- `CONTAINER_RUNTIME` also includes
  `cpython_copy_replace_hook_unexpected_keyword_error_subset`, covering CPython
  `copy.replace()` class-qualified `__replace__()` unexpected-keyword `TypeError`
...
- `CONTAINER_RUNTIME` also includes
  ...
  `cpython_copy_replace_classmethod_hook_arity_error_diff_subset`.
- `CONTAINER_RUNTIME` also includes
  `cpython_copy_replace_staticmethod_hook_arity_error_subset`, covering CPython
  `copy.replace()` staticmethod `__replace__()` arity `TypeError` text for
  class-defined function and lambda hooks plus external function hooks. Direct
  CPython output parity is tracked by
  `cpython_copy_replace_staticmethod_hook_arity_error_diff_subset`.
- `CONTAINER_RUNTIME` also includes
...
- `RUNTIME_BUILTINS` also includes `cpython_globals_locals_builtin_subset`,
  ...
  iterator-producing builtins, `enumerate()` keyword success paths plus exact
...
- `cpython_repr_builtin_subset`, backed by the `repr-builtin` CPython diff
  case, now keeps top-level `repr()` custom `__repr__` dispatch aligned with
  CPython: it preserves str-subclass `__repr__` results, while keeping
  blocked or non-string-returning `__repr__` paths as TypeErrors.
- `cpython_ascii_builtin_subset`, backed by
  `cpython_ascii_builtin_diff_subset`, now keeps top-level `ascii()` custom `__repr__` dispatch aligned with CPython: it ASCII-escapes the returned repr text, preserves str-subclass `__repr__` results when no escaping is needed, returns exact `str` after ASCII escaping, and preserves non-string-returning and raising `__repr__` paths, without expanding container-recursive custom `__repr__` dispatch.
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_repr_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_repr_wrapper_descriptor_metadata_diff_subset`,
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_sizeof_method_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_sizeof_method_descriptor_metadata_diff_subset`,
  covering function type `__sizeof__` inherited method_descriptor metadata:
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_dir_method_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_dir_method_descriptor_metadata_diff_subset`,
  covering function type `__dir__` inherited method_descriptor metadata:
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_ne_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_ne_wrapper_descriptor_metadata_diff_subset`,
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_le_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_le_wrapper_descriptor_metadata_diff_subset`,
  ...
  changing function-object rich-compare wrappers.
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_gt_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_gt_wrapper_descriptor_metadata_diff_subset`,
  covering function type `__gt__` inherited wrapper_descriptor metadata:
  `function.__gt__ is object.__gt__`,
  `function.__gt__.__qualname__`,
  `function.__gt__.__objclass__ is object`,
  `function.__gt__.__text_signature__`,
  `dir(function.__gt__)`, object-order forwarding, and TypeError paths without
  changing function-object rich-compare wrappers.
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_ge_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_ge_wrapper_descriptor_metadata_diff_subset`,
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_get_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_function_type_get_wrapper_descriptor_metadata_diff_subset`,
  covering function type `__get__` wrapper_descriptor metadata:
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_mro_metadata_subset` with direct CPython output parity
  in `cpython_function_type_mro_metadata_diff_subset`, covering function type `__mro__` metadata: `function.__mro__ == (function, object)`,
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_doc_metadata_subset` with direct CPython output parity
  in `cpython_function_type_doc_metadata_diff_subset`, covering function type `__doc__` metadata: `function.__doc__`,
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_text_signature_metadata_subset` with direct CPython
  output parity in
...
- `RUNTIME_BUILTINS` also includes
  `cpython_function_type_abstractmethods_absent_metadata_subset` with direct
  CPython output parity in
...
- `RUNTIME_BUILTINS` also includes `cpython_function_get_descriptor_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_get_descriptor_wrapper_diff_subset`, covering function __get__ descriptor wrapper metadata,
...
- `RUNTIME_BUILTINS` also includes `cpython_function_hash_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_hash_wrapper_diff_subset`, covering function __hash__ wrapper metadata,
  `method-wrapper` classification, missing `__module__`, text signature and
  doc metadata, identity-hash delegation through `hash(f)`, method-wrapper
  repr/str shape, and arity/keyword errors without depending on concrete hash or address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_rich_compare_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_rich_compare_wrapper_diff_subset`, covering function __eq__ / __ne__ wrapper metadata,
  `method-wrapper` classification, missing `__module__`, text signature and
  doc metadata, identity comparison for the same function, NotImplemented fallback
  for different functions and non-functions, method-wrapper repr/str shape,
  and arity/keyword errors without depending on concrete address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_order_compare_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_order_compare_wrapper_diff_subset`, covering function ordering wrapper metadata
  for `__lt__`, `__le__`, `__gt__`, and `__ge__`, `method-wrapper` classification,
  missing `__module__`, text signature and doc metadata, NotImplemented result
  for functions and non-functions, method-wrapper repr/str shape, and arity/keyword
  errors without depending on concrete address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_getattribute_wrapper_subset`
  with direct CPython output parity in
...
- `RUNTIME_BUILTINS` also includes
  ...
  method-wrapper repr/str shape, and arity/keyword/name-type errors without
  depending on concrete address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_init_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_init_wrapper_diff_subset`, covering function __init__ wrapper metadata,
  no-op initialization calls with extra positional and keyword arguments,
  custom attribute shadowing, `method-wrapper` classification, missing `__module__`,
  text signature and doc metadata, and repr/str shape without depending on concrete address values.
- `RUNTIME_BUILTINS` also includes
  `cpython_function_init_subclass_wrapper_subset` with direct CPython output parity in
  `cpython_function_init_subclass_wrapper_diff_subset`, covering function __init_subclass__ wrapper metadata,
  type-level builtin method binding through `function`, no-op calls, extra
  positional and keyword TypeError paths, custom attribute shadowing,
  `builtin_function_or_method` classification, text signature and doc metadata,
  and repr/str shape without depending on concrete address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_subclasshook_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_subclasshook_wrapper_diff_subset`, covering function __subclasshook__ wrapper metadata,
  type-level builtin method binding through `function`, NotImplemented fallback,
  missing-argument and keyword TypeError paths, custom attribute shadowing,
  `builtin_function_or_method` classification, text signature and doc metadata,
  and repr/str shape without depending on concrete address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_getstate_wrapper_subset`
  with direct CPython output parity in
  `cpython_function_getstate_wrapper_diff_subset`, covering function __getstate__ wrapper metadata,
  inherited pure-memory no-state results, object.__getstate__ direct call coverage,
  extra positional and keyword TypeError paths, custom attribute shadowing,
  `builtin_function_or_method` classification, text signature and doc metadata,
  and repr/str shape without depending on concrete address values.
- `RUNTIME_BUILTINS` also includes `cpython_function_dir_wrapper_subset` with
  direct CPython output parity in
  `cpython_function_dir_wrapper_diff_subset`, covering function __dir__ wrapper metadata,
  returned name-list membership, object.__dir__ direct list compatibility,
  extra positional and keyword TypeError paths, custom attribute shadowing,
  `builtin_function_or_method` classification, text signature and doc metadata,
...
- `RUNTIME_BUILTINS` also includes `cpython_object_getstate_direct_subset` with
  ...
  outside the sandbox contract.
- `RUNTIME_BUILTINS` also includes
  `cpython_object_getattribute_wrapper_descriptor_metadata_subset` with direct
  CPython output parity in
  `cpython_object_getattribute_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__getattribute__` wrapper_descriptor metadata:
  `object.__getattribute__.__qualname__`,
  `object.__getattribute__.__objclass__ is object`,
  `object.__getattribute__.__text_signature__`,
  and `dir(object.__getattribute__)` metadata membership without changing attribute lookup
  or error semantics.
- `RUNTIME_BUILTINS` also includes
  `cpython_object_setattr_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in `cpython_object_setattr_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__setattr__` wrapper_descriptor metadata:
  ...
  or error semantics.
- `RUNTIME_BUILTINS` also includes
  `cpython_object_delattr_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in `cpython_object_delattr_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__delattr__` wrapper_descriptor metadata:
...
- `RUNTIME_BUILTINS` also includes
  `cpython_object_ne_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in `cpython_object_ne_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__ne__` wrapper_descriptor metadata:
...
- `RUNTIME_BUILTINS` also includes
  ...
  or NotImplemented fallback semantics.
- `RUNTIME_BUILTINS` also includes
  `cpython_object_le_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in `cpython_object_le_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__le__` wrapper_descriptor metadata:
  `object.__le__.__qualname__`,
  `object.__le__.__objclass__ is object`,
  `object.__le__.__text_signature__`,
  and `dir(object.__le__)` metadata membership without changing ordering comparison behavior
  or NotImplemented fallback semantics.
- `RUNTIME_BUILTINS` also includes
  `cpython_object_gt_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in `cpython_object_gt_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__gt__` wrapper_descriptor metadata:
  ...
  or NotImplemented fallback semantics.
- `RUNTIME_BUILTINS` also includes
  `cpython_object_ge_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in `cpython_object_ge_wrapper_descriptor_metadata_diff_subset`,
  covering `object.__ge__` wrapper_descriptor metadata:
...
- `RUNTIME_BUILTINS` also includes
  `cpython_object_repr_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in
  `cpython_object_repr_wrapper_descriptor_metadata_diff_subset`, covering
...
- `RUNTIME_BUILTINS` also includes
  `cpython_object_init_wrapper_descriptor_metadata_subset` with direct CPython
  output parity in
  `cpython_object_init_wrapper_descriptor_metadata_diff_subset`, covering
...
- `RUNTIME_BUILTINS` also includes
  ...
  filename objects, `str` subclass filenames that preserve their public
  `code.co_filename` type, bytes subclass filenames, path-like `str` subclass
  results, and path-like bytes subclass results, memoryview filename rejection, CPython public
...
`cpython_set_empty_pop_keyerror_display_subset` is backed by
`cpython_set_empty_pop_keyerror_display_diff_subset`, keeping exact, direct,
and set-subclass empty `pop()` calls aligned with CPython's
`KeyError.args[0]` payload and `KeyError(message)` string display.

`cpython_set_iterator_mutation_subset` is backed by the `set-iterator-mutation`
...
preserving the subclass instance for direct and syntax forms, accepting
update-source operands, and doing so without changing ordinary dict union result types.
...
| `invalid_assignment` | supported | Invalid annotated targets, including MiniPython's `illegal target for annotation` stop-line for call/operator annotation targets, tuple annotations, missing annotated-assignment expressions, illegal annotation expressions, invalid chained assignment targets, yield assignment, illegal augmented-assignment expressions, and unparenthesized named-expression syntax messages before slice colons are rejected with CPython's `invalid syntax` message by `cpython_invalid_assignment_and_annotation_subset`, `cpython_invalid_assignment_target_subset`, and `cpython_syntax_error_message_parity_diff_subset`; representative literal, keyword, function-call, operator, and walrus-target failures assert parser source spans |
...
| `raise_stmt` | supported | `cpython_grammar_raise_and_try_except_subset` including custom exception classes deriving from `Exception`, custom exception `__class__` identity, and subclass exceptions caught by a custom base handler, `cpython_base_exception_args_subset` / `cpython_base_exception_args_diff_subset` including BaseException argument tuple/display/repr behavior, `cpython_base_exception_with_traceback_subset` / `cpython_base_exception_with_traceback_diff_subset` including traceback object identity preservation, the `with_traceback(None)` path, and non-traceback rejection, `cpython_base_exception_add_note_subset` / `cpython_base_exception_add_note_diff_subset` including public `BaseException.add_note()` and `__notes__` behavior for direct, bound, instance-local, assigned, deleted, subclass, and caught exceptions, `cpython_system_exit_oserror_attributes_subset` / `cpython_system_exit_oserror_attributes_diff_subset` including `SystemExit.code` and `OSError` attribute/args normalization plus CPython-style `OSError.__str__`, `cpython_syntax_error_attributes_subset` / `cpython_syntax_error_attributes_diff_subset` including `SyntaxError` construction attributes, `cpython_unicode_error_attributes_subset` / `cpython_unicode_error_attributes_diff_subset` including Unicode encode/decode/translate construction attributes, `cpython_attribute_error_keyword_attributes_subset` / `cpython_attribute_error_keyword_attributes_diff_subset` including capability-gated `AttributeError(name=..., obj=...)` keyword attributes, `cpython_builtin_exception_hierarchy_subset` / `cpython_builtin_exception_hierarchy_diff_subset` including standard builtin exception base-class catches, `cpython_invalid_simple_statement_subset`, `reports_unhandled_raise`, `preserves_explicit_exception_cause`, `preserves_implicit_exception_context`, `supports_raise_from_none`, `rejects_invalid_exception_cause` |
...
| `invalid_assert_stmt` | supported | Accidental assignment and unparenthesized named-expression syntax messages in assert forms are rejected with CPython's `invalid syntax` message by `cpython_invalid_simple_statement_subset` and `cpython_syntax_error_message_parity_diff_subset` |
...
| `try_stmt` | supported | `cpython_grammar_raise_and_try_except_subset` including custom exception subclass matching and custom exception `__class__` identity, `cpython_base_exception_args_subset` / `cpython_base_exception_args_diff_subset` including caught exception argument preservation, `cpython_base_exception_with_traceback_subset` / `cpython_base_exception_with_traceback_diff_subset` including traceback object identity preservation and catchable invalid-traceback `TypeError`, `cpython_base_exception_add_note_subset` / `cpython_base_exception_add_note_diff_subset` including `add_note()` on caught exceptions and CPython-aligned `__notes__` mutation boundaries, `cpython_system_exit_oserror_attributes_subset` / `cpython_system_exit_oserror_attributes_diff_subset`, `cpython_syntax_error_attributes_subset` / `cpython_syntax_error_attributes_diff_subset`, `cpython_unicode_error_attributes_subset` / `cpython_unicode_error_attributes_diff_subset`, and `cpython_attribute_error_keyword_attributes_subset` / `cpython_attribute_error_keyword_attributes_diff_subset` including capability-gated builtin exception keyword attribute preservation, `cpython_builtin_exception_hierarchy_subset` / `cpython_builtin_exception_hierarchy_diff_subset` including `ArithmeticError` and `LookupError` handler matching plus `GeneratorExit` / `Exception` separation, `cpython_invalid_control_flow_syntax_subset`, `cpython_invalid_control_flow_context_subset`, `cpython_runtime_exception_capture_subset` / `cpython_runtime_exception_capture_diff_subset` including catchable binary add `TypeError`, catchable binary subtract `TypeError`, catchable binary bit-xor `TypeError`, catchable binary bit-and `TypeError`, catchable binary left-shift `TypeError`, catchable binary right-shift `TypeError`, catchable in-place add `TypeError`, catchable in-place subtract `TypeError`, catchable in-place multiply `TypeError`, catchable matrix multiply `TypeError`, catchable in-place true-divide `TypeError`, catchable in-place floor-divide `TypeError`, catchable in-place modulo `TypeError`, catchable in-place power `TypeError`, catchable in-place matrix-multiply `TypeError`, catchable in-place left-shift `TypeError`, catchable in-place right-shift `TypeError`, catchable in-place bit-or `TypeError`, catchable in-place bit-xor `TypeError`, and catchable in-place bit-and `TypeError`, catchable builtin `property`, `staticmethod`, and `classmethod` descriptor method arity, keyword, and invalid owner `TypeError` paths, representative builtin method argument failures such as `list.__iter__` and `dict.keys`, and exact keyword `TypeError` text for `dict.keys`, list methods including `list.__iter__` and `list.append`, `set.add`, `frozenset.__hash__`, `int.bit_length`, and `float.hex`, `cpython_control_flow_in_finally_subset`, `cpython_control_flow_in_finally_override_subset` including issue #37830 pending-return override cases, `cpython_control_flow_inside_except_and_with_subset`, `cpython_grammar_try_star_subset`, `cpython_except_star_split_semantics_subset`, `cpython_except_star_rejects_exception_group_types_subset`, `catches_raised_exceptions`, `catches_tuple_exception_handlers`, `catches_dotted_exception_handler_type`, `catches_dynamic_exception_handler_type_expression`, `runs_except_star_handlers`, `splits_exception_groups_for_except_star_handlers`, `rejects_exception_group_types_in_except_star_handlers`, `runs_try_else_when_no_exception`, `runs_finally_without_exception`, `runs_finally_after_handled_exception`, `runs_finally_before_reraising_exception`, `runs_finally_before_returning_from_function`, `runs_finally_before_breaking_from_loop`, `runs_finally_before_continuing_loop` |
...
| `function_def_raw` | supported | Ordinary `def` and `async def` headers with optional type params, params, return annotations, function type comments, and inline/indented bodies are covered by `cpython_function_def_raw_rule_subset`, `cpython_ast_function_def_subset`, `cpython_func_type_comment_helper_rules_subset`, `cpython_invalid_function_def_raw_subset`, `cpython_syntax_error_message_parity_diff_subset` for missing return annotation expressions and starred return annotation expressions with CPython's `expected ':'` message, `cpython_type_params_metadata_subset`, `cpython_type_param_defaults_subset`, `defines_and_calls_function`, and `runs_function_annotations` |
| `invalid_def_raw` | supported | Missing function-header parentheses, missing return annotation expressions, starred return annotation expressions, missing colons, and missing indented suites for ordinary, async, typed, and type-parameterized definitions are rejected by `cpython_invalid_function_def_raw_subset` including parser diagnostic spans, with `cpython_syntax_error_message_parity_diff_subset` pinning the CPython `expected ':'` message |
| `async_funcdef` | supported | `cpython_async_funcdef_rule_subset`, `cpython_grammar_async_await_subset`, `cpython_grammar_generator_expression_subset`, `cpython_func_type_comment_helper_rules_subset`, `cpython_invalid_function_def_raw_subset`, `cpython_invalid_assignment_target_subset`, `runs_async_function_and_await_expression`, and `runs_coroutine_throw_and_close_methods` cover CPython's `async def` rule shape, empty coroutine bodies, type parameters, complex parameters, return annotations, function type comments, nested async functions, coroutine functions, CPython's conversion of unhandled coroutine `StopIteration` into `RuntimeError` with `__cause__`, CPython-style coroutine `__await__()` wrappers with iterator identity/reuse behavior, non-None send rejection on just-started coroutines, returned `StopIteration` objects staying ordinary return values, custom `__await__` iterators whose yielded values suspend through the outer coroutine and whose return values complete the await expression, CPython-style rejection of objects without `__await__` using the modern `'<type>' object can't be awaited` TypeError text, direct `cpython_program_output_parity_smoke_diff_subset` coverage for `Lib/test/test_coroutines.py::test_await_8`, rejection of `__await__` returning `None`, a coroutine, or another non-iterator object, awaited expression composition, nested await, await in keyword arguments and tuple values, send/throw forwarding into suspended await expressions including CPython's custom-exception `test_await_14` shape, rejection of awaiting an already-suspended coroutine, returned exception values with cleared await context, async generators, async-generator `anext`/`asend`/`athrow`/`aclose`, `StopAsyncIteration` exhaustion, and CPython's rejection of `yield from` inside async functions and `return value` inside async generators |
| `class_def` | supported | Decorated and undecorated class definitions are covered by `cpython_class_def_decorated_rule_subset`, `cpython_grammar_class_def_subset`, `cpython_class_def_raw_helper_rules_subset`, `cpython_grammar_decorators_subset`, `cpython_grammar_annotations_subset`, `cpython_type_params_metadata_subset`, `cpython_type_params_dunder_subset`, `cpython_type_param_defaults_subset`, `cpython_type_params_generic_alias_subset`, `cpython_invalid_type_scope_expression_subset`, `cpython_invalid_assignment_target_subset`, `defines_and_instantiates_class`, `runs_class_attributes`, `cpython_compile_static_attributes_exact_subset` for CPython-style `__static_attributes__`, `runs_class_decorators`, `runs_class_annotations`, `runs_class_inheritance_and_header_arguments`, CPython parity for user-class `isinstance` over direct instances and base classes, `cpython_issubclass_builtin_subset` for supported class hierarchy checks, `cpython_type_builtin_subset` for one-argument `type()`, `cpython_type_dynamic_class_subset` for first-pass three-argument dynamic class construction, `cpython_type_base_readonly_subset` / `cpython_type_base_readonly_diff_subset` for type `__base__` readonly AttributeError behavior that does not shadow the class dictionary, `cpython_type_mro_readonly_subset` / `cpython_type_mro_readonly_diff_subset` for type `__mro__` readonly AttributeError behavior that does not shadow the class dictionary, `cpython_type_dict_readonly_subset` / `cpython_type_dict_readonly_diff_subset` for type `__dict__` readonly AttributeError behavior that guards against int/None shadow values, `cpython_type_name_qualname_subset` / `cpython_type_name_qualname_diff_subset` for mutable dynamic-class `__name__` and `__qualname__`, `cpython_type_metadata_assignment_error_names_subset` / `cpython_type_metadata_assignment_error_names_diff_subset` for class metadata assignment TypeError behavior over `__name__` and `__qualname__` that uses the current public class name, `cpython_type_repr_module_qualname_subset` / `cpython_type_repr_module_qualname_diff_subset` for type `repr()` / `str()` module-qualified class names, `__mro__` tuple repr, and the `builtins` and non-string `__module__` boundaries, `cpython_type_metadata_delete_errors_subset` / `cpython_type_metadata_delete_errors_diff_subset` for class metadata deletion TypeError behavior over `__name__`, `__qualname__`, `__module__`, and `__doc__` that preserves reassigned metadata values, `cpython_type_doc_and_firstlineno_subset` / `cpython_type_doc_and_firstlineno_diff_subset` for dynamic-class `__doc__` and `__firstlineno__`, `cpython_type_bad_slots_subset` for public invalid dynamic-class `__slots__` error classes, `runs_slots_attribute_restrictions` for slot member descriptor access and method-wrapper `__get__` / `__set__` / `__delete__` errors including modern `__get__` / `__set__ expected ...` descriptor arity text with `cpython_descriptor_method_wrapper_arity_diff_subset` coverage, `cpython_type_nokwargs_subset` for `type()` keyword rejection, `cpython_type_typeparams_subset` for class `__type_params__` assignment/delete behavior, `cpython_type_typeparams_delete_error_name_subset` / `cpython_type_typeparams_delete_error_name_diff_subset` for type `__type_params__` deletion TypeError behavior that uses the current public class name and preserves the assigned override, `cpython_type_namespace_order_subset` for minimal `OrderedDict` metadata/display/recursive-display/copy/equality/generic-alias/fromkeys/popitem/reversed/union coverage including dict-style `OrderedDict({...})` repr with `cpython_ordered_dict_modern_repr_diff_subset` coverage plus ordered dynamic-class namespace preservation, `cpython_vars_dir_builtin_subset` for first-pass class/instance `vars()` and `dir()` introspection, `cpython_bound_method_metadata_subset` for bound-method metadata and class-body method aliasing, `cpython_bound_method_descriptor_and_repr_subset` for bound-method `__get__`, method-wrapper error paths, and stable repr metadata, `cpython_bound_method_identity_subset` for stored bound-method identity and fresh attribute-access method objects, `cpython_unbound_super_descriptor_subset` for one-argument/unbound `super` descriptor rebinding and metadata, `runs_function_class_and_alias_type_params`, `runs_generic_alias_type_subscripts`, and `runs_instance_methods` |
...
| `or_pattern` | supported | Literal/value OR-patterns, same-name capture alternatives, reordered capture alternatives, mapping alternatives, parenthesized `as` alternatives, non-final irrefutable alternatives, empty OR-pattern alternatives with CPython-style `invalid syntax`, and different-name binding errors are covered by `cpython_match_pattern_helper_rules_subset` / `cpython_match_pattern_helper_rules_diff_subset`, `cpython_grammar_match_stmt_subset`, and `runs_match_or_pattern` |
...
| `literal_pattern` | supported | Numeric, signed, complex, adjacent-string, boolean, and None literal patterns are covered by `cpython_grammar_match_stmt_subset`; f-string and t-string match values are rejected by the parser with CPython's `patterns may only match literals and attribute lookups` message, and invalid unary expression starts such as `+x`, `~x`, `not x`, and non-numeric negative starts such as `-x` are rejected with CPython-style `invalid syntax` |
| `literal_expr` | supported | Literal mapping keys, adjacent-string keys, complex keys, singleton keys, dotted value keys, duplicate literal-key rejection, and rejected f-string/t-string mapping keys with CPython's `mapping pattern keys may only match literals and attribute lookups` message are covered by `cpython_match_pattern_helper_rules_subset` / `cpython_match_pattern_helper_rules_diff_subset`, `cpython_match_mapping_helper_rules_subset`, and `cpython_grammar_match_stmt_subset` |
...
| `wildcard_pattern` | supported | Wildcard cases and grouped wildcards are covered by `cpython_match_capture_wildcard_group_helper_rules_subset` / `cpython_match_capture_wildcard_group_helper_rules_diff_subset`, `cpython_grammar_match_stmt_subset`, and `runs_match_wildcard_case`; wildcard arithmetic match patterns such as `case _ + y`, wildcard bitwise match patterns such as `case _ & y`, wildcard comparison match patterns such as `case _ == y`, wildcard boolean match patterns such as `case _ and y`, wildcard assignment-form match patterns such as `case _ := y`, wildcard reserved-keyword match patterns such as `case _ lambda y`, wildcard arrow match patterns such as `case _ -> y`, wildcard selector/call match patterns such as `case _.x`, wildcard literal/singleton-adjacent match patterns such as `case _ None`, wildcard interpolated-string-adjacent match patterns such as `case _ f'{1}'` and `case _ t'{1}'`, wildcard semicolon-adjacent match patterns such as `case _ ;`, wildcard unary-invert match patterns such as `case _ ~ y`, and wildcard brace-display-adjacent match patterns such as `case _ {}` are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `invalid syntax` messages |
| `value_pattern` | supported | Simple and recursive dotted value patterns, dotted mapping keys, and invalid equality/dangling-dot/leading-dot forms with CPython-style `invalid syntax` messages are covered by `cpython_match_value_attr_name_or_attr_helper_rules_subset` / `cpython_match_value_attr_name_or_attr_helper_rules_diff_subset`, `cpython_match_pattern_helper_rules_subset`, and `runs_match_value_patterns`; bare-name equality match patterns such as `case x=1`, bare-name subscript match patterns such as `case x[0]`, bare-name arithmetic match patterns such as `case x + y`, bare-name bitwise match patterns such as `case x & y`, bare-name comparison match patterns such as `case x == y`, bare-name boolean match patterns such as `case x and y`, bare-name assignment-operator match patterns such as `case x := y`, bare-name reserved-keyword match patterns such as `case x lambda y`, and bare-name arrow match patterns such as `case x -> y` are rejected by `cpython_invalid_match_pattern_subset` |
...
| `maybe_star_pattern` | supported | Plain and starred sequence subpatterns, including wildcard star targets, invalid non-name star targets with CPython-style `invalid syntax`, and duplicate-star rejection with CPython's `multiple starred names in sequence pattern` message, are covered by `cpython_match_sequence_helper_rules_subset` / `cpython_match_sequence_helper_rules_diff_subset`, `cpython_match_pattern_helper_rules_subset`, `cpython_invalid_match_pattern_subset`, `cpython_grammar_match_stmt_subset`, and `runs_match_star_sequence_patterns` |
| `star_pattern` | supported | `*name` and `*_` sequence star patterns plus CPython-style `invalid syntax` messages for standalone star patterns, standalone double-star patterns, and invalid sequence-star capture targets are covered by `cpython_match_capture_target_and_star_pattern_helper_rules_subset` / `cpython_match_capture_target_and_star_pattern_helper_rules_diff_subset`, `cpython_grammar_match_stmt_subset`, `cpython_invalid_match_pattern_subset`, and `runs_match_star_sequence_patterns` |
...
| `invalid_match_stmt` | supported | Missing match-subject colons across literal, name, call, tuple, list, dict, conditional, and EOF subjects are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `expected ':'` messages; match-subject semicolon forms such as `match 1; print(2)` and `match x; print(2)` are rejected with CPython-style `invalid syntax` messages; indented non-case match suites such as `match 1:\n    pass` are rejected with CPython-style `invalid syntax`; missing match indentation forms such as `match 1:\ncase 1:\n    pass`, `match 1:\npass`, and `match 1:\n` use CPython's `expected an indented block after 'match' statement` message; top-level case blocks such as `case 1:`, `case x:`, `case None:`, and mapping-shaped cases are rejected with CPython-style `invalid syntax` while legal soft-keyword uses remain expressions or assignments |
| `invalid_case_block` | supported | Missing case-block colons across literal, guarded, wildcard, capture, sequence, mapping, class, OR, and as-pattern cases are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `expected ':'` messages; case-block semicolon forms such as `case 1; print(2)`, `case x; print(x)`, guarded cases, and sequence patterns are rejected with CPython-style `invalid syntax`; missing case indentation forms such as `case 1:\npass`, `case 1:\n    case 2:`, EOF after `case 1:`, and guarded cases use CPython's `expected an indented block after 'case' statement` message; missing case patterns are also rejected by `cpython_invalid_match_pattern_subset` |
| `invalid_as_pattern` | supported | `_`, bare and parenthesized literal expression targets with `cannot use literal as pattern target`, bare and parenthesized `None` targets with `cannot use None as pattern target`, bare and parenthesized `True` targets with `cannot use True as pattern target`, bare and parenthesized `False` targets with `cannot use False as pattern target`, bare and parenthesized ellipsis targets with `cannot use ellipsis as pattern target`, bare and parenthesized f-string targets with `cannot use f-string expression as pattern target`, bare and parenthesized t-string targets with `cannot use t-string expression as pattern target`, unary expression targets including `-1`, `+x`, `~x`, `not x`, `((-1))`, `((+x))`, `((~x))`, and `((not x))` with `cannot use expression as pattern target`, invalid parenthesized expression as-pattern targets including nested-parenthesized expression forms with `cannot use expression as pattern target`, invalid parenthesized comparison as-pattern targets including nested-parenthesized comparison forms with `cannot use comparison as pattern target`, invalid parenthesized named-expression as-pattern targets with `cannot use named expression as pattern target`, invalid parenthesized conditional-expression as-pattern targets with `cannot use conditional expression as pattern target`, invalid parenthesized yield-expression as-pattern targets with `cannot use yield expression as pattern target`, invalid parenthesized await-expression as-pattern targets with `cannot use await expression as pattern target`, bare and parenthesized invalid lambda as-pattern targets with `cannot use lambda as pattern target`, invalid generator-expression as-pattern targets with `cannot use generator expression as pattern target`, invalid star as-pattern targets with CPython-style `invalid syntax`, bare and parenthesized invalid list as-pattern targets with `cannot use list as pattern target`, invalid list-comprehension as-pattern targets with `cannot use list comprehension as pattern target`, invalid dict-comprehension as-pattern targets with `cannot use dict comprehension as pattern target`, invalid set-comprehension as-pattern targets with `cannot use set comprehension as pattern target`, bare and parenthesized invalid dict as-pattern targets with `cannot use dict literal as pattern target`, invalid set-display as-pattern targets with `cannot use set display as pattern target`, invalid parenthesized-name as-pattern targets including nested-parenthesized and guard forms with `cannot use name as pattern target`, invalid tuple as-pattern targets including nested-parenthesized tuple forms with `cannot use tuple as pattern target`, CPython `test_syntax.py` attribute targets after `case ... as`, invalid parenthesized attribute as-pattern targets including nested-parenthesized attribute forms with `cannot use attribute as pattern target`, call-form plus invalid parenthesized call as-pattern targets including nested-parenthesized and call-result forms with `cannot use function call as pattern target`, invalid parenthesized subscript as-pattern targets including nested-parenthesized and subscript-result forms with `cannot use subscript as pattern target`, invalid bare subscript as-pattern targets with CPython-style `invalid syntax`, and missing as-pattern targets with CPython-style `invalid syntax` are rejected by `cpython_invalid_match_pattern_subset` |
...
| `invalid_mapping_pattern` | supported | Mapping rest patterns before other mapping items, both with and without preceding items, including CPython-style `invalid syntax` messages for mapping `**rest` before later mapping items, invalid mapping rest capture targets including wildcard, attribute, call, subscript, unary, and literal targets, missing mapping pattern values such as `case {1:}`, missing mapping pattern key colons such as `case {1}` and `case {1, 2}`, non-literal mapping keys such as bare-name, call, and list keys, and invalid f-string/t-string mapping keys with CPython's `mapping pattern keys may only match literals and attribute lookups` message are rejected by `cpython_invalid_match_pattern_subset` |
...
| `expression` | supported | The `invalid_if_expression`, `invalid_expression`, `invalid_legacy_expression`, `if_expression`, `disjunction`, and `lambdef` alternatives are covered by `cpython_invalid_expression_rules_subset`, `cpython_grammar_conditional_expression_subset`, `cpython_expression_without_invalid_subset`, `cpython_grammar_boolean_operations_subset`, `cpython_grammar_lambda_subset`, `runs_boolean_operators`, `evaluates_eval_input_expression`, and `cpython_compile_expression_stack_size_shapes_subset` long-expression compile-shape checks; standalone bang syntax forms, prefix bitwise operator expression forms, prefix comparison operator expression forms, prefix division/modulo operator expression forms, prefix assignment/annotation marker expression forms, prefix punctuation expression forms, prefix at operator expression forms, operator-follower starred expression forms, bare starred expression forms with `can't use starred expression here`, and prefix augmented assignment operator expression forms are rejected with CPython's public messages |
...
| `invalid_starred_expression_unpacking_sequence` | supported | leading list double-star unpacking forms, list tail double-star unpacking forms, set tail double-star unpacking forms, and tuple double-star unpacking forms with CPython-style `invalid syntax` for comma-bearing sequences while `(**items)` keeps `cannot use double starred expression here`, dict unpacking in starred expression sequences, and nested invalid starred unpacking are rejected by `cpython_invalid_starred_expression_subset` |
...
| `invalid_expression` | supported | Missing commas, string-adjacent expressions, incomplete conditional expressions, CPython `test_syntax.py` conditional-expression branches that accidentally contain statements, statement-in-expression positions, standalone bang syntax forms, prefix bitwise operator expression forms, prefix comparison operator expression forms, prefix division/modulo operator expression forms, prefix assignment/annotation marker expression forms, prefix punctuation expression forms, prefix at operator expression forms, operator-follower starred expression forms, bare starred expression forms with `can't use starred expression here`, prefix augmented assignment operator expression forms, and unparenthesized f-string/t-string lambda expressions are rejected by `cpython_invalid_expression_rules_subset` |
...
| `invalid_double_starred_kvpairs` | supported | Missing dictionary values, starred dictionary key forms and double-starred dictionary key forms with CPython-style `invalid syntax`, double-starred dictionary values after plain or `**` entries with CPython-style `invalid syntax`, unparenthesized conditional dict unpacking forms with CPython-style `invalid syntax`, and dict-unpack comprehension forms are rejected by `cpython_invalid_dict_display_syntax_subset` |
| `invalid_kvpair_unpacking` | supported | Starred dictionary key forms and double-starred dictionary key forms with CPython-style `invalid syntax`, double-starred dictionary value forms with CPython-style `invalid syntax`, starred dictionary values with CPython's dedicated value message, and unparenthesized conditional dict unpacking forms with CPython-style `invalid syntax` are rejected by `cpython_invalid_dict_display_syntax_subset` |
| `invalid_kvpair` | supported | Missing dictionary key colons, missing dictionary values, double-starred dictionary value forms with CPython-style `invalid syntax`, and starred dictionary values with CPython's dedicated value message are rejected by `cpython_invalid_dict_display_syntax_subset` |
...
direct `Counter.__add__()` binary addition and public `self` / `other` binding
diagnostics,
...
conversion boundaries plus public `self` / keyword argument binding
...
arguments, and CPython-style `Counter.__init__()` / `Counter.update()`
...
`setdefault()`, `setdefault()` descriptor-style argument diagnostics,
`get()` descriptor-style argument diagnostics, `items()` descriptor-style argument diagnostics,
`keys()` descriptor-style argument diagnostics, `values()` descriptor-style argument diagnostics,
`popitem()`, empty `popitem()` preserving CPython's `KeyError(message)` string
display, `clear()`, `clear()` descriptor-style argument diagnostics, zero lookup
after deletion, and `popitem()` descriptor-style argument diagnostics.
...
comparison over count values, including direct `Counter.__eq__()` /
`Counter.__ne__()` / ordering public dunder calls. The direct diff is gated
because older system CPython oracles predate `Counter.total()` and the new
zero-count rich-comparison rules.
...
| CPython rule | Status | Rust evidence |
| --- | --- | --- |
| `params` | supported | Valid function parameter alternatives and invalid function parameter alternatives are covered by `cpython_function_parameter_helper_rules_subset`, `cpython_invalid_parameter_syntax_subset`, `cpython_invalid_parameters_subset`, `cpython_syntax_error_message_parity_diff_subset` for missing parameter annotation expressions with CPython's `invalid syntax` message, and `cpython_ast_function_def_subset` |
...
| `param` | supported | Parameter names with and without annotations across positional, keyword-only, starred, and `**kwargs` forms are covered by `cpython_function_parameter_helper_rules_subset`, `cpython_invalid_parameters_subset`, `cpython_syntax_error_message_parity_diff_subset` for missing parameter annotation expressions before `)`, `,`, and `=`, `cpython_grammar_annotations_subset`, and `runs_function_annotations` |
| `param_star_annotation` | supported | Starred parameter annotations are covered by `cpython_function_parameter_helper_rules_subset`, `cpython_star_annotation_helper_rule_subset`, and `cpython_invalid_parameters_subset` for bare starred and double-starred parameter annotation expressions with CPython's `invalid syntax` message |
| `star_annotation` | supported | Starred parameter annotations, including annotations before defaults, are covered by `cpython_star_annotation_helper_rule_subset`; bare starred and double-starred parameter annotation expressions are rejected by `cpython_invalid_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` pinning CPython's `invalid syntax` message |
| `default` | supported | Valid parameter default expressions and invalid missing default expressions are covered by `cpython_function_parameter_helper_rules_subset`, `cpython_ast_function_defaults_and_keywords_subset`, `cpython_invalid_parameters_subset`, `cpython_invalid_lambda_parameters_subset`, `cpython_syntax_error_message_parity_diff_subset` for lambda default values before body colons with CPython's `invalid syntax` message, and `runs_function_default_parameters` |
| `invalid_parameters` | supported | Positional-only marker ordering, including empty positional-only marker forms with CPython's `at least one argument must precede /` message, default ordering before and after `/`, duplicate parameter name forms with CPython's `duplicate argument` message, singleton parameter name forms, reserved keyword parameter name forms, literal parameter name forms, ellipsis parameter name forms, unary operator parameter name forms, list/dict display parameter name forms, postfix expression parameter name forms, binary arithmetic parameter name forms, comparison operator parameter name forms, keyword comparison parameter name forms, boolean operator parameter name forms, bitwise operator parameter name forms, shift operator parameter name forms, floor division parameter name forms, matrix multiply parameter name forms, power operator parameter name forms, conditional expression parameter name forms, walrus expression parameter name forms, arrow parameter name forms, for-clause parameter name forms, yield expression parameter name forms, await expression parameter name forms, else-clause parameter name forms, identifier follower parameter name forms, statement keyword parameter name forms, definition keyword parameter name forms, declaration/import keyword parameter name forms, block/exception keyword parameter name forms, augmented assignment parameter name forms, semicolon separator parameter name forms, unary invert follower parameter name forms, left brace follower parameter name forms, ellipsis follower parameter name forms, string literal follower parameter name forms, integer literal follower parameter name forms, large integer literal follower parameter name forms, float literal follower parameter name forms, imaginary literal follower parameter name forms, bytes literal follower parameter name forms, true literal follower parameter name forms, false literal follower parameter name forms, none literal follower parameter name forms, f-string literal follower parameter name forms, t-string literal follower parameter name forms, and bang follower parameter name forms with CPython's `invalid syntax` message, parenthesized parameters, missing parameter annotation expressions before `)`, `,`, and `=`, bare starred parameter annotation expressions, double-starred parameter annotation expressions, and slash/star ordering are rejected by `cpython_invalid_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` evidence |
| `invalid_default` | supported | Missing default values before `)`, `,`, and lambda default values before body colons are rejected by `cpython_invalid_parameters_subset` and `cpython_invalid_lambda_parameters_subset`, with `cpython_syntax_error_message_parity_diff_subset` pinning CPython's `invalid syntax` message for the lambda body-colon boundary |
| `invalid_star_etc` | supported | Bare star, bare-star type-comment, `*, **kwargs`, vararg default, and repeated star forms are rejected by `cpython_invalid_parameters_subset`; bare-star parameter forms use CPython's `named arguments must follow bare *` message with `cpython_syntax_error_message_parity_diff_subset` evidence |
| `invalid_kwds` | supported | Kwarg defaults and parameters or `*`/`**`/`/` markers after `**kwargs` are rejected by `cpython_invalid_parameters_subset`; var-keyword follower forms use CPython's `arguments cannot follow var-keyword argument` message with `cpython_syntax_error_message_parity_diff_subset` evidence |
...
| `lambda_star_etc` | supported | Lambda varargs, keyword-only parameters, `**kwargs`, and invalid star alternatives are covered by `cpython_lambda_parameter_helper_rules_subset`, `cpython_grammar_lambda_subset`, `cpython_invalid_lambda_parameter_syntax_subset`, `cpython_invalid_lambda_parameters_subset`, `cpython_syntax_error_message_parity_diff_subset` for vararg default values with CPython's `cannot have default value` messages, and `runs_lambda_defaults_keywords_and_starred_parameters` |
| `lambda_kwds` | supported | Lambda `**kwargs`, invalid defaults, and invalid followers are covered by `cpython_lambda_parameter_helper_rules_subset`, `cpython_grammar_lambda_subset`, `cpython_invalid_lambda_parameter_syntax_subset`, `cpython_invalid_lambda_parameters_subset`, `cpython_syntax_error_message_parity_diff_subset` for var-keyword default values with CPython's `cannot have default value` message, and `runs_lambda_defaults_keywords_and_starred_parameters` |
| `invalid_lambda_parameters` | supported | Positional-only marker ordering, including empty positional-only marker forms with CPython's `at least one argument must precede /` message, default ordering before and after `/`, duplicate parameter name forms with CPython's `duplicate argument` message, singleton parameter name forms, reserved keyword parameter name forms, literal parameter name forms, ellipsis parameter name forms, unary operator parameter name forms, list/dict display parameter name forms, postfix expression parameter name forms, binary arithmetic parameter name forms, comparison operator parameter name forms, keyword comparison parameter name forms, boolean operator parameter name forms, bitwise operator parameter name forms, shift operator parameter name forms, floor division parameter name forms, matrix multiply parameter name forms, power operator parameter name forms, conditional expression parameter name forms, walrus expression parameter name forms, arrow parameter name forms, for-clause parameter name forms, yield expression parameter name forms, await expression parameter name forms, else-clause parameter name forms, identifier follower parameter name forms, statement keyword parameter name forms, definition keyword parameter name forms, declaration/import keyword parameter name forms, block/exception keyword parameter name forms, augmented assignment parameter name forms, semicolon separator parameter name forms, unary invert follower parameter name forms, left brace follower parameter name forms, ellipsis follower parameter name forms, string literal follower parameter name forms, integer literal follower parameter name forms, large integer literal follower parameter name forms, float literal follower parameter name forms, imaginary literal follower parameter name forms, bytes literal follower parameter name forms, true literal follower parameter name forms, false literal follower parameter name forms, none literal follower parameter name forms, f-string literal follower parameter name forms, t-string literal follower parameter name forms, and bang follower parameter name forms with CPython's `invalid syntax` message, parenthesized parameters, and slash/star ordering are rejected by `cpython_invalid_lambda_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` evidence |
...
| `invalid_lambda_star_etc` | supported | Bare star, `*, **kwargs`, vararg default, and repeated star lambda forms are rejected by `cpython_invalid_lambda_parameters_subset`; bare-star parameter forms use CPython's `named arguments must follow bare *` message with `cpython_syntax_error_message_parity_diff_subset` evidence |
| `invalid_lambda_kwds` | supported | Kwarg defaults and parameters or `*`/`**`/`/` markers after `**kwargs` are rejected by `cpython_invalid_lambda_parameters_subset`; var-keyword follower forms use CPython's `arguments cannot follow var-keyword argument` message with `cpython_syntax_error_message_parity_diff_subset` evidence |
...
`cpython_tuple_constructor_keyword_error_subset`, backed by
`cpython_tuple_constructor_keyword_error_diff_subset`, covers the
