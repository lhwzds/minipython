...
| `invalid_double_starred_kvpairs` | supported | Missing dictionary values, invalid starred/double-starred dictionary keys and values after plain or `**` entries, unparenthesized conditional dict unpacking, and dict-unpack comprehension forms are rejected by `cpython_invalid_dict_display_syntax_subset` |
