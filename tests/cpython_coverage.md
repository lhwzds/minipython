...
| `invalid_mapping_pattern` | supported | Mapping rest patterns before other mapping items, both with and without preceding items, including CPython-style `invalid syntax` messages for mapping `**rest` before later mapping items, invalid mapping rest capture targets including wildcard, attribute, call, subscript, unary, and literal targets, non-literal mapping keys such as bare-name, call, and list keys, and invalid f-string/t-string mapping keys with CPython's `mapping pattern keys may only match literals and attribute lookups` message are rejected by `cpython_invalid_match_pattern_subset` |
