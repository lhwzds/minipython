...
| `star_pattern` | supported | `*name` and `*_` sequence star patterns plus CPython-style `invalid syntax` messages for standalone star patterns and invalid sequence-star capture targets are covered by `cpython_match_capture_target_and_star_pattern_helper_rules_subset` / `cpython_match_capture_target_and_star_pattern_helper_rules_diff_subset`, `cpython_grammar_match_stmt_subset`, `cpython_invalid_match_pattern_subset`, and `runs_match_star_sequence_patterns` |
