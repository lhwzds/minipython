...
| `function_def_raw` | supported | Ordinary `def` and `async def` headers with optional type params, params, return annotations, function type comments, and inline/indented bodies are covered by `cpython_function_def_raw_rule_subset`, `cpython_ast_function_def_subset`, `cpython_func_type_comment_helper_rules_subset`, `cpython_invalid_function_def_raw_subset`, `cpython_syntax_error_message_parity_diff_subset` for missing return annotation expressions with CPython's `expected ':'` message, `cpython_type_params_metadata_subset`, `cpython_type_param_defaults_subset`, `defines_and_calls_function`, and `runs_function_annotations` |
| `invalid_def_raw` | supported | Missing function-header parentheses, missing return annotation expressions, missing colons, and missing indented suites for ordinary, async, typed, and type-parameterized definitions are rejected by `cpython_invalid_function_def_raw_subset` including parser diagnostic spans, with `cpython_syntax_error_message_parity_diff_subset` pinning the CPython `expected ':'` message |
