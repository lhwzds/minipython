...
| `invalid_lambda_parameters` | supported | Positional-only marker ordering, including empty positional-only marker forms with CPython's `at least one argument must precede /` message, default ordering before and after `/`, duplicate parameter name forms with CPython's `duplicate argument` message, singleton parameter name forms, reserved keyword parameter name forms, literal parameter name forms, ellipsis parameter name forms, unary operator parameter name forms, list/dict display parameter name forms, postfix expression parameter name forms, binary arithmetic parameter name forms, comparison operator parameter name forms, keyword comparison parameter name forms, boolean operator parameter name forms, bitwise operator parameter name forms, shift operator parameter name forms, and floor division parameter name forms with CPython's `invalid syntax` message, parenthesized parameters, and slash/star ordering are rejected by `cpython_invalid_lambda_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` evidence |
