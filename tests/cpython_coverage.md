...
| `literal_pattern` | supported | Numeric, signed, complex, adjacent-string, boolean, and None literal patterns are covered by `cpython_grammar_match_stmt_subset`; f-string and t-string match values are rejected by the parser with CPython's `patterns may only match literals and attribute lookups` message |
