...
| `wildcard_pattern` | supported | Wildcard cases and grouped wildcards are covered by `cpython_match_capture_wildcard_group_helper_rules_subset` / `cpython_match_capture_wildcard_group_helper_rules_diff_subset`, `cpython_grammar_match_stmt_subset`, and `runs_match_wildcard_case`; wildcard arithmetic match patterns such as `case _ + y` are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `invalid syntax` messages |
