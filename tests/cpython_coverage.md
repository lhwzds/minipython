...
| `invalid_match_stmt` | supported | Missing match-subject colons across literal, name, call, tuple, list, dict, conditional, and EOF subjects are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `expected ':'` messages; missing match indentation and top-level case blocks are also rejected by `cpython_invalid_match_pattern_subset` |
