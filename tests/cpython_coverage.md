...
- `cpython_runtime_exception_capture_subset`, backed by
  ...
  modulo `TypeError` behavior, catchable in-place left-shift `TypeError`
  behavior, catchable in-place right-shift `TypeError` behavior, catchable
  in-place bit-or `TypeError` behavior, catchable in-place bit-xor
