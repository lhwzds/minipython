...
| `maybe_star_pattern` | supported | Plain and starred sequence subpatterns, including wildcard star targets and duplicate-star rejection with CPython's `multiple starred names in sequence pattern` message, are covered by `cpython_match_sequence_helper_rules_subset` / `cpython_match_sequence_helper_rules_diff_subset`, `cpython_match_pattern_helper_rules_subset`, `cpython_invalid_match_pattern_subset`, `cpython_grammar_match_stmt_subset`, and `runs_match_star_sequence_patterns` |
