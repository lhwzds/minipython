...
| `invalid_assignment` | supported | Invalid annotated targets, including MiniPython's `illegal target for annotation` stop-line for call/operator annotation targets, tuple annotations, missing annotated-assignment expressions, illegal annotation expressions, invalid chained assignment targets, yield assignment, illegal augmented-assignment expressions, and unparenthesized named expressions before slice colons are rejected by `cpython_invalid_assignment_and_annotation_subset`, `cpython_invalid_assignment_target_subset`, and `cpython_syntax_error_message_parity_diff_subset`; representative literal, keyword, function-call, operator, and walrus-target failures assert parser source spans |
