...
| `invalid_parameters` | supported | Positional-only marker ordering, default ordering before and after `/`, parenthesized parameters, missing parameter annotation expressions before `)`, `,`, and `=`, bare starred parameter annotation expressions, double-starred parameter annotation expressions, and slash/star ordering are rejected by `cpython_invalid_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` pinning the CPython `invalid syntax` message |
