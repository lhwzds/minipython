...
| `invalid_case_block` | supported | Missing case-block colons across literal, guarded, wildcard, capture, sequence, mapping, class, OR, and as-pattern cases are rejected by `cpython_invalid_match_pattern_subset` with CPython-style `expected ':'` messages; missing case indentation forms such as `case 1:\npass`, `case 1:\n    case 2:`, EOF after `case 1:`, and guarded cases use CPython's `expected an indented block after 'case' statement` message; missing case patterns are also rejected by `cpython_invalid_match_pattern_subset` |
