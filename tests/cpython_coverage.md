...
| `invalid_parameters` | supported | Positional-only marker ordering, including empty positional-only marker forms with CPython's `at least one argument must precede /` message, default ordering before and after `/`, duplicate parameter name forms with CPython's `duplicate argument` message, singleton parameter name forms and reserved keyword parameter name forms with CPython's `invalid syntax` message, parenthesized parameters, missing parameter annotation expressions before `)`, `,`, and `=`, bare starred parameter annotation expressions, double-starred parameter annotation expressions, and slash/star ordering are rejected by `cpython_invalid_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` evidence |
...
| `invalid_lambda_parameters` | supported | Positional-only marker ordering, including empty positional-only marker forms with CPython's `at least one argument must precede /` message, default ordering before and after `/`, duplicate parameter name forms with CPython's `duplicate argument` message, singleton parameter name forms and reserved keyword parameter name forms with CPython's `invalid syntax` message, parenthesized parameters, and slash/star ordering are rejected by `cpython_invalid_lambda_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` evidence |
