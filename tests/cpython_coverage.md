...
- `collections.deque` is currently a minimal sandbox type surface only:
  ...
  display, direct type-level public method calls, basic `append` /
  `appendleft` / `extend` / `extendleft` / `insert` / `remove` / `pop` /
  `popleft` / `count` / `index` / `rotate` / `reverse` / `clear` / `copy`,
  membership, rich comparison between deque instances, integer
  indexing/assignment/deletion, concat/repeat, in-place concat/repeat,
