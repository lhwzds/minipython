...
`keys()` descriptor-style argument diagnostics, `popitem()`, empty `popitem()`
preserving CPython's `KeyError(message)` string display, `clear()`,
`clear()` descriptor-style argument diagnostics, zero lookup after deletion, and
