...
| `param_star_annotation` | supported | Starred parameter annotations are covered by `cpython_function_parameter_helper_rules_subset`, `cpython_star_annotation_helper_rule_subset`, and `cpython_invalid_parameters_subset` for bare starred parameter annotation expressions with CPython's `invalid syntax` message |
| `star_annotation` | supported | Starred parameter annotations, including annotations before defaults, are covered by `cpython_star_annotation_helper_rule_subset`; bare starred parameter annotation expressions are rejected by `cpython_invalid_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` pinning CPython's `invalid syntax` message |
...
| `invalid_parameters` | supported | Positional-only marker ordering, default ordering before and after `/`, parenthesized parameters, missing parameter annotation expressions before `)`, `,`, and `=`, bare starred parameter annotation expressions, and slash/star ordering are rejected by `cpython_invalid_parameters_subset` with `cpython_syntax_error_message_parity_diff_subset` pinning the CPython `invalid syntax` message |
