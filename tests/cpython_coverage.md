...
- `collections.deque` is currently a minimal sandbox type surface only:
  ...
  display, basic `append` / `appendleft` / `extend` / `extendleft` /
  `insert` / `remove` / `pop` / `popleft` / `count` / `index` / `rotate` /
  `reverse` / `clear` / `copy`, membership, rich comparison between deque
  instances, integer indexing/assignment/deletion, concat/repeat, in-place
  concat/repeat, `.copy()` / `__copy__()` / `copy.copy()`, reverse iteration,
  concrete type identity, and `MutableSequence` registration. Full deque construction/mutation APIs,
