...
- `cpython_runtime_exception_capture_subset`, backed by
  ...
  modulo `TypeError` behavior, catchable in-place bit-or `TypeError` behavior, and
  catchable in-place bit-and `TypeError` behavior with unsupported
  `+=` / `-=` / `/=` / `//=` / `%=` / `|=` / `&=` text,
