...
| `test_type_name` | `partial` | `cpython_type_name_qualname_subset` and `cpython_type_name_qualname_diff_subset` cover valid dynamic-class names, writable `__name__`, bytes-name rejection, and NUL-containing name rejection while preserving existing metadata. `cpython_type_repr_module_qualname_subset` and `cpython_type_repr_module_qualname_diff_subset` cover type `repr()` / `str()` module-qualified class names and `__mro__` tuple repr. `cpython_type_metadata_delete_errors_subset` and `cpython_type_metadata_delete_errors_diff_subset` cover `__name__` deletion TypeError text and preservation. | Full parity still needs surrogate-code-point `UnicodeEncodeError` branches for construction and assignment; MiniPython currently rejects lone-surrogate string escapes during lexing because runtime strings use Rust scalar values. |
| `test_type_qualname` | `ported` | `cpython_type_name_qualname_subset` and `cpython_type_name_qualname_diff_subset` cover `__qualname__` supplied in the namespace, writable class `__qualname__`, and non-string rejection. `cpython_type_repr_module_qualname_subset` and `cpython_type_repr_module_qualname_diff_subset` cover nested and mutated `__qualname__` in type repr. `cpython_type_metadata_delete_errors_subset` and `cpython_type_metadata_delete_errors_diff_subset` cover `__qualname__` deletion TypeError text and preservation. | None for this method. |
