...
covered by `cpython_memoryview_basic_methods_and_release_subset`, including
CPython-style `tobytes(order=...)` TypeErrors, `__exit__()` keyword rejection,
and `__setitem__()` arity diagnostics.
...
`cpython_memoryview_cast_one_byte_format_subset`, including CPython-style
invalid `format` type diagnostics for `int` and bytes-subclass inputs.
...
item rejection.
Direct diff mapping note:
`cpython_bytes_memoryview_contiguity_methods_diff_subset` directly compares
...
contiguous sliced `memoryview` acceptance and non-contiguous `memoryview`
TypeErrors.
...
Direct diff mapping note: `cpython_bytes_method_typeerror_messages_diff_subset`
...
| `Lib/test/test_float.py` | `GeneralFloatCases` | 22 | `ported_public` | The method audit below classifies all 22 current methods. Current evidence covers the non-locale public surface for core `float()` construction, Unicode and bytes-like input parsing, underscore validation, numeric conversion protocols, `float.from_number()`, `is_integer()` / `as_integer_ratio()`, NaN containment identity behavior, floor/ceil/modulo, the active IEEE power matrix, hashing, subclass keyword construction, and float/int comparison boundaries. The remaining locale-controlled comma-decimal branch is classified as `blocked_by_runtime` because sandbox scope does not include host locale configuration. |
...
`cpython_bytearray_mutating_index_conversion_diff_subset` and capability-gated
`cpython_bytearray_mutating_index_safety_diff_subset`, ports the Python-level
public behavior of current CPython `ByteArrayTest::test_mutating_index` and
...
methods. The reentrant safety rows stay as local runtime evidence when default
system CPython versions disagree on those crash-regression branches.
`_testlimitedcapi.sequence_setitem` branches remain classified as C API coverage
outside MiniPython's runtime surface.
`cpython_bytearray_search_reentrancy_buffererror_subset`, backed by
`cpython_bytearray_search_reentrancy_buffererror_diff_subset`, ports the public
...
| `test_ascii` | `partial` | `cpython_ascii_builtin_subset`, backed by `cpython_ascii_builtin_diff_subset`, covers empty/basic values, recursive list/dict rendering, scalar Unicode escaping, f-string `!a`, non-ASCII printable escaping, and top-level `ascii()` custom `__repr__` dispatch: it ASCII-escapes the returned repr text, preserves str-subclass `__repr__` results when no escaping is needed, returns exact `str` after ASCII escaping, and preserves non-string-returning and raising `__repr__` paths, without expanding container-recursive custom `__repr__` dispatch. | Lone surrogate storage/rendering remains future Unicode-runtime work because MiniPython strings currently use Rust scalar values. |
...
| `test_callable` | `ported` | `cpython_attribute_introspection_builtins_subset` and the focused `cpython_builtin_callable_public_subset` / `cpython_builtin_callable_public_diff_subset` cover ordinary callables, classes, bound methods, class-level `__call__`, inherited `__call__`, ignored instance-level `__call__`, and arity errors. | None for this method. |
...
| `test___ne__` | `ported` | `cpython_builtin_none_ne_direct_subset` and `cpython_builtin_none_ne_direct_diff_subset` cover direct `None.__ne__` behavior, inherited `object.__eq__` / `object.__ne__` direct-call identity comparison, `NoneType` `NotImplemented` fallback for unrelated objects, CPython's direct `object.__ne__` bool result for builtin containers, and arity TypeError paths. | None for this method. |
...
| `test_repr` | `ported` | `cpython_repr_builtin_subset`, backed by the `repr-builtin` CPython diff case, covers primitive/container repr, recursive list/dict placeholders, top-level `repr()` custom `__repr__` dispatch, preserves str-subclass `__repr__` results, and blocked or non-string-returning `__repr__` paths. `cpython_object_repr_str_direct_subset` and `cpython_object_repr_str_direct_diff_subset` cover direct `object.__repr__` / `object.__str__` descriptor lookup and calls, inherited bindings, generic object display, `object.__str__` delegation to `__repr__`, raw direct-call non-string return behavior, container subclass display, and arity/keyword errors. `cpython_object_instance_default_repr_shape_subset` and `cpython_object_instance_default_repr_shape_diff_subset` cover default user-instance repr/str shape with module and qualname without depending on concrete addresses. `cpython_function_repr_str_wrapper_subset` and `cpython_function_repr_str_wrapper_diff_subset` cover function repr/str wrapper shape, nested function qualname display, `__name__` versus `__qualname__` effects, method-wrapper visibility, and wrapper arity/keyword errors without depending on concrete addresses. `cpython_function_call_wrapper_subset` and `cpython_function_call_wrapper_diff_subset` cover function __call__ method-wrapper metadata, receiver identity, positive positional/keyword forwarding, missing `__module__`, method-wrapper repr/str shape containing `function object at 0x`, and repr-wrapper arity/keyword errors without depending on concrete addresses. `cpython_function_get_descriptor_wrapper_subset` and `cpython_function_get_descriptor_wrapper_diff_subset` cover function __get__ descriptor wrapper metadata, `None`/owner handling including `__get__(None, None) is invalid`, bound-method creation and receiver identity, missing `__module__`, method-wrapper repr/str shape, and wrapper arity/keyword errors without depending on concrete addresses. `cpython_function_format_wrapper_subset` and `cpython_function_format_wrapper_diff_subset` cover function __format__ wrapper metadata, `builtin_function_or_method` classification, missing `__module__`, text signature and doc metadata, empty-format delegation to the function repr/str path, non-empty format rejection, and arity/keyword errors without depending on concrete addresses. `cpython_function_hash_wrapper_subset` and `cpython_function_hash_wrapper_diff_subset` cover function __hash__ wrapper metadata, `method-wrapper` classification, missing `__module__`, text signature and doc metadata, identity-hash delegation through `hash(f)`, method-wrapper repr/str shape, and arity/keyword errors without depending on concrete hash or address values. `cpython_function_rich_compare_wrapper_subset` and `cpython_function_rich_compare_wrapper_diff_subset` cover function __eq__ / __ne__ wrapper metadata, `method-wrapper` classification, missing `__module__`, text signature and doc metadata, identity comparison for the same function, NotImplemented fallback for different functions and non-functions, method-wrapper repr/str shape, and arity/keyword errors without depending on concrete address values. `cpython_function_order_compare_wrapper_subset` and `cpython_function_order_compare_wrapper_diff_subset` cover function ordering wrapper metadata for `__lt__`, `__le__`, `__gt__`, and `__ge__`, `method-wrapper` classification, missing `__module__`, text signature and doc metadata, NotImplemented result for functions and non-functions, method-wrapper repr/str shape, and arity/keyword errors without depending on concrete address values. `cpython_function_getattribute_wrapper_subset` and `cpython_function_getattribute_wrapper_diff_subset` cover function __getattribute__ wrapper metadata, attribute forwarding for function metadata and wrapper attributes, `method-wrapper` classification, missing `__module__`, text signature and doc metadata, missing-attribute normalization to CPython public function-object error text, method-wrapper repr/str shape, and arity/keyword/name-type errors without depending on concrete address values. `cpython_function_setattr_delattr_wrapper_subset` and `cpython_function_setattr_delattr_wrapper_diff_subset` cover function __setattr__ / __delattr__ wrapper metadata, attribute mutation, custom attribute shadowing of function wrapper attributes, `method-wrapper` classification, missing `__module__`, text signature and doc metadata, missing-attribute normalization to CPython public function-object error text, method-wrapper repr/str shape, and arity/keyword/name-type errors without depending on concrete address values. `cpython_function_init_wrapper_subset` and `cpython_function_init_wrapper_diff_subset` cover function __init__ wrapper metadata, no-op initialization calls with extra positional and keyword arguments, custom attribute shadowing, `method-wrapper` classification, missing `__module__`, text signature and doc metadata, and repr/str shape without depending on concrete address values. `cpython_function_init_subclass_wrapper_subset` and `cpython_function_init_subclass_wrapper_diff_subset` cover function __init_subclass__ wrapper metadata, type-level builtin method binding, no-op calls, extra positional and keyword TypeError paths, custom attribute shadowing, `builtin_function_or_method` classification, text signature and doc metadata, and repr/str shape without depending on concrete address values. `cpython_function_subclasshook_wrapper_subset` and `cpython_function_subclasshook_wrapper_diff_subset` cover function __subclasshook__ wrapper metadata, type-level builtin method binding, NotImplemented fallback, missing-argument and keyword TypeError paths, custom attribute shadowing, `builtin_function_or_method` classification, text signature and doc metadata, and repr/str shape without depending on concrete address values. `cpython_function_getstate_wrapper_subset` and `cpython_function_getstate_wrapper_diff_subset` cover function __getstate__ wrapper metadata, inherited pure-memory no-state results, object.__getstate__ direct call coverage, extra positional and keyword TypeError paths, custom attribute shadowing, `builtin_function_or_method` classification, text signature and doc metadata, and repr/str shape without depending on concrete address values. `cpython_function_dir_wrapper_subset` and `cpython_function_dir_wrapper_diff_subset` cover function __dir__ wrapper metadata, returned name-list membership, object.__dir__ direct list compatibility, extra positional and keyword TypeError paths, custom attribute shadowing, `builtin_function_or_method` classification, text signature and doc metadata, and wrapper repr/str shape without depending on complete CPython name-list order. `cpython_function_dict_assignment_subset` and `cpython_function_dict_assignment_diff_subset` cover function __dict__ assignment, dict replacement identity, live external dict mutation, dict subclass replacement, custom attribute visibility through `dir()`, non-dict assignment TypeError paths, and cannot delete __dict__ errors without expanding pickle or host IO. `cpython_str_builtin_custom_dunder_subset` and `cpython_str_builtin_custom_dunder_diff_subset` cover ordinary `str()` / `print()` / f-string `!s` / default f-string / string `%s` dispatch through class-level `__str__`, non-string result rejection, propagated exceptions, instance-level `__str__` bypass, and `str` subclass empty-format behavior. | None for this method. |
...
| `test_format` | `ported` | `cpython_format_builtin_and_custom_dunder_format_subset` covers basic `format()`, empty format specs for the builtin values used by CPython's method, inherited custom class `__format__`, class-level lookup over instance attributes, derived-`str` format-spec objects passed to builtin and user `__format__` paths, str-subclass `__format__` results for single-field `format()`, single-field f-string, and single-field `str.format` outputs, concatenated formatted strings normalizing to exact `str`, empty object format, wrong-result TypeError, non-string spec TypeError, `object.__format__` argument rejection, and non-empty object format rejection. `cpython_str_builtin_custom_dunder_subset` and `cpython_str_builtin_custom_dunder_diff_subset` add empty `object.__format__(..., "")` dispatch through class-level `__str__` plus custom `__format__` precedence over `__str__`. | None for this method. |
...
Format conversion repr protocol note: `cpython_format_conversion_repr_custom_dunder_subset`, backed by `cpython_format_conversion_repr_custom_dunder_diff_subset`, covers f-string and `str.format` `!r` / `!a` conversion through class-level `__repr__`, normalizes str-subclass `__repr__` results to formatted `str` output, ASCII-escapes `!a`, and preserves non-string-returning and raising `__repr__` paths, with top-level `repr()` / `ascii()` and old-style `%r` / `%a` result identity covered by separate focused evidence.

## `Lib/test/test_builtin.py::BuiltinTest Attribute/Introspection Method Audit`

| CPython method | Status | Rust evidence | Remaining acceptance |
| --- | --- | --- | --- |
| `test_delattr` | `ported` | `cpython_attribute_introspection_builtins_subset` and the focused `cpython_builtin_setattr_delattr_public_subset` / `cpython_builtin_setattr_delattr_public_diff_subset` cover module attribute deletion, instance deletion, class attribute deletion, builtin arity errors, and non-string attribute-name rejection. | None for this method. |
...
| `test_getattr` | `partial` | `cpython_attribute_introspection_builtins_subset` and the focused `cpython_builtin_getattr_public_subset` / `cpython_builtin_getattr_public_diff_subset` cover module attributes, `sys.stdout` lookup, instance/class lookup, missing attributes, default values, builtin arity errors, non-string attribute-name rejection, and the maximum valid Unicode scalar attribute name. | Lone-surrogate attribute names remain future Unicode-runtime work because MiniPython strings currently use Rust scalar values. |
| `test_hasattr` | `ported` | `cpython_attribute_introspection_builtins_subset` and the focused `cpython_builtin_hasattr_public_subset` / `cpython_builtin_hasattr_public_diff_subset` cover present/missing module and instance attributes, class attributes seen through instances, maximum-valid-scalar attribute names, non-string attribute-name rejection, `AttributeError` suppression, and propagation of non-`AttributeError` exceptions from `__getattr__`. | None for this method. |
...
| `test_setattr` | `ported` | `cpython_attribute_introspection_builtins_subset` and the focused `cpython_builtin_setattr_delattr_public_subset` / `cpython_builtin_setattr_delattr_public_diff_subset` cover module and instance attribute assignment, class attribute assignment through instance lookup, builtin arity errors, non-string attribute-name rejection, and attribute-write failure on immutable scalar values. | None for this method. |
...
| CPython method | Status | Rust evidence | Remaining acceptance |
| --- | --- | --- | --- |
| `test_new_type` | `ported` | `cpython_type_dynamic_class_subset` covers ordinary dynamic class construction, metadata, base selection for MiniPython classes, instance `type()` / `__class__`, inherited methods, class-dict method installation, and `type('C', (B, int), ...)` public int-subclass layout including inherited int behavior, equality to `42`, and `int.to_bytes()` on the dynamic subclass instance. `cpython_type_base_readonly_subset` and `cpython_type_base_readonly_diff_subset` cover `__base__` assignment/delete readonly errors, `cpython_type_mro_readonly_subset` and `cpython_type_mro_readonly_diff_subset` cover `__mro__` assignment/delete readonly errors, `cpython_type_dict_readonly_subset` / `cpython_type_dict_readonly_diff_subset` cover `__dict__` assignment/delete readonly errors without int/None shadow values, `cpython_type_bases_delete_error_subset` / `cpython_type_bases_delete_error_diff_subset` cover `__bases__` deletion TypeError text, and `cpython_type_bases_assignment_error_subset` / `cpython_type_bases_assignment_error_diff_subset` cover non-tuple `__bases__` assignment TypeError text. | None for this method. |
...
| `test_type_name` | `partial` | `cpython_type_name_qualname_subset` and `cpython_type_name_qualname_diff_subset` cover valid dynamic-class names, writable `__name__`, bytes-name rejection, and NUL-containing name rejection while preserving existing metadata. `cpython_type_metadata_assignment_error_names_subset` and `cpython_type_metadata_assignment_error_names_diff_subset` cover `__name__` non-string assignment TypeError text. `cpython_type_repr_module_qualname_subset` and `cpython_type_repr_module_qualname_diff_subset` cover type `repr()` / `str()` module-qualified class names and `__mro__` tuple repr. `cpython_type_metadata_delete_errors_subset` and `cpython_type_metadata_delete_errors_diff_subset` cover `__name__` deletion TypeError text and preservation. | Full parity still needs surrogate-code-point `UnicodeEncodeError` branches for construction and assignment; MiniPython currently rejects lone-surrogate string escapes during lexing because runtime strings use Rust scalar values. |
| `test_type_qualname` | `ported` | `cpython_type_name_qualname_subset` and `cpython_type_name_qualname_diff_subset` cover `__qualname__` supplied in the namespace, writable class `__qualname__`, and non-string rejection. `cpython_type_metadata_assignment_error_names_subset` and `cpython_type_metadata_assignment_error_names_diff_subset` cover `__qualname__` non-string assignment TypeError text. `cpython_type_repr_module_qualname_subset` and `cpython_type_repr_module_qualname_diff_subset` cover nested and mutated `__qualname__` in type repr. `cpython_type_metadata_delete_errors_subset` and `cpython_type_metadata_delete_errors_diff_subset` cover `__qualname__` deletion TypeError text and preservation. | None for this method. |
...
| `test_type_typeparams` | `ported` | `cpython_type_typeparams_subset` covers generic class `__type_params__`, `typing.TypeVar` identity, user assignment override, delete rejection, and preserving the override. `cpython_type_typeparams_delete_error_name_subset` and `cpython_type_typeparams_delete_error_name_diff_subset` cover type `__type_params__` deletion TypeError behavior that uses the current public class name and preserves the assigned override. `cpython_type_parameters_missing_for_nongeneric_subset` and `cpython_type_parameters_missing_for_nongeneric_diff_subset` cover ordinary class `__parameters__` absence while preserving generic class `__parameters__`. | None for this method. |
| `test_type_doc` | `partial` | `cpython_type_doc_and_firstlineno_subset` and `cpython_type_doc_and_firstlineno_diff_subset` cover dynamic-class `__doc__` construction and assignment for strings, non-ASCII strings, NUL-containing strings, bytes, integers, and `None`. `cpython_type_metadata_delete_errors_subset` and `cpython_type_metadata_delete_errors_diff_subset` cover `__doc__` and `__module__` deletion TypeError text and preservation. | Full parity still needs the surrogate-code-point `UnicodeEncodeError` branch during dynamic-class construction; MiniPython currently rejects lone-surrogate string escapes during lexing because runtime strings use Rust scalar values. |
...
| `test_dunder_get_signature` | `ported` | `cpython_types_dunder_get_signature_subset` covers `object.__init__.__get__`, `str.join.__get__`, and `int.__dict__['from_bytes'].__get__` as `types.MethodWrapperType` instances with `inspect.signature(...)` returning `(instance, owner, /)`. | None for this method's public descriptor-signature behavior. |
...
| `test_float_with_comma` | `blocked_by_runtime` | `cpython_float_constructor_core_subset` covers comma-containing strings staying invalid and ordinary dot-decimal syntax staying accepted. | The remaining CPython branch requires locale-controlled `LC_NUMERIC` runs for comma decimal-point locales; locale-sensitive behavior is outside the default sandbox scope. |
