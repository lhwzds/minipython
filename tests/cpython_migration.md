...
| `operator` | Arithmetic/comparison helpers, sequence/member helpers, attrgetter/itemgetter/methodcaller, selected signature/module/qualname/doc metadata, and helper repr subsets. Subset evidence: `cpython_operator_public_helpers_subset`, `cpython_operator_length_hint_subset`, `cpython_operator_comparison_predicate_subset`, `cpython_operator_arithmetic_bitwise_subset`, `cpython_operator_sequence_member_subset`, `cpython_operator_callable_helper_subset`, `cpython_operator_inplace_helper_subset`, `cpython_operator_module_metadata_subset`, `cpython_operator_signature_helper_subset`, and `cpython_operator_helper_repr_subset`. | `cpython_operator_public_helpers_diff_subset`, `cpython_operator_length_hint_diff_subset`, `cpython_operator_comparison_predicate_diff_subset`, `cpython_operator_arithmetic_bitwise_diff_subset`, `cpython_operator_sequence_member_diff_subset`, `cpython_operator_callable_helper_diff_subset`, `cpython_operator_inplace_helper_diff_subset`, `cpython_operator_module_metadata_diff_subset`, `cpython_operator_signature_helper_diff_subset`, `cpython_operator_helper_repr_diff_subset`. | Full pickle metadata and every CPython helper edge case until separately migrated. |
...
- Added `cpython_operator_comparison_predicate_subset`, migrating public
  ...
  `cpython_operator_comparison_predicate_diff_subset`; newer `is_none` /
  `is_not_none` helpers stay covered by subset evidence under the current
  default CPython 3.9 oracle.
...
- Added `cpython_operator_callable_helper_subset`, migrating public behavior
  ...
  slice is in `cpython_operator_callable_helper_diff_subset`; newer
  `operator.call` stays covered by subset evidence under the current default
  CPython 3.9 oracle.
...
- Added `cpython_operator_module_metadata_subset`, migrating public behavior
  ...
  metadata slice is in `cpython_operator_module_metadata_diff_subset`; newer
  `operator.__all__` entries such as `call`, `is_none`, and `is_not_none` stay
  covered by subset evidence under the current default CPython 3.9 oracle.
...
- Added direct public-surface evidence for the minimal `deque` sandbox slice in
  ...
  access, iteration, len/bool/repr, and basic `append` / `appendleft` / `pop` /
  `popleft` / `clear` / `copy` operations. Broader deque APIs, pickling,
  performance, lifetime, and thread-safety semantics remain outside the
  sandbox default scope.
