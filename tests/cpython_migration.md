...
- Added `cpython_list_index_missing_valueerror_message_subset` and
...
- Added `cpython_immutable_sequence_count_rich_compare_subset` and
...
- Added `cpython_int_instance_doc_attribute_subset` and
  `cpython_int_instance_doc_attribute_diff_subset` for int instance `__doc__`
  lookup on small, negative, and large integers, matching `int.__doc__` and
...
- Added `cpython_bool_public_attributes_subset` and
  ...
  public integer attributes, read-only inherited int method attributes such as
  `bit_length`, `conjugate`, and `as_integer_ratio`, and CPython's
  bool-specific no-`__dict__` assignment/deletion errors without adding bool instance dictionaries.
- Added `cpython_bool_instance_doc_attribute_subset` and
  `cpython_bool_instance_doc_attribute_diff_subset` for bool instance `__doc__`
  lookup on `False` and `True`, matching `bool.__doc__` rather than
...
- Added `cpython_str_instance_doc_attribute_subset` and
  `cpython_str_instance_doc_attribute_diff_subset` for str instance `__doc__`
  lookup on ordinary strings and identity strings such as `super.__module__`,
  matching `str.__doc__` and `dir(value)` without adding writable instance dictionaries.
...
- Added `cpython_bytearray_attribute_assignment_errors_subset` and
...
- Added `cpython_list_instance_doc_attribute_subset` and
  `cpython_list_instance_doc_attribute_diff_subset` for list instance `__doc__`
  lookup on empty and populated lists, matching `list.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- Added `cpython_tuple_instance_doc_attribute_subset` and
  `cpython_tuple_instance_doc_attribute_diff_subset` for tuple instance `__doc__`
  lookup on empty and populated tuples, matching `tuple.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- Added `cpython_bytes_instance_doc_attribute_subset` and
  `cpython_bytes_instance_doc_attribute_diff_subset` for bytes instance `__doc__`
  lookup on empty and populated bytes, matching `bytes.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- Added `cpython_bytearray_instance_doc_attribute_subset` and
  `cpython_bytearray_instance_doc_attribute_diff_subset` for bytearray instance `__doc__`
  lookup on empty and populated bytearrays, matching `bytearray.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- Added `cpython_range_instance_doc_attribute_subset` and
  `cpython_range_instance_doc_attribute_diff_subset` for range instance `__doc__`
  lookup on empty and stepped ranges, matching `range.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- Added `cpython_slice_instance_doc_attribute_subset` and
  `cpython_slice_instance_doc_attribute_diff_subset` for slice instance `__doc__`
  lookup on empty and stepped slices, matching `slice.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
- Added `cpython_tuple_attribute_assignment_errors_subset` and
...
- Added `cpython_dict_instance_doc_attribute_subset` and
  `cpython_dict_instance_doc_attribute_diff_subset` for dict instance `__doc__`
  lookup on empty and populated dicts, matching `dict.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
...
- Added `cpython_set_instance_doc_attribute_subset` and
  `cpython_set_instance_doc_attribute_diff_subset` for set instance `__doc__`
  lookup on empty and populated sets, matching `set.__doc__` and `dir(value)`
  without adding writable instance dictionaries.
...
- Added `cpython_frozenset_instance_doc_attribute_subset` and
  `cpython_frozenset_instance_doc_attribute_diff_subset` for frozenset instance `__doc__`
  lookup on empty and populated frozensets, matching `frozenset.__doc__` and
  `dir(value)` without adding writable instance dictionaries.
- Added `cpython_frozenset_attribute_assignment_errors_subset` and
  `cpython_frozenset_attribute_assignment_errors_diff_subset` for frozenset attribute assignment errors
  on missing attributes and read-only frozenset method attributes (`union` /
  `copy`) without adding frozenset instance dictionaries.
- Added `cpython_memoryview_instance_doc_attribute_subset` and
...
- Added `cpython_memoryview_getstate_subset` and
  `cpython_memoryview_getstate_diff_subset` for inherited
  `object.__getstate__` on memoryview instances and slices:
...
- Added `cpython_types_mappingproxy_attribute_assignment_errors_subset` and
  `cpython_types_mappingproxy_attribute_assignment_errors_diff_subset` for mappingproxy attribute assignment errors
  on missing attributes and read-only mappingproxy method attributes (`keys`)
  without adding mappingproxy instance dictionaries.
- Added `cpython_types_mappingproxy_instance_doc_attribute_subset` and
  `cpython_types_mappingproxy_instance_doc_attribute_diff_subset` for mappingproxy instance `__doc__`
  lookup on exact proxies, builtin type dictionaries, and dict-view `.mapping`,
  matching `type(value).__doc__` and `dir(value)` without adding writable instance dictionaries.
- Added `cpython_collections_deque_attribute_assignment_errors_subset` and
  `cpython_collections_deque_attribute_assignment_errors_diff_subset` for deque attribute assignment errors
  on missing attributes, read-only deque data attributes (`maxlen`), and
...
- Added `cpython_range_public_attributes_subset` and
  ...
  default, explicit, negative-step, and large-integer ranges, including range data attributes in dir()
  for both instances and the builtin type, CPython's read-only range method attributes
  (`count` and `index`), readonly, and no-`__dict__` assignment/deletion errors without adding range instance dictionaries.
- Added `cpython_slice_public_attributes_subset` and
...
- Added `cpython_complex_instance_doc_attribute_subset` and
  `cpython_complex_instance_doc_attribute_diff_subset` for complex instance `__doc__`
  lookup on zero, finite, signed-component, and infinite-component complex
...
- Added `cpython_float_instance_doc_attribute_subset` and
  `cpython_float_instance_doc_attribute_diff_subset` for float instance `__doc__`
  lookup on positive zero, negative zero, finite, and infinite floats, matching
...
- Added `cpython_json_function_get_missing_owner_error_subset` and
  `cpython_json_function_get_missing_owner_error_diff_subset` for json public
  function `__get__` missing-owner `TypeError` parity, keeping
  `loads.__get__(None)` / `dumps.__get__(None)` aligned with CPython without
  adding function `__code__` or full stdlib json expansion.
- Added `cpython_json_function_module_identity_metadata_subset` and
  `cpython_json_function_module_identity_metadata_diff_subset` for json public
  function `__module__` identity stability across repeated lookup, loads/dumps
  sharing, and bound-method delegation, without adding full module globals or
  file APIs.
- Added `cpython_json_function_name_qualname_identity_metadata_subset` and
  `cpython_json_function_name_qualname_identity_metadata_diff_subset` for json
  public function `__name__` / `__qualname__` identity stability across
  repeated lookup, per-function sharing, and bound-method delegation, without
  adding function `__code__` or file APIs.
- Added `cpython_json_function_name_assignment_metadata_subset` and
  `cpython_json_function_name_assignment_metadata_diff_subset` for json public
  function `__name__` assignment and deletion errors, including string and
  str-subclass replacements, empty-string assignment, and unchanged
  `__qualname__`, without adding `__qualname__` assignment, pickle helpers, or
  file APIs.
- Added `cpython_json_function_qualname_assignment_metadata_subset` and
  `cpython_json_function_qualname_assignment_metadata_diff_subset` for json
  public function `__qualname__` assignment and deletion errors, including
  string and str-subclass replacements, empty-string assignment, and unchanged
  `__name__`, without adding function `__code__`, pickle helpers, or file
  APIs.
- Added `cpython_json_function_type_class_metadata_subset` and
  `cpython_json_function_type_class_metadata_diff_subset` for json public
  function type / `__class__` metadata alignment with CPython's `function`
  type, without changing ordinary builtins or adding file APIs.
- Added `cpython_json_function_class_assignment_metadata_subset` and
  `cpython_json_function_class_assignment_metadata_diff_subset` for json
  public function `__class__` assignment and deletion errors, including
  ...
  adding mutable `__class__` reassignment, function `__code__`, pickle helpers,
  or file APIs.
- Added `cpython_json_function_repr_str_wrapper_metadata_subset` and
  `cpython_json_function_repr_str_wrapper_metadata_diff_subset` for json
  public function repr / str wrapper metadata, including wrapper visibility,
  method-wrapper type shape, and argument error text, without adding function
  `__code__` or file APIs.
- Added `cpython_json_function_call_wrapper_metadata_subset` and
  `cpython_json_function_call_wrapper_metadata_diff_subset` for json public
  function `__call__` wrapper metadata, including forwarding and
  method-wrapper metadata, without adding function `__code__` or file APIs.
- Added `cpython_json_function_init_wrapper_metadata_subset` and
  `cpython_json_function_init_wrapper_metadata_diff_subset` for json public
  function `__init__` wrapper metadata, including bound calls, direct
  `object.__init__(function, ...)` calls, and permissive extra-argument /
  keyword behavior for function objects, without adding function `__code__`,
  `__sizeof__`, pickle helpers, or file APIs.
- Added `cpython_json_function_init_subclass_wrapper_metadata_subset` and
  `cpython_json_function_init_subclass_wrapper_metadata_diff_subset` for json
  public function `__init_subclass__` wrapper metadata, including binding to
  the shared `function` type, zero-argument success, and arity / keyword
  `TypeError` text, without adding function `__new__`, function `__code__`,
  `__sizeof__`, `__getstate__`, pickle helpers, or file APIs.
- Added `cpython_json_function_subclasshook_wrapper_metadata_subset` and
  `cpython_json_function_subclasshook_wrapper_metadata_diff_subset` for json
  public function `__subclasshook__` wrapper metadata, including binding to the
  shared `function` type, one-argument `NotImplemented`, and missing-argument /
  keyword `TypeError` text, without adding function `__new__`, function
  `__code__`, `__sizeof__`, `__getstate__`, pickle helpers, or file APIs.
- Added `cpython_json_function_format_wrapper_metadata_subset` and
  `cpython_json_function_format_wrapper_metadata_diff_subset` for json public
  function `__format__` wrapper metadata, including empty-format rendering,
  non-empty-format `TypeError`, and arity / keyword errors, without adding
  function `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_hash_wrapper_metadata_subset` and
  `cpython_json_function_hash_wrapper_metadata_diff_subset` for json public
  function `__hash__` wrapper metadata, including hash result consistency with
  `hash(function)`, missing `__module__` behavior, and arity / keyword errors,
  without adding function `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_rich_compare_wrapper_metadata_subset` and
  `cpython_json_function_rich_compare_wrapper_metadata_diff_subset` for json
  public function `__eq__` / `__ne__` wrapper metadata, including same-function
  bool results, different-function and non-function `NotImplemented` results,
  and arity / keyword errors, without adding function `__code__`, ordering
  wrappers, pickle helpers, or file APIs.
- Added `cpython_json_function_order_wrapper_metadata_subset` and
  `cpython_json_function_order_wrapper_metadata_diff_subset` for json public
  function ordering wrapper metadata, including `__lt__` / `__le__` /
  `__gt__` / `__ge__` `NotImplemented` results and arity / keyword errors,
  without adding function `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_getattribute_wrapper_metadata_subset` and
  `cpython_json_function_getattribute_wrapper_metadata_diff_subset` for json
  public function `__getattribute__` wrapper metadata, including supported
  metadata lookup and error text, without adding function `__code__` or file
  APIs.
- Added `cpython_json_function_setattr_delattr_wrapper_metadata_subset` and
  `cpython_json_function_setattr_delattr_wrapper_metadata_diff_subset` for json
  public function `__setattr__` / `__delattr__` wrapper metadata and ordinary
  function-attribute mutation through `__dict__`, including wrapper-name
  shadowing and arity / keyword / attribute-name errors, without adding
  function `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_doc_module_assignment_metadata_subset` and
  `cpython_json_function_doc_module_assignment_metadata_diff_subset` for json public function `__doc__` / `__module__` assignment and deletion, including
  bound-method delegation, no `__dict__` pollution for those controlled
  attributes, and repeated deletion to `None`, without adding full Python
  function object mutation semantics, pickle helpers, or file APIs.
- Added `cpython_json_function_dict_assignment_metadata_subset` and
  `cpython_json_function_dict_assignment_metadata_diff_subset` for json public function `__dict__` assignment and deletion errors, including exact dict and
  dict-subclass replacement-backed attribute lookup / `dir()` visibility,
  without adding full Python function object mutation semantics, pickle
  helpers, or file APIs.
- Added `cpython_json_function_defaults_assignment_metadata_subset` and
  `cpython_json_function_defaults_assignment_metadata_diff_subset` for json
  public function `__defaults__` assignment and deletion, including tuple and
  tuple-subclass replacement identity, empty tuple assignment, `None` reset,
  deletion reset to `None`, and non-tuple assignment errors, without adding
  `__defaults__` call-binding mutation semantics, function `__code__`, pickle
  helpers, or file APIs.
- Added `cpython_json_function_kwdefaults_assignment_metadata_subset` and
  `cpython_json_function_kwdefaults_assignment_metadata_diff_subset` for json
  public function `__kwdefaults__` assignment and deletion, including exact
  dict, dict-subclass, and OrderedDict replacement identity, empty dict
  assignment, `None` reset, deletion reset to `None`, and non-dict assignment
  errors, without adding `__kwdefaults__` call-binding mutation semantics,
  function `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_globals_assignment_metadata_subset` and
  `cpython_json_function_globals_assignment_metadata_diff_subset` for json
  public function `__globals__` assignment and deletion errors, including
  direct assignment, direct deletion, and `__setattr__` / `__delattr__`
  wrapper entry points, without adding writable function globals, function
  `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_builtins_assignment_metadata_subset` and
  `cpython_json_function_builtins_assignment_metadata_diff_subset` for json
  public function `__builtins__` assignment and deletion errors, including
  direct assignment, direct deletion, and `__setattr__` / `__delattr__`
  wrapper entry points, without adding writable function builtins, function
  `__code__`, pickle helpers, or file APIs.
- Added `cpython_json_function_closure_assignment_metadata_subset` and
  `cpython_json_function_closure_assignment_metadata_diff_subset` for json
  public function `__closure__` assignment and deletion errors, including
  direct assignment, direct deletion, and `__setattr__` / `__delattr__`
  wrapper entry points, without adding writable closures, function `__code__`,
  pickle helpers, or file APIs.
- Added `cpython_json_function_type_params_assignment_metadata_subset` and
  `cpython_json_function_type_params_assignment_metadata_diff_subset` for json public function `__type_params__` assignment and deletion errors, including shared
  initial empty-tuple identity, per-function tuple / tuple-subclass
  replacement identity, and non-tuple assignment errors, without adding generic
  type-parameter evaluation, annotationlib, pickle helpers, or file APIs.
- Added `cpython_json_function_annotate_assignment_metadata_subset` and
  `cpython_json_function_annotate_assignment_metadata_diff_subset` for json public function `__annotate__` assignment and deletion errors, including callable
  replacement identity, `None` reset, and non-callable assignment errors,
  without adding lazy annotation evaluation, pickle helpers, or file APIs.
- Added `cpython_json_function_doc_identity_metadata_subset` and
  `cpython_json_function_doc_identity_metadata_diff_subset` for json public
  function `__doc__` identity stability across repeated lookup and bound-method
  delegation, without adding full CPython docstring text or file APIs.
- Added `cpython_json_function_bound_method_name_dir_visibility_subset` and
  `cpython_json_function_bound_method_name_dir_visibility_diff_subset` for json
  public function bound method name metadata, keeping `bound.__name__` /
  `bound.__qualname__` / `bound.__module__` lookup available while hiding those
  names from `dir(bound)`, without adding `__reduce__`, `__reduce_ex__`, or
  `__sizeof__`.
- Added `cpython_json_function_bound_method_class_dir_visibility_subset` and
  `cpython_json_function_bound_method_class_dir_visibility_diff_subset` for json
  public function bound method `__class__` metadata, keeping `__class__` visible
  in `dir(bound)` without adding other object slot wrappers such as
  `__delattr__`, `__setattr__`, `__sizeof__`, `__reduce__`, or `__reduce_ex__`.
- Added `cpython_json_function_bound_method_repr_subset` and
  `cpython_json_function_bound_method_repr_diff_subset` for json public
  function bound method `repr()` / `str()` rendering, keeping
  `loads.__get__(...)` / `dumps.__get__(...)` display names aligned with
  CPython without adding method `__repr__` attributes or full stdlib json
  expansion.
- Added `cpython_json_function_bound_method_repr_str_wrapper_subset` and
  `cpython_json_function_bound_method_repr_str_wrapper_diff_subset` for json
  public function bound method `__repr__` / `__str__` wrapper access, keeping
  wrapper type names, calls, and arity / keyword errors aligned with CPython
  without adding `__reduce__`, `__reduce_ex__`, or method `__getattribute__`.
- Added `cpython_json_function_bound_method_call_wrapper_subset` and
  `cpython_json_function_bound_method_call_wrapper_diff_subset` for json public
  function bound method `__call__` wrapper access, keeping receiver-bound
  forwarding, method-wrapper metadata, and duplicate-argument errors aligned
  with CPython without adding `__reduce__` or `__reduce_ex__`.
- Added `cpython_json_function_bound_method_get_wrapper_subset` and
  `cpython_json_function_bound_method_get_wrapper_diff_subset` for json public
  function bound method `__get__` wrapper access, keeping descriptor rebinding
  identity, method-wrapper metadata, and descriptor argument errors aligned with
  CPython without adding `__reduce__` or `__reduce_ex__`.
- Added `cpython_json_function_bound_method_dir_wrapper_subset` and
  `cpython_json_function_bound_method_dir_wrapper_diff_subset` for json public
  function bound method `__dir__` wrapper access, keeping name-list membership,
  builtin-function metadata, and arity / keyword errors aligned with CPython
  without adding `__reduce__` or `__reduce_ex__`.
- Added `cpython_json_function_bound_method_rich_compare_wrapper_subset` and
  `cpython_json_function_bound_method_rich_compare_wrapper_diff_subset` for json
  public function bound method `__eq__` / `__ne__` wrapper access, keeping
  same-receiver identity comparison, equal-but-not-identical receiver rejection,
  non-method `NotImplemented`, metadata, and arity / keyword errors aligned with
  CPython without adding `__reduce__`, `__reduce_ex__`, or `__sizeof__`.
- Added `cpython_json_function_bound_method_rich_compare_operator_subset` and
  `cpython_json_function_bound_method_rich_compare_operator_diff_subset` for json
  public function bound method `==` / `!=` operator behavior, keeping
  receiver-identity equality, equal-but-distinct receiver rejection, different
  function rejection, and non-method false/true results aligned with CPython
  without adding `__reduce__`, `__reduce_ex__`, or `__sizeof__`.
- Added `cpython_json_function_bound_method_order_wrapper_subset` and
  `cpython_json_function_bound_method_order_wrapper_diff_subset` for json public
  function bound method `__lt__` / `__le__` / `__gt__` / `__ge__` wrapper
  access, keeping method-wrapper metadata, `NotImplemented` results, and arity /
  keyword errors aligned with CPython without adding `__reduce__`,
  `__reduce_ex__`, or `__sizeof__`.
- Added `cpython_json_function_bound_method_format_wrapper_subset` and
  `cpython_json_function_bound_method_format_wrapper_diff_subset` for json
  public function bound method `__format__` wrapper access, keeping empty-format
  `str(bound)` delegation, non-empty format rejection, metadata, and arity /
  keyword errors aligned with CPython without adding `__reduce__`,
  `__reduce_ex__`, or `__sizeof__`.
- Added `cpython_json_function_bound_method_hash_wrapper_subset` and
  `cpython_json_function_bound_method_hash_wrapper_diff_subset` for json public
  function bound method `__hash__` wrapper access, keeping method-wrapper
  metadata, `hash(bound)` equality, and arity / keyword errors aligned with
  CPython without adding `__reduce__`, `__reduce_ex__`, or `__sizeof__`.
- Added `cpython_json_function_bound_method_getattribute_wrapper_subset` and
  `cpython_json_function_bound_method_getattribute_wrapper_diff_subset` for json
  public function bound method `__getattribute__` wrapper access, keeping
  selected method-object attribute lookup and arity / keyword / name-type
  errors aligned with CPython without adding `__reduce__` or `__reduce_ex__`.
- Added `cpython_json_function_bound_method_getattribute_missing_attr_subset`
  and `cpython_json_function_bound_method_getattribute_missing_attr_diff_subset`
  for json public function bound method `__getattribute__` missing-attribute
  `AttributeError` text and `args`, without adding `__reduce__` or
  `__reduce_ex__`.
- Added `cpython_json_function_bound_method_doc_dir_metadata_subset` and
  `cpython_json_function_bound_method_doc_dir_metadata_diff_subset` for json
  public function bound method `__doc__` direct / `__getattribute__` lookup and
  `dir(bound)` visibility, without adding stable docstring identity or file
  APIs.
- Added `cpython_json_function_bound_method_text_signature_missing_attr_subset`
  and `cpython_json_function_bound_method_text_signature_missing_attr_diff_subset`
  for json public function bound method `__text_signature__` missing-attribute
  `AttributeError` text and direct / `__getattribute__` lookup, without adding
  `__reduce__`, `__reduce_ex__`, function `__code__`, or file APIs.
- Added `cpython_json_function_bound_method_annotate_metadata_subset` and
  `cpython_json_function_bound_method_annotate_metadata_diff_subset` for json
  public function bound method `__annotate__` metadata and
  `__getattribute__('__annotate__')` lookup, without adding function `__code__`
  or file APIs.
- Added `cpython_json_function_bound_method_type_params_metadata_subset` and
  `cpython_json_function_bound_method_type_params_metadata_diff_subset` for json
  public function bound method `__type_params__` metadata and shared empty
  tuple identity, without adding function `__code__` or file APIs.
- Added `cpython_json_function_bound_method_defaults_metadata_subset` and
  `cpython_json_function_bound_method_defaults_metadata_diff_subset` for json
  public function bound method `__defaults__` metadata and
  `__getattribute__('__defaults__')` lookup.
- Added `cpython_json_function_bound_method_kwdefaults_metadata_subset` and
  `cpython_json_function_bound_method_kwdefaults_metadata_diff_subset` for json
  public function bound method `__kwdefaults__` metadata and
  `__getattribute__('__kwdefaults__')` lookup.
- Added `cpython_json_function_bound_method_annotations_metadata_subset` and
  `cpython_json_function_bound_method_annotations_metadata_diff_subset` for json
  public function bound method `__annotations__` metadata and
  `__getattribute__('__annotations__')` lookup.
- Added `cpython_json_function_bound_method_dict_metadata_subset` and
  `cpython_json_function_bound_method_dict_metadata_diff_subset` for json
  public function bound method `__dict__` metadata and
  `__getattribute__('__dict__')` lookup.
- Added `cpython_json_function_bound_method_globals_metadata_subset` and
  `cpython_json_function_bound_method_globals_metadata_diff_subset` for json
  public function bound method `__globals__` metadata and
  `__getattribute__('__globals__')` lookup, without adding file APIs.
- Added `cpython_json_function_bound_method_builtins_metadata_subset` and
  `cpython_json_function_bound_method_builtins_metadata_diff_subset` for json
  public function bound method `__builtins__` metadata and
  `__getattribute__('__builtins__')` lookup, without adding file APIs.
- Added `cpython_json_function_bound_method_closure_none_metadata_subset` and
  `cpython_json_function_bound_method_closure_none_metadata_diff_subset` for
  json public function bound method `__closure__` metadata and
  `__getattribute__('__closure__')` lookup, without adding function `__code__`
  or file APIs.
...
- Added `cpython_collections_deque_mutating_eq_subset` and
  `cpython_collections_deque_mutating_eq_diff_subset` for deque mutation during comparison
  across membership, `__contains__`, `count`, `index`, and `remove`,
  preserving CPython's mutated-iteration errors and the `remove`-specific
  `IndexError` classification without expanding into thread-safety or lifetime
  internals.
- Added `cpython_collections_userlist_mutating_eq_subset` and
  `cpython_collections_userlist_mutating_eq_diff_subset` for UserList mutation during comparison
  across membership, `__contains__`, `count`, `index`, and `remove`, keeping
  the fix within the existing pure-memory list-search
  semantics instead of expanding the `collections` stdlib surface.
...
- Extended `cpython_runtime_exception_capture_subset` and
  ...
  object.__getattribute__ Counter type-object __base__ metadata,
  object.__getattribute__ Counter type-object __bases__ metadata,
  object.__getattribute__ Counter type-object __mro__ metadata,
  object.__getattribute__ ChainMap type-object __module__ metadata,
  object.__getattribute__ ChainMap type-object __base__ metadata,
  object.__getattribute__ ChainMap type-object __bases__ metadata,
  object.__getattribute__ defaultdict type-object __base__ metadata,
  object.__getattribute__ defaultdict type-object __bases__ metadata,
  object.__getattribute__ defaultdict type-object __mro__ metadata,
  object.__getattribute__ UserDict type-object __module__ metadata,
  object.__getattribute__ UserDict type-object __base__ metadata,
  object.__getattribute__ UserDict type-object __bases__ metadata,
  object.__getattribute__ UserList type-object __module__ metadata,
  object.__getattribute__ UserList type-object __base__ metadata,
  object.__getattribute__ UserList type-object __bases__ metadata,
  ...
  object.__getattribute__ _count_elements builtin __module__ metadata,
  object.__getattribute__ _count_elements builtin __qualname__ metadata,
  object.__getattribute__ _count_elements builtin __doc__ metadata,
  object.__getattribute__ _count_elements builtin __text_signature__ metadata,
  object.__getattribute__ Hashable ABC type-object __doc__ metadata,
  object.__getattribute__ Hashable ABC type-object __text_signature__ metadata,
  object.__getattribute__ Iterable ABC type-object __doc__ metadata,
  object.__getattribute__ Iterable ABC type-object __text_signature__ metadata,
  object.__getattribute__ Iterator ABC type-object __doc__ metadata,
  object.__getattribute__ Iterator ABC type-object __text_signature__ metadata,
  object.__getattribute__ Sized ABC type-object __doc__ metadata,
  object.__getattribute__ Sized ABC type-object __text_signature__ metadata,
  object.__getattribute__ Container ABC type-object __doc__ metadata,
  object.__getattribute__ Container ABC type-object __text_signature__ metadata,
  object.__getattribute__ Callable ABC type-object __doc__ metadata,
  object.__getattribute__ Callable ABC type-object __text_signature__ metadata,
  object.__getattribute__ Collection ABC type-object __doc__ metadata,
  object.__getattribute__ Collection ABC type-object __text_signature__ metadata,
  object.__getattribute__ Reversible ABC type-object __doc__ metadata,
  object.__getattribute__ Reversible ABC type-object __text_signature__ metadata,
  object.__getattribute__ Sequence ABC type-object __doc__ metadata,
  ...
  object.__getattribute__ Awaitable ABC type-object __doc__ metadata,
  object.__getattribute__ Awaitable ABC type-object __text_signature__ metadata,
  object.__getattribute__ Coroutine ABC type-object __doc__ metadata,
  object.__getattribute__ Coroutine ABC type-object __text_signature__ metadata,
  object.__getattribute__ AsyncIterable ABC type-object __doc__ metadata,
  object.__getattribute__ AsyncIterable ABC type-object __text_signature__ metadata,
  object.__getattribute__ AsyncIterator ABC type-object __doc__ metadata,
  object.__getattribute__ AsyncIterator ABC type-object __text_signature__ metadata,
  ...
  object.__getattribute__ Generator ABC type-object __doc__ metadata,
  object.__getattribute__ Generator ABC type-object __text_signature__ metadata,
  object.__getattribute__ Mapping ABC type-object __doc__ metadata,
  object.__getattribute__ Mapping ABC type-object __text_signature__ metadata,
  object.__getattribute__ MutableMapping ABC type-object __doc__ metadata,
  object.__getattribute__ MutableMapping ABC type-object __text_signature__ metadata,
  object.__getattribute__ MappingView ABC type-object __doc__ metadata,
  ...
  object.__getattribute__ ValuesView ABC type-object __doc__ metadata,
  ...
  collections.abc module __all__ public export list,
  collections.abc module __package__ metadata,
  collections.abc module __doc__ metadata,
  ...
  object.__getattribute__ UserString type-object __qualname__ metadata,
  ...
  object.__getattribute__ UserString type-object __text_signature__ metadata,
  ...
  object.__getattribute__ OrderedDict type-object __doc__ metadata,
  ...
  `list.__iter__` and `list.append`, tuple methods including `tuple.__len__`
  and `tuple.count`, string and bytes methods including `str.upper` and
  `bytes.upper`, `set.add`, `frozenset.__hash__`,
  ...
  `math.gcd()`, `math.lcm()`, public generator/coroutine no-keyword paths
  including `generator.throw()`, `coroutine.throw()`, and
  `async_generator.athrow()`, `BaseException.with_traceback()` keyword
  rejection for base, inherited, concrete, and custom exception bound methods,
  and common stdlib-backed builtins such as `id()`,
...
- Added `cpython_descriptor_constructor_arity_errors_subset` and direct CPython
  evidence in `cpython_descriptor_constructor_arity_errors_diff_subset` for
  descriptor constructor arity diagnostics. `staticmethod`, `classmethod`, and
  `property` now match CPython's public positional-arity text, including
  `staticmethod expected 1 argument` and
...
`cpython_builtins_module_package_metadata_diff_subset`, keeps builtins module `__package__` metadata
aligned with CPython through `builtins.__package__`.
...
| --- | --- | --- | --- |
| `builtins` | Core constructors, exceptions, descriptors, import hook, `eval()` / `exec()` / `compile()`, iteration helpers including `aiter()` / `anext()`, aggregates, numeric helpers, sandbox-safe `print()` keyword handling, `breakpoint()` custom-hook path and sandbox default no-op hook. Subset evidence: `cpython_builtins_module_package_metadata_subset`, `cpython_eval_builtin_subset`, `cpython_exec_builtin_subset`, `cpython_exec_closure_subset`, `cpython_eval_exec_builtins_mapping_subset`, `cpython_compile_builtin_code_object_subset`, `cpython_compile_specifics_compile_filename_subset`, `cpython_globals_locals_builtin_subset`, `cpython_vars_dir_builtin_subset`, `cpython_isinstance_builtin_subset`, `cpython_issubclass_builtin_subset`, `cpython_attribute_introspection_builtins_subset`, `cpython_all_any_builtin_subset`, `cpython_len_builtin_subset`, `cpython_min_max_sum_builtin_subset`, `cpython_iter_next_builtin_subset`, `cpython_aiter_anext_builtin_subset`, `cpython_aiter_anext_keyword_error_subset`, `cpython_stop_iteration_value_subset`, `cpython_enumerate_zip_sorted_builtin_subset`, `cpython_builtin_sorted_exact_subset`, `cpython_zip_strict_builtin_subset`, `cpython_map_filter_builtin_subset`, `cpython_map_strict_builtin_subset`, `cpython_abs_builtin_subset`, `cpython_builtin_print_keyword_subset`, `cpython_divmod_builtin_subset`, `cpython_round_builtin_subset`, `cpython_pow_builtin_subset`, `cpython_chr_ord_builtin_subset`, `cpython_format_builtin_and_custom_dunder_format_subset`, `cpython_format_builtin_keyword_error_subset`, `cpython_ascii_builtin_subset`, `cpython_builtin_cmp_absent_subset`, `cpython_builtin_none_ne_direct_subset`, `cpython_builtin_exception_hierarchy_subset`, `cpython_runtime_exception_capture_subset`, `cpython_base_exception_args_subset`, `cpython_base_exception_with_traceback_subset`, `cpython_system_exit_oserror_attributes_subset`, `cpython_syntax_error_attributes_subset`, `cpython_unicode_error_attributes_subset`, `cpython_attribute_error_keyword_attributes_subset`, `cpython_object_repr_str_direct_subset`, `cpython_object_getstate_direct_subset`, `cpython_object_getstate_builtin_instance_subset`, `cpython_bool_instance_doc_attribute_subset`, `cpython_int_instance_doc_attribute_subset`, `cpython_float_instance_doc_attribute_subset`, `cpython_complex_instance_doc_attribute_subset`, `cpython_dict_instance_doc_attribute_subset`, `cpython_set_instance_doc_attribute_subset`, `cpython_frozenset_instance_doc_attribute_subset`, `cpython_memoryview_instance_doc_attribute_subset`, `cpython_memoryview_getstate_subset`, `cpython_str_builtin_custom_dunder_subset`, `cpython_str_instance_doc_attribute_subset`, `cpython_list_instance_doc_attribute_subset`, `cpython_tuple_instance_doc_attribute_subset`, `cpython_bytes_instance_doc_attribute_subset`, `cpython_bytearray_instance_doc_attribute_subset`, `cpython_range_instance_doc_attribute_subset`, `cpython_slice_instance_doc_attribute_subset`, `cpython_descriptor_constructor_arity_errors_subset`, `cpython_staticmethod_callable_subset`, `cpython_staticmethod_metadata_subset`, `cpython_classmethod_metadata_subset`, `cpython_staticmethod_classmethod_abstractmethod_subset`, `cpython_property_abstractmethod_subset`, `cpython_property_attribute_assignment_errors_subset`, `cpython_property_name_metadata_subset`, `cpython_property_set_name_metadata_subset`, `cpython_property_no_getter_error_subset`, `cpython_property_no_setter_deleter_error_subset`, `cpython_property_doc_metadata_subset`, `cpython_super_attribute_assignment_errors_subset`, `cpython_super_object_dir_supported_attributes_subset`, `cpython_super_type_public_descriptors_subset`, `cpython_super_repr_wrapper_descriptor_subset`, `cpython_super_getattribute_wrapper_descriptor_subset`, `cpython_builtin_bool_notimplemented_subset`, `cpython_builtin_construct_singletons_subset`, `cpython_object_constructor_argument_error_subset`, `cpython_list_constructor_keyword_error_subset`, `cpython_tuple_constructor_keyword_error_subset`, `cpython_set_constructor_keyword_error_subset`, `cpython_frozenset_constructor_keyword_error_subset`, `cpython_slice_constructor_keyword_error_subset`, `cpython_builtin_singleton_attribute_access_subset`, `cpython_hash_builtin_subset`, `cpython_id_builtin_subset`, `cpython_builtin_breakpoint_custom_hook_subset`, `cpython_builtin_breakpoint_default_stub_subset`, and `cpython_builtin_breakpoint_passthru_error_subset`. | `cpython_builtins_module_package_metadata_diff_subset`, `cpython_globals_locals_builtin_diff_subset`, `cpython_vars_dir_builtin_diff_subset`, `cpython_eval_builtin_diff_subset`, `cpython_exec_builtin_diff_subset`, `cpython_eval_exec_builtins_mapping_diff_subset`, `cpython_compile_builtin_code_object_diff_subset`, `cpython_compile_specifics_compile_filename_diff_subset`, `cpython_isinstance_builtin_diff_subset`, `cpython_issubclass_builtin_diff_subset`, `cpython_attribute_introspection_builtins_diff_subset`, `cpython_ascii_builtin_diff_subset`, `cpython_chr_ord_builtin_diff_subset`, `cpython_builtin_cmp_absent_diff_subset`, `cpython_builtin_none_ne_direct_diff_subset`, `cpython_builtin_exception_hierarchy_diff_subset`, `cpython_runtime_exception_capture_diff_subset`, `cpython_base_exception_args_diff_subset`, `cpython_base_exception_with_traceback_diff_subset`, `cpython_system_exit_oserror_attributes_diff_subset`, `cpython_syntax_error_attributes_diff_subset`, `cpython_unicode_error_attributes_diff_subset`, `cpython_attribute_error_keyword_attributes_diff_subset` is gated for CPython oracles with AttributeError name=/obj= keyword support, `cpython_object_repr_str_direct_diff_subset`, `cpython_object_getstate_direct_diff_subset`, `cpython_object_getstate_builtin_instance_diff_subset`, `cpython_bool_instance_doc_attribute_diff_subset`, `cpython_int_instance_doc_attribute_diff_subset`, `cpython_float_instance_doc_attribute_diff_subset`, `cpython_complex_instance_doc_attribute_diff_subset`, `cpython_dict_instance_doc_attribute_diff_subset`, `cpython_set_instance_doc_attribute_diff_subset`, `cpython_frozenset_instance_doc_attribute_diff_subset`, `cpython_memoryview_instance_doc_attribute_diff_subset`, `cpython_memoryview_getstate_diff_subset`, `cpython_str_builtin_custom_dunder_diff_subset`, `cpython_str_instance_doc_attribute_diff_subset`, `cpython_list_instance_doc_attribute_diff_subset`, `cpython_tuple_instance_doc_attribute_diff_subset`, `cpython_bytes_instance_doc_attribute_diff_subset`, `cpython_bytearray_instance_doc_attribute_diff_subset`, `cpython_range_instance_doc_attribute_diff_subset`, `cpython_slice_instance_doc_attribute_diff_subset`, `cpython_descriptor_constructor_arity_errors_diff_subset`, `cpython_staticmethod_callable_diff_subset` is gated for CPython oracles with current callable `staticmethod` object behavior, `cpython_staticmethod_metadata_diff_subset` is gated for CPython oracles with wrapped staticmethod metadata support, `cpython_classmethod_metadata_diff_subset` is gated for CPython oracles with wrapped classmethod metadata support, `cpython_staticmethod_classmethod_abstractmethod_diff_subset`, `cpython_property_abstractmethod_diff_subset`, `cpython_property_attribute_assignment_errors_diff_subset`, `cpython_property_name_metadata_diff_subset` is gated for CPython oracles with property.__name__ support, `cpython_property_set_name_metadata_diff_subset` is gated for CPython oracles with property.__set_name__/__name__ support, `cpython_property_no_getter_error_diff_subset`, `cpython_property_no_setter_deleter_error_diff_subset`, `cpython_property_doc_metadata_diff_subset`, `cpython_super_attribute_assignment_errors_diff_subset`, `cpython_super_object_dir_supported_attributes_diff_subset`, `cpython_super_type_public_descriptors_diff_subset`, `cpython_super_repr_wrapper_descriptor_diff_subset`, `cpython_super_getattribute_wrapper_descriptor_diff_subset`, `cpython_builtin_bool_notimplemented_diff_subset` is gated for CPython oracles with the current NotImplemented TypeError behavior, `cpython_builtin_singleton_construction_and_attributes_diff_subset`, `cpython_object_constructor_argument_error_diff_subset`, `cpython_list_constructor_keyword_error_diff_subset`, `cpython_tuple_constructor_keyword_error_diff_subset`, `cpython_set_constructor_keyword_error_diff_subset`, `cpython_frozenset_constructor_keyword_error_diff_subset`, `cpython_slice_constructor_keyword_error_diff_subset`, `cpython_all_any_builtin_diff_subset`, `cpython_len_builtin_diff_subset`, `cpython_min_max_sum_builtin_diff_subset`, `cpython_iter_next_builtin_diff_subset`, `cpython_aiter_anext_builtin_diff_subset`, `cpython_aiter_anext_keyword_error_diff_subset`, `cpython_stop_iteration_value_diff_subset`, `cpython_map_filter_builtin_diff_subset`, `cpython_map_strict_builtin_diff_subset` is gated for CPython oracles with map strict support, `cpython_enumerate_zip_sorted_builtin_diff_subset`, `cpython_builtin_sorted_exact_diff_subset`, `cpython_zip_strict_builtin_diff_subset`, `cpython_divmod_builtin_diff_subset`, `cpython_pow_builtin_diff_subset`, `cpython_abs_builtin_diff_subset`, `cpython_builtin_print_keyword_diff_subset`, `cpython_round_builtin_diff_subset`, `cpython_format_builtin_and_custom_dunder_format_diff_subset`, `cpython_format_builtin_keyword_error_diff_subset`, `cpython_hash_id_builtins_diff_subset`, `cpython_builtin_breakpoint_custom_hook_diff_subset`, `cpython_builtin_breakpoint_passthru_error_diff_subset`. | `open()`, `input()`, host TTY behavior, non-`None` `print(file=...)` targets, default pdb-backed breakpoint behavior, and process/environment side effects. |
| `sys` | In-memory module metadata including `builtin_module_names`, `byteorder`, `dont_write_bytecode`, `flags.debug`, `flags.dont_write_bytecode`, `flags.dev_mode`, `flags.hash_randomization`, `flags.ignore_environment`, `flags.inspect`, `flags.interactive`, `flags.n_fields`, `flags.n_sequence_fields`, `flags.n_unnamed_fields`, `flags.no_user_site`, `flags.no_site`, `flags.isolated`, `flags.optimize`, `flags.quiet`, `flags.utf8_mode`, `flags.verbose`, `flags.warn_default_encoding`, `flags.safe_path`, `flags.int_max_str_digits`, `float_info` / `hash_info` / `version_info` / `flags` structure metadata and `__match_args__`, `hexversion`, `implementation`, `version_info`, `warnoptions`, `modules`, stdio placeholders, `maxsize`, float/hash info, `getdefaultencoding()`, `is_finalizing()`, `exc_info()`, int digit-limit controls, frame inspection subset, breakpoint hook metadata, and sandbox default no-op breakpoint hook. Subset evidence: `cpython_sys_module_package_metadata_subset`, `cpython_float_hash_and_sys_info_subset`, `cpython_builtin_negation_sys_maxsize_subset`, `cpython_int_max_str_digits_runtime_subset`, `cpython_attribute_introspection_builtins_subset`, `cpython_builtin_breakpoint_custom_hook_subset`, `cpython_builtin_breakpoint_default_stub_subset`, `cpython_builtin_breakpoint_passthru_error_subset`, and `cpython_types_frame_locals_proxy_type_subset`. | `cpython_sys_module_package_metadata_diff_subset`, `cpython_globals_locals_builtin_diff_subset`, `cpython_attribute_introspection_builtins_diff_subset`, `cpython_builtin_negation_sys_maxsize_diff_subset`, `cpython_int_max_str_digits_runtime_diff_subset` is gated for CPython oracles with sys.get_int_max_str_digits() / sys.set_int_max_str_digits(), `cpython_builtin_breakpoint_custom_hook_diff_subset`, `cpython_builtin_breakpoint_passthru_error_diff_subset`, `cpython_float_hash_and_sys_info_diff_subset`, `cpython_types_frame_locals_proxy_type_diff_subset`. | Real argv/process state, real stdin/stdout/stderr streams, implementation refcount/GC/debug APIs, host filesystem encoding policy, host warning/environment option injection, bytecode cache writes, and cross-thread interpreter state. |
| `types` | Public type aliases and selected constructors/helpers for module/class creation, mappingproxy, simple namespace, generic alias/union, coroutine helpers, frame/code/traceback aliases. Subset evidence: `cpython_types_module_package_metadata_subset`, `cpython_types_accelerator_module_package_metadata_subset`, `cpython_types_names_public_surface_subset`, `cpython_types_singleton_type_aliases_subset`, `cpython_types_nonetype_unacceptable_base_type_subset`, `cpython_types_notimplementedtype_unacceptable_base_type_subset`, `cpython_types_ellipsistype_unacceptable_base_type_subset`, `cpython_types_capsuletype_unacceptable_base_type_subset`, `cpython_types_capsuletype_module_metadata_subset`, `cpython_types_capsuletype_qualname_metadata_subset`, `cpython_types_capsuletype_doc_metadata_subset`, `cpython_types_capsuletype_text_signature_metadata_subset`, `cpython_types_module_type_subset`, `cpython_types_runtime_type_aliases_subset`, `cpython_types_generic_alias_union_type_subset`, `cpython_types_union_public_operator_and_classinfo_subset`, `cpython_types_union_forward_ref_subset`, `cpython_types_union_typevar_parameter_subset`, `cpython_types_union_parameter_substitution_subset`, `cpython_types_union_bad_classinfo_checks_subset`, `cpython_types_union_newtype_subset`, `cpython_types_mappingproxy_exact_dict_subset`, `cpython_types_mappingproxy_type_metadata_subset`, `cpython_types_mappingproxy_instance_doc_attribute_subset`, `cpython_types_mappingproxy_type_hierarchy_subset`, `cpython_types_mappingproxy_type_subclasses_subset`, `cpython_types_mappingproxy_type_dict_subset`, `cpython_types_mappingproxy_type_richcompare_subset`, `cpython_types_mappingproxy_keyword_constructor_subset`, `cpython_types_mappingproxy_method_surface_subset`, `cpython_types_mappingproxy_attribute_assignment_errors_subset`, `cpython_types_mappingproxy_custom_mapping_subset`, `cpython_types_mappingproxy_union_subset`, `cpython_types_mappingproxy_hash_subset`, `cpython_types_mappingproxy_richcompare_subset`, `cpython_types_mappingproxy_contains_subset`, `cpython_types_mappingproxy_views_subset`, `cpython_types_mappingproxy_len_subset`, `cpython_types_mappingproxy_iterators_subset`, `cpython_types_mappingproxy_reversed_subset`, `cpython_types_mappingproxy_copy_subset`, `cpython_types_simple_namespace_basic_subset`, `cpython_types_simple_namespace_recursive_and_replace_subset`, `cpython_types_simple_namespace_new_and_invalid_replace_subset`, `cpython_types_simple_namespace_remaining_public_subset`, `cpython_types_class_creation_new_class_resolve_bases_subset`, `cpython_types_class_creation_new_class_meta_helper_subset`, `cpython_types_class_creation_new_class_metaclass_keywords_subset`, `cpython_types_class_creation_prepare_resolve_bases_subset`, `cpython_types_class_creation_mro_entries_core_subset`, `cpython_types_class_creation_mro_entries_multiple_subset`, `cpython_types_class_creation_prepare_and_metaclass_callable_subset`, `cpython_types_class_creation_metaclass_override_function_subset`, `cpython_types_class_creation_non_type_metaclass_derivation_subset`, `cpython_types_class_creation_metaclass_derivation_subset`, `cpython_types_class_creation_one_argument_type_subset`, `cpython_types_class_creation_init_subclass_keyword_error_subset`, `cpython_types_class_creation_init_subclass_return_value_subset`, `cpython_types_class_creation_init_subclass_unexpected_keyword_owner_subset`, `cpython_types_class_creation_init_subclass_missing_required_owner_subset`, `cpython_types_class_creation_init_subclass_bound_classmethod_subset`, `cpython_types_coroutine_public_subset`, `cpython_types_coroutine_async_def_subset`, `cpython_types_coroutine_generator_wrapper_subset`, `cpython_types_coroutine_generator_frame_subset`, `cpython_types_coroutine_generator_yieldfrom_subset`, `cpython_types_coroutine_duck_generator_wrapper_subset`, `cpython_types_coroutine_duck_generator_await_subset`, `cpython_types_coroutine_duck_generator_proxy_subset`, `cpython_types_function_type_subset`, `cpython_types_functiontype_unacceptable_base_type_subset`, `cpython_types_codetype_unacceptable_base_type_subset`, `cpython_types_tracebacktype_unacceptable_base_type_subset`, `cpython_types_frametype_unacceptable_base_type_subset`, `cpython_types_generatortype_unacceptable_base_type_subset`, `cpython_types_coroutinetype_unacceptable_base_type_subset`, `cpython_types_asyncgeneratortype_unacceptable_base_type_subset`, `cpython_types_builtinfunctiontype_unacceptable_base_type_subset`, `cpython_types_methodtype_unacceptable_base_type_subset`, `cpython_types_methoddescriptortype_unacceptable_base_type_subset`, `cpython_types_classmethoddescriptortype_unacceptable_base_type_subset`, `cpython_types_wrapperdescriptortype_unacceptable_base_type_subset`, `cpython_types_methodwrappertype_unacceptable_base_type_subset`, `cpython_types_getsetdescriptortype_unacceptable_base_type_subset`, `cpython_types_memberdescriptortype_unacceptable_base_type_subset`, `cpython_types_code_traceback_type_aliases_subset`, `cpython_types_frame_type_alias_subset`, `cpython_types_celltype_constructor_behavior_subset`, `cpython_types_celltype_cell_contents_descriptor_subset`, `cpython_types_celltype_attribute_errors_subset`, `cpython_types_celltype_dir_surface_subset`, `cpython_types_celltype_getstate_subset`, `cpython_types_celltype_hash_semantics_subset`, `cpython_types_celltype_instance_display_subset`, `cpython_types_celltype_rich_compare_subset`, `cpython_types_celltype_module_metadata_subset`, `cpython_types_celltype_base_metadata_subset`, `cpython_types_celltype_display_metadata_subset`, `cpython_types_celltype_name_metadata_subset`, `cpython_types_celltype_qualname_metadata_subset`, `cpython_types_celltype_text_signature_metadata_subset`, `cpython_types_celltype_doc_metadata_subset`, `cpython_types_celltype_keyword_error_subset`, `cpython_types_celltype_unacceptable_base_type_subset`, `cpython_types_slot_and_method_wrapper_types_subset`, `cpython_types_dunder_get_signature_subset`, and `cpython_types_frame_locals_proxy_type_subset`. | `cpython_types_module_package_metadata_diff_subset`, `cpython_types_accelerator_module_package_metadata_diff_subset`, `cpython_types_names_public_surface_diff_subset` is gated for CPython oracles with the current public `types.__all__` surface, `cpython_types_singleton_type_aliases_diff_subset` is gated for CPython oracles with public singleton aliases, `cpython_types_nonetype_unacceptable_base_type_diff_subset`, `cpython_types_notimplementedtype_unacceptable_base_type_diff_subset`, `cpython_types_ellipsistype_unacceptable_base_type_diff_subset`, `cpython_types_capsuletype_unacceptable_base_type_diff_subset`, `cpython_types_capsuletype_module_metadata_diff_subset`, `cpython_types_capsuletype_qualname_metadata_diff_subset`, `cpython_types_capsuletype_doc_metadata_diff_subset`, `cpython_types_capsuletype_text_signature_metadata_diff_subset`, `cpython_types_module_type_diff_subset`, `cpython_types_generic_alias_union_type_diff_subset` is gated for CPython oracles with public GenericAlias and UnionType aliases, `cpython_types_union_public_operator_and_classinfo_diff_subset` is gated for CPython oracles with PEP 604 union support, `cpython_types_union_forward_ref_diff_subset` is gated for CPython oracles with public UnionType and ForwardRef support, `cpython_types_union_forward_get_type_hints_diff_subset` is gated for CPython oracles with PEP 604 TypeVar/string forward-ref union support, `cpython_types_union_typevar_parameter_diff_subset` is gated for CPython oracles with PEP 604 TypeVar substitution support, `cpython_types_union_parameter_substitution_diff_subset` is gated for CPython oracles with PEP 604 parameter substitution support, `cpython_types_union_copy_pickle_diff_subset` is gated for CPython oracles with PEP 604 GenericAlias union support, `cpython_types_union_bad_module_guard_diff_subset`, `cpython_types_union_genericalias_subclass_bad_eq_diff_subset` is gated for CPython oracles with PEP 604 GenericAlias subclass union support, `cpython_types_union_bad_classinfo_checks_diff_subset` is gated for CPython oracles with PEP 604 metaclass union support, `cpython_types_union_unhashable_metaclass_diff_subset` is gated for CPython oracles with PEP 604 metaclass union support, `cpython_types_union_dynamic_hashability_diff_subset` is gated for CPython oracles with PEP 604 metaclass union support, `cpython_types_union_newtype_diff_subset` is gated for CPython oracles with PEP 604 NewType union support, `cpython_types_union_io_diff_subset` is gated for CPython oracles with PEP 604 typing.IO union support, `cpython_types_union_typed_dict_diff_subset` is gated for CPython oracles with PEP 604 TypedDict union support, `cpython_types_union_protocol_diff_subset` is gated for CPython oracles with PEP 604 Protocol union support, `cpython_types_union_special_form_diff_subset` is gated for CPython oracles with PEP 604 SpecialForm union support, `cpython_types_union_literal_diff_subset` is gated for CPython oracles with PEP 604 Literal union support, `cpython_types_class_creation_new_class_meta_helper_diff_subset`, `cpython_types_class_creation_one_argument_type_diff_subset`, `cpython_types_class_creation_get_original_bases_diff_subset` is gated for CPython oracles with types.get_original_bases(), `cpython_types_class_creation_metaclass_new_error_diff_subset`, `cpython_types_class_creation_subclass_inherited_slot_update_diff_subset`, `cpython_types_class_creation_init_subclass_keyword_error_diff_subset`, `cpython_types_class_creation_init_subclass_return_value_diff_subset`, `cpython_types_class_creation_init_subclass_unexpected_keyword_owner_diff_subset`, `cpython_types_class_creation_init_subclass_missing_required_owner_diff_subset`, `cpython_types_class_creation_init_subclass_bound_classmethod_diff_subset`, `cpython_types_class_creation_new_class_resolve_bases_diff_subset`, `cpython_types_class_creation_mro_entries_core_diff_subset` is gated for CPython oracles with generic-alias class creation support, `cpython_types_class_creation_mro_entries_multiple_diff_subset`, `cpython_types_class_creation_prepare_resolve_bases_diff_subset` is gated for CPython oracles with generic-alias resolve_bases support, `cpython_types_class_creation_prepare_and_metaclass_callable_diff_subset`, `cpython_types_class_creation_metaclass_override_function_diff_subset`, `cpython_types_class_creation_non_type_metaclass_derivation_diff_subset`, `cpython_types_class_creation_metaclass_derivation_diff_subset`, `cpython_types_coroutine_public_diff_subset`, `cpython_types_coroutine_async_def_diff_subset`, `cpython_types_coroutine_generator_wrapper_diff_subset` is gated for CPython oracles with _GeneratorWrapper suspended aliases, `cpython_types_coroutine_generator_frame_diff_subset` is gated for CPython oracles with _GeneratorWrapper frame aliases, `cpython_types_coroutine_generator_yieldfrom_diff_subset` is gated for CPython oracles with _GeneratorWrapper yield-from aliases, `cpython_types_coroutine_duck_generator_wrapper_diff_subset`, `cpython_types_coroutine_duck_generator_await_diff_subset`, `cpython_types_coroutine_duck_generator_proxy_diff_subset` is gated for CPython oracles with _GeneratorWrapper cr_suspended proxy aliases, `cpython_types_function_type_diff_subset`, `cpython_types_functiontype_unacceptable_base_type_diff_subset`, `cpython_types_codetype_unacceptable_base_type_diff_subset`, `cpython_types_tracebacktype_unacceptable_base_type_diff_subset`, `cpython_types_frametype_unacceptable_base_type_diff_subset`, `cpython_types_generatortype_unacceptable_base_type_diff_subset`, `cpython_types_coroutinetype_unacceptable_base_type_diff_subset`, `cpython_types_asyncgeneratortype_unacceptable_base_type_diff_subset`, `cpython_types_builtinfunctiontype_unacceptable_base_type_diff_subset`, `cpython_types_methodtype_unacceptable_base_type_diff_subset`, `cpython_types_methoddescriptortype_unacceptable_base_type_diff_subset`, `cpython_types_classmethoddescriptortype_unacceptable_base_type_diff_subset`, `cpython_types_wrapperdescriptortype_unacceptable_base_type_diff_subset`, `cpython_types_methodwrappertype_unacceptable_base_type_diff_subset`, `cpython_types_getsetdescriptortype_unacceptable_base_type_diff_subset`, `cpython_types_memberdescriptortype_unacceptable_base_type_diff_subset`, `cpython_types_code_traceback_type_aliases_diff_subset`, `cpython_types_frame_type_alias_diff_subset`, `cpython_types_runtime_type_aliases_diff_subset`, `cpython_types_celltype_constructor_behavior_diff_subset`, `cpython_types_celltype_cell_contents_descriptor_diff_subset`, `cpython_types_celltype_attribute_errors_diff_subset`, `cpython_types_celltype_dir_surface_diff_subset`, `cpython_types_celltype_getstate_diff_subset`, `cpython_types_celltype_hash_semantics_diff_subset`, `cpython_types_celltype_instance_display_diff_subset`, `cpython_types_celltype_rich_compare_diff_subset`, `cpython_types_celltype_module_metadata_diff_subset`, `cpython_types_celltype_base_metadata_diff_subset`, `cpython_types_celltype_display_metadata_diff_subset`, `cpython_types_celltype_name_metadata_diff_subset`, `cpython_types_celltype_qualname_metadata_diff_subset`, `cpython_types_celltype_text_signature_metadata_diff_subset`, `cpython_types_celltype_doc_metadata_diff_subset`, `cpython_types_celltype_keyword_error_diff_subset`, `cpython_types_celltype_unacceptable_base_type_diff_subset`, `cpython_types_float_constructor_edges_diff_subset`, `cpython_types_float_to_string_diff_subset`, `cpython_types_normal_integers_diff_subset`, `cpython_types_format_spec_errors_diff_subset`, `cpython_types_mappingproxy_exact_dict_diff_subset`, `cpython_types_mappingproxy_type_metadata_diff_subset`, `cpython_types_mappingproxy_instance_doc_attribute_diff_subset`, `cpython_types_mappingproxy_type_hierarchy_diff_subset`, `cpython_types_mappingproxy_type_subclasses_diff_subset`, `cpython_types_mappingproxy_type_dict_diff_subset`, `cpython_types_mappingproxy_type_richcompare_diff_subset`, `cpython_types_mappingproxy_keyword_constructor_diff_subset`, `cpython_types_mappingproxy_method_surface_diff_subset`, `cpython_types_mappingproxy_attribute_assignment_errors_diff_subset`, `cpython_types_mappingproxy_union_diff_subset`, `cpython_types_mappingproxy_hash_diff_subset` is gated for CPython oracles with hashable mappingproxy support, `cpython_types_mappingproxy_contains_diff_subset`, `cpython_types_mappingproxy_views_diff_subset`, `cpython_types_mappingproxy_missing_diff_subset`, `cpython_types_mappingproxy_len_diff_subset`, `cpython_types_mappingproxy_iterators_diff_subset`, `cpython_types_mappingproxy_reversed_diff_subset`, `cpython_types_mappingproxy_copy_diff_subset`, `cpython_types_mappingproxy_richcompare_diff_subset`, `cpython_types_mappingproxy_custom_mapping_diff_subset`, `cpython_types_mappingproxy_chainmap_diff_subset`, `cpython_types_simple_namespace_basic_diff_subset`, `cpython_types_simple_namespace_recursive_diff_subset`, `cpython_types_simple_namespace_new_and_invalid_replace_diff_subset` is gated for CPython oracles with copy.replace() and types.SimpleNamespace.__replace__, `cpython_types_simple_namespace_remaining_public_diff_subset` is gated for CPython oracles with SimpleNamespace positional mapping construction, `cpython_types_simple_namespace_state_order_diff_subset`, `cpython_types_simple_namespace_fake_comparison_diff_subset`, `cpython_types_method_descriptor_types_diff_subset`, `cpython_types_dunder_get_signature_diff_subset`, `cpython_types_slot_and_method_wrapper_types_diff_subset`, `cpython_types_frame_locals_proxy_type_diff_subset`, `cpython_types_int_format_diff_subset`, `cpython_types_float_format_diff_subset`. | CPython object-layout internals, exact C descriptor types beyond the public aliases, pickle identity matrices, and interpreter lifecycle behavior. |
| `collections` / `collections.abc` | `namedtuple`, `Counter`, `ChainMap`, `UserDict`, `UserList`, `deque`, `defaultdict`, selected ABCs and mixins used by the runtime and migrated CPython tests. Subset evidence: `cpython_collections_module_package_metadata_subset`, `cpython_collections_counter_basics_subset`, `cpython_collections_counter_public_subset`, `cpython_collections_counter_instance_doc_attribute_subset`, `cpython_collections_counter_type_base_metadata_subset`, `cpython_collections_counter_type_mro_metadata_subset`, `cpython_collections_counter_conversions_subset`, `cpython_collections_counter_init_update_subset`, `cpython_collections_counter_comparison_subset`, `cpython_collections_counter_fromkeys_subset`, `cpython_collections_counter_most_common_subset`, `cpython_collections_counter_mapping_mutation_subset`, `cpython_collections_counter_repr_nonsortable_subset`, `cpython_collections_counter_subtract_unary_subset`, `cpython_collections_counter_copy_subclass_subset`, `cpython_collections_counter_copying_subset`, `cpython_collections_counter_order_preservation_subset`, `cpython_collections_counter_update_reentrant_add_clears_counter_subset`, `cpython_collections_counter_helper_function_subset`, `cpython_collections_counter_multiset_operations_subset`, `cpython_collections_counter_multiset_operations_matrix_subset`, `cpython_collections_counter_multiset_operations_equivalent_to_set_operations_subset`, `cpython_collections_counter_symmetric_difference_absent_subset`, `cpython_collections_counter_inplace_operations_subset`, `cpython_collections_counter_inplace_operations_matrix_subset`, `cpython_collections_chainmap_public_methods_subset`, `cpython_collections_chainmap_instance_doc_attribute_subset`, `cpython_collections_chainmap_type_base_metadata_subset`, `cpython_collections_chainmap_keyword_error_subset`, `cpython_collections_chainmap_constructor_lazy_mapping_subset`, `cpython_collections_chainmap_constructor_source_repr_subset`, `cpython_collections_chainmap_subclass_source_repr_subset`, `cpython_collections_chainmap_subclass_display_type_methods_subset`, `cpython_collections_chainmap_constructor_source_truthiness_subset`, `cpython_collections_chainmap_subclass_source_truthiness_subset`, `cpython_collections_chainmap_constructor_source_len_subset`, `cpython_collections_chainmap_subclass_source_len_subset`, `cpython_collections_chainmap_constructor_source_iter_subset`, `cpython_collections_chainmap_constructor_source_items_subset`, `cpython_collections_chainmap_copy_sharing_subset`, `cpython_collections_namedtuple_factory_instance_subset`, `cpython_collections_namedtuple_public_subset`, `cpython_collections_namedtuple_defaults_rename_readonly_subset`, `cpython_collections_namedtuple_repr_subset`, `cpython_collections_namedtuple_name_conflicts_subset`, `cpython_collections_namedtuple_subclass_issue_24931_subset`, `cpython_collections_namedtuple_match_args_subset`, `cpython_collections_namedtuple_large_size_subset`, `cpython_collections_namedtuple_field_doc_subset`, `cpython_collections_namedtuple_copy_keyword_generic_alias_subset`, `cpython_collections_namedtuple_new_builtins_issue_43102_subset`, `cpython_collections_namedtuple_new_builtins_globals_subset`, `cpython_collections_userdict_userlist_public_subset`, `cpython_collections_userdict_public_methods_subset`, `cpython_collections_userdict_instance_doc_attribute_subset`, `cpython_collections_userdict_type_doc_attribute_subset`, `cpython_collections_userdict_type_base_metadata_subset`, `cpython_collections_userlist_instance_doc_attribute_subset`, `cpython_collections_userlist_type_base_metadata_subset`, `cpython_collections_userlist_public_methods_subset`, `cpython_collections_userlist_mutating_eq_subset`, `cpython_collections_userlist_namedtuple_sequence_order_subset`, `cpython_collections_userstring_type_base_metadata_subset`, `cpython_collections_userstring_protocol_and_userdict_missing_subset`, `cpython_collections_defaultdict_core_subset`, `cpython_collections_defaultdict_instance_doc_attribute_subset`, `cpython_collections_defaultdict_type_base_metadata_subset`, `cpython_collections_defaultdict_type_mro_metadata_subset`, `cpython_collections_defaultdict_default_factory_descriptor_subset`, `cpython_collections_defaultdict_attribute_assignment_errors_subset`, `cpython_collections_defaultdict_copy_module_subset`, `cpython_collections_defaultdict_dunder_copy_subset`, `cpython_collections_defaultdict_copy_descriptor_errors_subset`, `cpython_collections_defaultdict_fromkeys_subset`, `cpython_collections_defaultdict_missing_descriptor_subset`, `cpython_collections_defaultdict_format_error_subset`, `cpython_collections_deque_public_surface_subset`, `cpython_collections_deque_attribute_assignment_errors_subset`, `cpython_collections_deque_mutating_eq_subset`, `cpython_collections_chainmap_missing_and_first_map_mutation_subset`, `cpython_collections_chainmap_iter_does_not_call_getitem_subset`, `cpython_collections_chainmap_new_child_custom_mapping_subset`, `cpython_collections_chainmap_order_preservation_subset`, `cpython_collections_chainmap_union_operators_subset`, `cpython_collections_abc_core_runtime_subset`, `cpython_collections_abc_type_hierarchy_metadata_subset`, `cpython_collections_abc_iterable_iterator_subset`, `cpython_collections_abc_iterable_sample_matrix_subset`, `cpython_collections_abc_iterator_sample_matrix_subset`, `cpython_collections_abc_sequence_subset`, `cpython_collections_abc_sequence_mixins_subset`, `cpython_collections_abc_mapping_subset`, `cpython_collections_abc_mapping_view_subset`, `cpython_collections_abc_mutable_sequence_subset`, `cpython_collections_abc_mapping_mixins_subset`, `cpython_collections_abc_mapping_mixin_views_subset`, `cpython_collections_abc_userdict_view_snapshot_subset`, `cpython_collections_abc_set_mutable_set_mixins_subset`, `cpython_collections_abc_set_from_iterable_operator_subset`, `cpython_collections_abc_set_real_set_interoperability_subset`, `cpython_collections_abc_set_hash_matches_frozenset_subset`, `cpython_collections_abc_issue26915_identity_first_object_subset`, `cpython_collections_abc_set_noncomparable_comparison_subset`, `cpython_collections_abc_reversible_subset`, `cpython_collections_abc_reversible_direct_subclass_subset`, `cpython_collections_abc_collection_direct_subclass_subset`, `cpython_collections_abc_async_runtime_subset`, `cpython_collections_abc_async_iterator_mixin_subset`, `cpython_collections_abc_async_generator_core_mixin_subset`, `cpython_collections_abc_async_generator_throw_close_mixin_subset`, `cpython_collections_abc_generator_mixin_subset`, `cpython_collections_abc_generator_sample_matrix_subset`, `cpython_collections_abc_generator_runtime_subset`, `cpython_collections_abc_types_coroutine_subset`, `cpython_collections_abc_coroutine_mixin_subset`, `cpython_collections_abc_abstract_methods_subset`, `cpython_collections_abc_validate_isinstance_subset`, `cpython_collections_abc_direct_subclassing_subset`, `cpython_collections_abc_hashable_direct_subclass_subset`, `cpython_collections_abc_registration_subset`, `cpython_collections_abc_bytestring_buffer_subset`, `cpython_collections_abc_bytestring_deprecation_warnings_subset`, and `cpython_collections_abc_composite_abstract_methods_subset`. | `cpython_collections_module_package_metadata_diff_subset`, `cpython_collections_counter_basics_diff_subset`, `cpython_collections_counter_public_diff_subset`, `cpython_collections_counter_instance_doc_attribute_diff_subset`, `cpython_collections_counter_type_base_metadata_diff_subset`, `cpython_collections_counter_type_mro_metadata_diff_subset`, `cpython_collections_counter_conversions_diff_subset`, `cpython_collections_counter_init_update_diff_subset`, `cpython_collections_counter_comparison_diff_subset` (gated for CPython oracles with Counter.total() and zero-count rich comparison), `cpython_collections_counter_fromkeys_diff_subset`, `cpython_collections_counter_most_common_diff_subset`, `cpython_collections_counter_mapping_mutation_diff_subset`, `cpython_collections_counter_repr_nonsortable_diff_subset`, `cpython_collections_counter_subtract_unary_diff_subset`, `cpython_collections_counter_copy_subclass_diff_subset`, `cpython_collections_counter_copying_diff_subset`, `cpython_collections_counter_order_preservation_diff_subset`, `cpython_collections_counter_update_reentrant_add_clears_counter_diff_subset`, `cpython_collections_counter_helper_function_diff_subset`, `cpython_collections_counter_multiset_operations_diff_subset`, `cpython_collections_counter_multiset_operations_matrix_diff_subset`, `cpython_collections_counter_multiset_operations_equivalent_to_set_operations_diff_subset` (gated for CPython oracles with Counter.__xor__), `cpython_collections_counter_symmetric_difference_diff_subset` (gated for CPython oracles with Counter.__xor__), `cpython_collections_counter_inplace_operations_diff_subset`, `cpython_collections_counter_inplace_operations_matrix_diff_subset`, `cpython_collections_chainmap_public_methods_diff_subset`, `cpython_collections_chainmap_instance_doc_attribute_diff_subset`, `cpython_collections_chainmap_type_base_metadata_diff_subset`, `cpython_collections_chainmap_keyword_error_diff_subset`, `cpython_collections_chainmap_constructor_lazy_mapping_diff_subset`, `cpython_collections_chainmap_constructor_source_repr_diff_subset`, `cpython_collections_chainmap_subclass_source_repr_diff_subset`, `cpython_collections_chainmap_subclass_display_type_methods_diff_subset`, `cpython_collections_chainmap_constructor_source_truthiness_diff_subset`, `cpython_collections_chainmap_subclass_source_truthiness_diff_subset`, `cpython_collections_chainmap_constructor_source_len_diff_subset`, `cpython_collections_chainmap_subclass_source_len_diff_subset`, `cpython_collections_chainmap_constructor_source_iter_diff_subset`, `cpython_collections_chainmap_constructor_source_items_diff_subset`, `cpython_collections_chainmap_copy_sharing_diff_subset`, `cpython_collections_namedtuple_factory_instance_diff_subset` (gated for CPython oracles with generated namedtuple match args), `cpython_collections_namedtuple_public_diff_subset`, `cpython_collections_namedtuple_defaults_rename_readonly_diff_subset`, `cpython_collections_namedtuple_repr_diff_subset`, `cpython_collections_namedtuple_name_conflicts_diff_subset`, `cpython_collections_namedtuple_subclass_issue_24931_diff_subset`, `cpython_collections_namedtuple_match_args_diff_subset` (gated for CPython oracles with generated namedtuple match args), `cpython_collections_namedtuple_large_size_diff_subset`, `cpython_collections_namedtuple_field_doc_diff_subset`, `cpython_collections_namedtuple_copy_keyword_generic_alias_diff_subset`, `cpython_collections_namedtuple_new_builtins_globals_diff_subset`, `cpython_collections_namedtuple_new_builtins_issue_43102_diff_subset` (gated for CPython oracles with function.__builtins__), `cpython_collections_userdict_userlist_public_diff_subset`, `cpython_collections_userdict_public_methods_diff_subset`, `cpython_collections_userdict_instance_doc_attribute_diff_subset`, `cpython_collections_userdict_type_doc_attribute_diff_subset`, `cpython_collections_userdict_type_base_metadata_diff_subset`, `cpython_collections_userlist_instance_doc_attribute_diff_subset`, `cpython_collections_userlist_type_base_metadata_diff_subset`, `cpython_collections_userlist_public_methods_diff_subset`, `cpython_collections_userlist_mutating_eq_diff_subset`, `cpython_collections_userlist_namedtuple_sequence_order_diff_subset`, `cpython_collections_userstring_type_base_metadata_diff_subset`, `cpython_collections_userstring_protocol_and_userdict_missing_diff_subset`, `cpython_collections_defaultdict_core_diff_subset`, `cpython_collections_defaultdict_instance_doc_attribute_diff_subset`, `cpython_collections_defaultdict_type_base_metadata_diff_subset`, `cpython_collections_defaultdict_type_mro_metadata_diff_subset`, `cpython_collections_defaultdict_default_factory_descriptor_diff_subset`, `cpython_collections_defaultdict_attribute_assignment_errors_diff_subset`, `cpython_collections_defaultdict_copy_module_diff_subset`, `cpython_collections_defaultdict_dunder_copy_diff_subset`, `cpython_collections_defaultdict_copy_descriptor_errors_diff_subset`, `cpython_collections_defaultdict_fromkeys_diff_subset`, `cpython_collections_defaultdict_missing_descriptor_diff_subset`, `cpython_collections_defaultdict_format_error_diff_subset`, `cpython_collections_deque_public_surface_diff_subset`, `cpython_collections_deque_attribute_assignment_errors_diff_subset`, `cpython_collections_deque_mutating_eq_diff_subset`, `cpython_collections_chainmap_missing_and_first_map_mutation_diff_subset`, `cpython_collections_chainmap_iter_does_not_call_getitem_diff_subset`, `cpython_collections_chainmap_new_child_custom_mapping_diff_subset`, `cpython_collections_chainmap_order_preservation_diff_subset`, `cpython_collections_chainmap_union_operators_diff_subset`, `cpython_collections_abc_core_runtime_diff_subset`, `cpython_collections_abc_type_hierarchy_metadata_diff_subset`, `cpython_collections_abc_iterable_iterator_diff_subset`, `cpython_collections_abc_iterable_sample_matrix_diff_subset`, `cpython_collections_abc_iterator_sample_matrix_diff_subset`, `cpython_collections_abc_sequence_diff_subset`, `cpython_collections_abc_sequence_mixins_diff_subset`, `cpython_collections_abc_mapping_diff_subset`, `cpython_collections_abc_mapping_view_diff_subset`, `cpython_collections_abc_mutable_sequence_diff_subset`, `cpython_collections_abc_mapping_mixins_diff_subset`, `cpython_collections_abc_mapping_mixin_views_diff_subset`, `cpython_collections_abc_userdict_view_snapshot_diff_subset`, `cpython_collections_abc_set_mutable_set_mixins_diff_subset`, `cpython_collections_abc_set_from_iterable_operator_diff_subset`, `cpython_collections_abc_set_real_set_interoperability_diff_subset`, `cpython_collections_abc_set_hash_matches_frozenset_diff_subset`, `cpython_collections_abc_issue26915_identity_first_object_diff_subset`, `cpython_collections_abc_set_noncomparable_comparison_diff_subset`, `cpython_collections_abc_reversible_diff_subset`, `cpython_collections_abc_reversible_direct_subclass_diff_subset`, `cpython_collections_abc_collection_direct_subclass_diff_subset`, `cpython_collections_abc_async_runtime_diff_subset`, `cpython_collections_abc_async_iterator_mixin_diff_subset`, `cpython_collections_abc_async_generator_core_mixin_diff_subset`, `cpython_collections_abc_async_generator_throw_close_mixin_diff_subset`, `cpython_collections_abc_generator_mixin_diff_subset`, `cpython_collections_abc_generator_sample_matrix_diff_subset`, `cpython_collections_abc_generator_runtime_diff_subset`, `cpython_collections_abc_types_coroutine_diff_subset`, `cpython_collections_abc_coroutine_mixin_diff_subset`, `cpython_collections_abc_abstract_methods_diff_subset`, `cpython_collections_abc_validate_isinstance_diff_subset`, `cpython_collections_abc_direct_subclassing_diff_subset`, `cpython_collections_abc_hashable_direct_subclass_diff_subset`, `cpython_collections_abc_registration_diff_subset`, `cpython_collections_abc_bytestring_buffer_diff_subset` (gated for CPython oracles with collections.abc.Buffer), `cpython_collections_abc_bytestring_deprecation_warnings_diff_subset` (gated for CPython oracles with ByteString deprecation warnings), and `cpython_collections_abc_composite_abstract_methods_diff_subset` (gated for CPython oracles with collections.abc.Buffer). | Full deque construction/mutation APIs, full defaultdict pickle/merge operators/subclass compatibility, performance/lifetime internals, thread-safety stress, pickle/eval identity matrices, and unported ABC edge matrices. |
| `math` / `math.integer` | Constants, classification, elementary functions, integer math, selected IEEE edge behavior, public function qualname/doc metadata, deterministic public numeric results, and representative no-keyword `TypeError` diagnostics for pure-memory public functions, including the CPython-prefixed `math.log()` keyword diagnostic. Subset evidence: `cpython_math_core_subset`, `cpython_math_keyword_error_messages_subset`, `cpython_math_constants_and_classification_subset`, `cpython_math_integer_subset`, `cpython_math_isclose_subset`, `cpython_math_hypot_dist_subset`, `cpython_math_copysign_subset`, `cpython_math_signbit_subset`, `cpython_math_trunc_subset`, `cpython_math_ceil_subset`, `cpython_math_floor_subset`, `cpython_math_degrees_radians_subset`, `cpython_math_cbrt_subset`, `cpython_math_erf_erfc_subset`, `cpython_math_gamma_lgamma_subset`, `cpython_math_exp_exp2_subset`, `cpython_math_expm1_subset`, `cpython_math_log_family_subset`, `cpython_math_trig_subset`, `cpython_math_hyperbolic_subset`, `cpython_math_fabs_subset`, `cpython_math_fma_subset`, `cpython_math_fmax_fmin_subset`, `cpython_math_fmod_remainder_subset`, `cpython_math_frexp_ldexp_modf_subset`, `cpython_math_fsum_subset`, `cpython_math_sumprod_subset`, `cpython_math_nextafter_ulp_subset`, `cpython_math_pow_subset`, `cpython_math_sqrt_subset`, `cpython_math_gcd_subset`, `cpython_math_lcm_subset`, and `cpython_math_prod_subset`. | `cpython_math_core_diff_subset`, `cpython_math_keyword_error_messages_diff_subset`, `cpython_math_constants_and_classification_diff_subset` is gated for newer CPython oracles, `cpython_math_isclose_diff_subset`, `cpython_math_hypot_dist_diff_subset`, `cpython_math_gcd_diff_subset`, `cpython_math_lcm_diff_subset`, `cpython_math_prod_diff_subset`, `cpython_math_integer_diff_subset`, `cpython_math_integer_alias_diff_subset` is gated for CPython oracles with `math.integer`, `cpython_math_sqrt_diff_subset`, `cpython_math_fabs_diff_subset`, `cpython_math_copysign_diff_subset`, `cpython_math_signbit_diff_subset` is gated for newer CPython oracles, `cpython_math_trunc_diff_subset`, `cpython_math_ceil_diff_subset`, `cpython_math_floor_diff_subset`, `cpython_math_degrees_radians_diff_subset`, `cpython_math_cbrt_diff_subset` is gated for newer CPython oracles, `cpython_math_erf_erfc_diff_subset`, `cpython_math_gamma_lgamma_diff_subset`, `cpython_math_fma_diff_subset` is gated for newer CPython oracles, `cpython_math_fmax_fmin_diff_subset` is gated for newer CPython oracles, `cpython_math_exp_exp2_diff_subset`, `cpython_math_expm1_diff_subset`, `cpython_math_log_family_diff_subset`, `cpython_math_trig_diff_subset`, `cpython_math_hyperbolic_diff_subset`, `cpython_math_fmod_remainder_diff_subset`, `cpython_math_frexp_ldexp_modf_diff_subset`, `cpython_math_fsum_diff_subset`, `cpython_math_sumprod_diff_subset` is gated for newer CPython oracles, `cpython_math_nextafter_ulp_diff_subset` is gated for newer CPython oracles, `cpython_math_pow_diff_subset`. | Platform/libm implementation quirks, exact libm special-function precision, and locale-sensitive parsing/formatting. |
| `array` | Pure-memory `array.array` construction, sequence/mutation methods, bytes conversion, copy/deepcopy, buffer exposure, and `io.BytesIO` backed `tofile()` / `fromfile()` subset. Subset evidence: `cpython_array_module_package_metadata_subset`, `cpython_array_module_and_constructor_public_surface_subset`, `cpython_array_subclass_public_construction_subset`, `cpython_array_one_byte_public_sequence_subset`, `cpython_array_short_public_sequence_and_mutation_subset`, `cpython_array_int_public_sequence_and_mutation_subset`, `cpython_array_long_long_public_sequence_and_mutation_subset`, `cpython_array_native_long_public_sequence_and_mutation_subset`, `cpython_array_float_public_sequence_and_mutation_subset`, `cpython_array_unicode_public_sequence_and_mutation_subset`, `cpython_array_one_byte_public_mutation_methods_subset`, `cpython_array_one_byte_public_clear_subset`, `cpython_array_one_byte_public_subscript_mutation_subset`, `cpython_array_one_byte_public_copy_byteswap_compare_subset`, `cpython_array_one_byte_public_concat_repeat_subset`, `cpython_array_inplace_repeat_error_subset`, `cpython_array_one_byte_public_buffer_info_subset`, `cpython_array_one_byte_public_unicode_method_rejection_subset`, and `cpython_array_one_byte_public_file_methods_subset`. | `cpython_array_module_package_metadata_diff_subset`, `cpython_array_module_and_constructor_public_surface_diff_subset`, `cpython_array_subclass_public_construction_diff_subset`, `cpython_array_one_byte_public_sequence_diff_subset`, `cpython_array_short_public_sequence_and_mutation_diff_subset`, `cpython_array_int_public_sequence_and_mutation_diff_subset`, `cpython_array_long_long_public_sequence_and_mutation_diff_subset`, `cpython_array_native_long_public_sequence_and_mutation_diff_subset`, `cpython_array_float_public_sequence_and_mutation_diff_subset`, `cpython_array_unicode_public_sequence_and_mutation_diff_subset`, `cpython_array_one_byte_public_mutation_methods_diff_subset`, `cpython_array_one_byte_public_clear_diff_subset`, `cpython_array_one_byte_public_subscript_mutation_diff_subset`, `cpython_array_one_byte_public_copy_byteswap_compare_diff_subset`, `cpython_array_one_byte_public_concat_repeat_diff_subset`, `cpython_array_inplace_repeat_error_diff_subset`, `cpython_array_one_byte_public_buffer_info_diff_subset`, `cpython_array_one_byte_public_unicode_method_rejection_diff_subset`, and `cpython_array_one_byte_public_file_methods_diff_subset`. | Real file descriptors and C buffer/allocator internals. |
| `copy` | Public module `__all__` exports, copy public function `__module__`, `__qualname__`, `__doc__`, `__type_params__`, `__annotate__`, `__closure__` None, empty `__annotations__`, empty `__dict__`, `__defaults__` None, `deepcopy.__defaults__` shape, and `__kwdefaults__` metadata, public `Error` / `error` exception metadata, `dispatch_table` module attribute shape, immutable scalar equality/identity preservation, pure Python `__copy__` / `__deepcopy__` hook dispatch with recursive memo reuse and TypeError propagation for bad hook signatures, `deepcopy()` memo preservation for shared list/dict/tuple members, explicit memo dict pre-seeding and population for supported identities, self-referential lists, shared user-instance fields, user-instance self-cycles, shared `UserList` / `UserDict` / `deque` members and `UserList` self-cycles, `copy.copy(defaultdict(...))` shallow copies, `io.BytesIO` shallow/deep copy of open in-memory buffers with position and custom-attribute handling, and `copy()`, `deepcopy()`, and `replace()` for supported runtime values, AST/simple namespace use cases, custom `__replace__` hook dispatch through class-level lookup including `staticmethod` / `classmethod` hook forms, unsupported-object `TypeError` messages using `replace() does not support T objects`, class-qualified `__replace__()` unexpected-keyword `TypeError` text for bad custom hook signatures, classmethod `__replace__()` arity `TypeError` text for bad classmethod hook signatures, and staticmethod `__replace__()` arity `TypeError` text for class-defined and external staticmethod hooks. Subset evidence: `cpython_copy_module_package_metadata_subset`, `cpython_copy_module_all_exports_subset`, `cpython_copy_function_module_metadata_subset`, `cpython_copy_function_qualname_metadata_subset`, `cpython_copy_function_doc_metadata_subset`, `cpython_copy_function_type_params_metadata_subset`, `cpython_copy_function_annotate_metadata_subset`, `cpython_copy_function_closure_none_metadata_subset`, `cpython_copy_function_annotations_empty_metadata_subset`, `cpython_copy_function_dict_empty_metadata_subset`, `cpython_copy_function_defaults_none_metadata_subset`, `cpython_copy_function_deepcopy_defaults_shape_metadata_subset`, `cpython_copy_function_kwdefaults_metadata_subset`, `cpython_copy_public_subset`, `cpython_collections_defaultdict_copy_module_subset`, `cpython_copy_replace_custom_hook_subset`, `cpython_copy_replace_unsupported_type_error_subset`, `cpython_copy_replace_hook_unexpected_keyword_error_subset`, `cpython_copy_replace_classmethod_hook_arity_error_subset`, and `cpython_copy_replace_staticmethod_hook_arity_error_subset`. | `cpython_copy_module_package_metadata_diff_subset`, `cpython_copy_module_all_exports_diff_subset`, `cpython_copy_function_module_metadata_diff_subset`, `cpython_copy_function_qualname_metadata_diff_subset`, `cpython_copy_function_doc_metadata_diff_subset`, `cpython_copy_function_type_params_metadata_diff_subset`, `cpython_copy_function_annotate_metadata_diff_subset`, `cpython_copy_function_closure_none_metadata_diff_subset`, `cpython_copy_function_annotations_empty_metadata_diff_subset`, `cpython_copy_function_dict_empty_metadata_diff_subset`, `cpython_copy_function_defaults_none_metadata_diff_subset`, `cpython_copy_function_deepcopy_defaults_shape_metadata_diff_subset`, `cpython_copy_function_kwdefaults_metadata_diff_subset`, `cpython_copy_public_diff_subset`, `cpython_collections_defaultdict_copy_module_diff_subset`, `cpython_copy_replace_custom_hook_diff_subset`, `cpython_copy_replace_unsupported_type_error_diff_subset`, `cpython_copy_replace_hook_unexpected_keyword_error_diff_subset`, `cpython_copy_replace_classmethod_hook_arity_error_diff_subset`, `cpython_copy_replace_staticmethod_hook_arity_error_diff_subset`, `cpython_array_one_byte_public_copy_byteswap_compare_diff_subset`. | Full pickle dispatch-table contents, exact `copy.*.__doc__` CPython text, function `__dict__` mutation/persistence and `__annotations__` mutation/persistence, `copy.deepcopy.__defaults__` tuple/list identity and mutation persistence, function `__builtins__` globals view, pickle protocol byte compatibility, arbitrary extension-object copy hooks outside pure Python hook dispatch, and arbitrary mapping-protocol memo hooks. |
| `io.BytesIO` | In-memory io module `__package__` metadata (`io.__package__` is `''`) plus bytes read/read1/write/writelines/readline/readlines/getvalue/getbuffer/readinto/readinto1/tell/seek/truncate plus io.SEEK_SET/SEEK_CUR/SEEK_END constants behavior used by arrays, memoryview, copy, and byte-oriented tests, including writable `bytearray` / memoryview targets, contiguous memoryview `write()` / `writelines()` and non-contiguous memoryview `BufferError` rejection, `getbuffer()` writable memoryview mutation back into the stream buffer and active-export `BufferError` protection for `write()` / all `truncate()` requests / `close()` across direct and derived memoryviews, release of function-scoped, deleted-binding, and expression-temporary exported views, sequential bytes-like line writes, sparse write NUL filling, non-extending `truncate()` growth requests, post-EOF empty reads/readinto results, position tracking, size/sizehint line reads, line iteration through `__iter__()` / `__next__()` with EOF `StopIteration`, `readable()` / `writable()` / `seekable()` / `isatty()` / `flush()` IOBase query methods, `fileno()` / `detach()` `io.UnsupportedOperation` stubs, type metadata, instance `__dict__` and custom attributes, `BytesIO.__getstate__()` state tuples for untouched and materialized open streams plus closed-stream `ValueError`, `BytesIO.__setstate__()` buffer/position/attribute merge behavior plus active-export `BufferError`, the `io.BytesIO.read` method descriptor, the `io.BytesIO.read1` method descriptor, the `io.BytesIO.readline` method descriptor, the `io.BytesIO.readlines` method descriptor, the `io.BytesIO.write` method descriptor, the `io.BytesIO.writelines` method descriptor, the `io.BytesIO.getvalue` method descriptor, the `io.BytesIO.getbuffer` method descriptor, the `io.BytesIO.tell` method descriptor, the `io.BytesIO.seek` method descriptor, the `io.BytesIO.truncate` method descriptor, the `io.BytesIO.readable` method descriptor, the `io.BytesIO.writable` method descriptor, the `io.BytesIO.seekable` method descriptor, the `io.BytesIO.isatty` method descriptor, the `io.BytesIO.flush` method descriptor, the `io.BytesIO._checkClosed` method descriptor, the `io.BytesIO._checkReadable` method descriptor, the `io.BytesIO.fileno` method descriptor, the `io.BytesIO.detach` method descriptor, the `io.BytesIO.close` method descriptor, the `io.BytesIO.__enter__` method descriptor, the `io.BytesIO.__exit__` method descriptor, the `io.BytesIO.__iter__` wrapper descriptor, the `io.BytesIO.__next__` wrapper descriptor, state method descriptors for `io.BytesIO.__getstate__` / `io.BytesIO.__setstate__`, `copy.copy()` / `copy.deepcopy()` behavior for open in-memory streams, `close()` / `closed` state handling, context-manager `__enter__()` / `__exit__()` lifecycle behavior, supported method visibility through `dir()`, and `None` / bytes-like `initial_bytes=` construction plus CPython-style unknown-keyword constructor diagnostics. Subset evidence: `cpython_io_module_package_metadata_subset`, `cpython_io_bytesio_public_subset`, `cpython_io_bytesio_read_method_descriptor_subset`, `cpython_io_bytesio_read1_method_descriptor_subset`, `cpython_io_bytesio_readline_method_descriptor_subset`, `cpython_io_bytesio_readlines_method_descriptor_subset`, `cpython_io_bytesio_write_method_descriptor_subset`, `cpython_io_bytesio_writelines_method_descriptor_subset`, `cpython_io_bytesio_getvalue_method_descriptor_subset`, `cpython_io_bytesio_getbuffer_method_descriptor_subset`, `cpython_io_bytesio_tell_method_descriptor_subset`, `cpython_io_bytesio_seek_method_descriptor_subset`, `cpython_io_bytesio_truncate_method_descriptor_subset`, `cpython_io_bytesio_readable_method_descriptor_subset`, `cpython_io_bytesio_writable_method_descriptor_subset`, `cpython_io_bytesio_seekable_method_descriptor_subset`, `cpython_io_bytesio_isatty_method_descriptor_subset`, `cpython_io_bytesio_flush_method_descriptor_subset`, `cpython_io_bytesio_check_closed_method_descriptor_subset`, `cpython_io_bytesio_check_readable_method_descriptor_subset`, `cpython_io_bytesio_fileno_method_descriptor_subset`, `cpython_io_bytesio_detach_method_descriptor_subset`, `cpython_io_bytesio_close_method_descriptor_subset`, `cpython_io_bytesio_enter_method_descriptor_subset`, `cpython_io_bytesio_exit_method_descriptor_subset`, `cpython_io_bytesio_iter_wrapper_descriptor_subset`, `cpython_io_bytesio_next_wrapper_descriptor_subset`, `cpython_io_bytesio_getstate_subset`, `cpython_io_bytesio_setstate_subset`, `cpython_io_bytesio_state_method_descriptor_subset`, and `cpython_memoryview_bytesio_readinto_subset`. | `cpython_io_module_package_metadata_diff_subset`, `cpython_io_bytesio_public_diff_subset`, `cpython_io_bytesio_read_method_descriptor_diff_subset`, `cpython_io_bytesio_read1_method_descriptor_diff_subset`, `cpython_io_bytesio_readline_method_descriptor_diff_subset`, `cpython_io_bytesio_readlines_method_descriptor_diff_subset`, `cpython_io_bytesio_write_method_descriptor_diff_subset`, `cpython_io_bytesio_writelines_method_descriptor_diff_subset`, `cpython_io_bytesio_getvalue_method_descriptor_diff_subset`, `cpython_io_bytesio_getbuffer_method_descriptor_diff_subset`, `cpython_io_bytesio_tell_method_descriptor_diff_subset`, `cpython_io_bytesio_seek_method_descriptor_diff_subset`, `cpython_io_bytesio_truncate_method_descriptor_diff_subset`, `cpython_io_bytesio_readable_method_descriptor_diff_subset`, `cpython_io_bytesio_writable_method_descriptor_diff_subset`, `cpython_io_bytesio_seekable_method_descriptor_diff_subset`, `cpython_io_bytesio_isatty_method_descriptor_diff_subset`, `cpython_io_bytesio_flush_method_descriptor_diff_subset`, `cpython_io_bytesio_check_closed_method_descriptor_diff_subset`, `cpython_io_bytesio_check_readable_method_descriptor_diff_subset`, `cpython_io_bytesio_fileno_method_descriptor_diff_subset`, `cpython_io_bytesio_detach_method_descriptor_diff_subset`, `cpython_io_bytesio_close_method_descriptor_diff_subset`, `cpython_io_bytesio_enter_method_descriptor_diff_subset`, `cpython_io_bytesio_exit_method_descriptor_diff_subset`, `cpython_io_bytesio_iter_wrapper_descriptor_diff_subset`, `cpython_io_bytesio_next_wrapper_descriptor_diff_subset`, `cpython_io_bytesio_getstate_diff_subset`, `cpython_io_bytesio_setstate_diff_subset`, `cpython_io_bytesio_state_method_descriptor_diff_subset`, `cpython_memoryview_bytesio_readinto_diff_subset`, `cpython_array_one_byte_public_file_methods_diff_subset`, `cpython_copy_public_diff_subset`. | Real files, buffering layers, text I/O, file descriptors, and OS-backed stream semantics. |
| `operator` | Arithmetic/comparison helpers, sequence/member helpers, attrgetter/itemgetter/methodcaller, selected signature/module/qualname/doc metadata, and helper repr subsets. Subset evidence: `cpython_operator_public_helpers_subset`, `cpython_operator_index_normalization_subset`, `cpython_operator_length_hint_subset`, `cpython_operator_comparison_predicate_subset`, `cpython_operator_is_none_predicates_subset`, `cpython_operator_arithmetic_bitwise_subset`, `cpython_operator_sequence_member_subset`, `cpython_operator_callable_helper_subset`, `cpython_operator_call_helper_subset`, `cpython_operator_inplace_helper_subset`, `cpython_operator_module_metadata_subset`, `cpython_operator_helper_instance_module_metadata_subset`, `cpython_operator_signature_helper_subset`, and `cpython_operator_helper_repr_subset`. | `cpython_operator_public_helpers_diff_subset`, `cpython_operator_index_normalization_diff_subset` is gated for CPython oracles with current int-subclass result normalization, `cpython_operator_length_hint_diff_subset`, `cpython_operator_comparison_predicate_diff_subset`, `cpython_operator_is_none_predicates_diff_subset` is gated for CPython oracles with `operator.is_none`, `cpython_operator_arithmetic_bitwise_diff_subset`, `cpython_operator_sequence_member_diff_subset`, `cpython_operator_callable_helper_diff_subset`, `cpython_operator_call_helper_diff_subset` is gated for CPython oracles with `operator.call`, `cpython_operator_inplace_helper_diff_subset`, `cpython_operator_module_metadata_diff_subset`, `cpython_operator_helper_instance_module_metadata_diff_subset` is gated for CPython oracles with helper instance `__module__`, `cpython_operator_signature_helper_diff_subset`, `cpython_operator_helper_repr_diff_subset`. | Full pickle metadata and every CPython helper edge case until separately migrated. |
| `functools` | `partial`, including nested partial flattening for public `func` / `args` / `keywords`, `partialmethod`, `reduce` including positional-only `function` / `sequence` keyword error text and non-iterable / bad-iterator-return second-argument error text, `cmp_to_key`, wrapper helpers, cache/lru-cache/cached-property, singledispatch, singledispatchmethod, and total_ordering subsets. Subset evidence: `cpython_functools_public_helpers_subset`, `cpython_functools_partial_subset`, `cpython_functools_partial_instance_module_metadata_subset`, `cpython_functools_partialmethod_subset`, `cpython_functools_cmp_to_key_subset`, `cpython_functools_update_wrapper_wraps_subset`, `cpython_functools_total_ordering_subset`, `cpython_functools_cache_subset`, `cpython_functools_lru_cache_subset`, `cpython_functools_lru_cache_exception_subset`, `cpython_functools_lru_cache_keyword_order_subset`, `cpython_functools_lru_cache_argument_shape_subset`, `cpython_functools_cache_wrapper_methods_subset`, `cpython_functools_cache_wrapper_module_metadata_subset`, `cpython_functools_cached_property_subset`, `cpython_functools_cached_property_module_metadata_subset`, `cpython_functools_reduce_subset`, `cpython_functools_reduce_initial_keyword_subset`, `cpython_functools_singledispatch_subset`, and `cpython_functools_singledispatchmethod_subset`. | `cpython_functools_public_helpers_diff_subset`, `cpython_functools_partial_diff_subset`, `cpython_functools_partial_instance_module_metadata_diff_subset` is gated for CPython oracles with partial instance `__module__`, `cpython_functools_reduce_diff_subset`, `cpython_functools_reduce_initial_keyword_diff_subset` is gated for CPython oracles with reduce(initial=...), `cpython_functools_cmp_to_key_diff_subset`, `cpython_functools_update_wrapper_wraps_diff_subset`, `cpython_functools_total_ordering_diff_subset`, `cpython_functools_partialmethod_diff_subset`, `cpython_functools_cached_property_diff_subset`, `cpython_functools_cached_property_module_metadata_diff_subset` is gated for CPython oracles with function-derived cached_property `__module__`, `cpython_functools_cache_diff_subset`, `cpython_functools_lru_cache_diff_subset`, `cpython_functools_lru_cache_exception_diff_subset`, `cpython_functools_lru_cache_keyword_order_diff_subset`, `cpython_functools_lru_cache_argument_shape_diff_subset`, `cpython_functools_cache_wrapper_methods_diff_subset`, `cpython_functools_cache_wrapper_module_metadata_diff_subset` is gated for CPython oracles with cache wrapper `__module__` deletion fallback, `cpython_functools_singledispatch_diff_subset`, `cpython_functools_singledispatchmethod_diff_subset`, `cpython_functools_singledispatch_union_diff_subset` is gated for CPython oracles with union registration support, `cpython_pow_builtin_diff_subset`. | Full CPython cache implementation internals, weakref/lifecycle subtleties, and unsupported descriptor edge cases. |
| `itertools` | `accumulate()` including `initial=None` omission, `count()` with integer/float/complex/bool arithmetic, `cycle()`, `repeat()`, `chain()`, `chain.from_iterable()`, `compress()`, `filterfalse()`, `takewhile()`, `dropwhile()`, `starmap()`, `zip_longest()`, `islice()` including two-argument `stop=None` and bad-`__index__` error classification, `pairwise()`, `product()`, `combinations()`, `combinations_with_replacement()`, `permutations()` including `r=None` default-length behavior, `tee()`, `batched()`, `groupby()` pure iterator behavior, public repr shapes for supported iterator objects without exact address binding, public iterator/helper type `__module__` / `__qualname__` / `__doc__` metadata including `_grouper.__doc__ is None`, and public constructor `__qualname__` / `__module__` / `__doc__` metadata including gated `pairwise()` and `batched()` constructor surfaces. Subset evidence: `cpython_itertools_core_iterator_subset`, `cpython_itertools_chain_subset`, `cpython_itertools_chain_from_iterable_subset`, `cpython_itertools_repeat_subset`, `cpython_itertools_cycle_subset`, `cpython_itertools_compress_subset`, `cpython_itertools_filterfalse_subset`, `cpython_itertools_takewhile_subset`, `cpython_itertools_dropwhile_subset`, `cpython_itertools_starmap_subset`, `cpython_itertools_accumulate_subset`, `cpython_itertools_zip_longest_subset`, `cpython_itertools_islice_subset`, `cpython_itertools_islice_error_subset`, `cpython_itertools_count_subset`, `cpython_itertools_count_error_subset`, `cpython_itertools_count_bool_arithmetic_subset`, `cpython_itertools_keyword_error_subset`, `cpython_itertools_pairwise_subset`, `cpython_itertools_product_subset`, `cpython_itertools_combinations_subset`, `cpython_itertools_combinations_with_replacement_subset`, `cpython_itertools_permutations_subset`, `cpython_itertools_tee_subset`, `cpython_itertools_batched_subset`, `cpython_itertools_groupby_subset`, and `cpython_itertools_repr_subset`. | `cpython_itertools_core_diff_subset`, `cpython_itertools_core_iterator_diff_subset`, `cpython_itertools_chain_diff_subset`, `cpython_itertools_chain_from_iterable_diff_subset`, `cpython_itertools_repeat_diff_subset`, `cpython_itertools_cycle_diff_subset`, `cpython_itertools_compress_diff_subset`, `cpython_itertools_filterfalse_diff_subset`, `cpython_itertools_takewhile_diff_subset`, `cpython_itertools_dropwhile_diff_subset`, `cpython_itertools_starmap_diff_subset`, `cpython_itertools_accumulate_diff_subset`, `cpython_itertools_zip_longest_diff_subset`, `cpython_itertools_islice_diff_subset`, `cpython_itertools_islice_error_diff_subset`, `cpython_itertools_count_diff_subset`, `cpython_itertools_count_error_diff_subset`, `cpython_itertools_count_bool_arithmetic_diff_subset`, `cpython_itertools_keyword_error_diff_subset`; `cpython_itertools_pairwise_diff_subset` is gated for CPython 3.10+ oracles; `cpython_itertools_product_diff_subset`; `cpython_itertools_combinations_diff_subset`; `cpython_itertools_combinations_with_replacement_diff_subset`; `cpython_itertools_permutations_diff_subset`; `cpython_itertools_tee_diff_subset`; `cpython_itertools_batched_diff_subset`; `cpython_itertools_groupby_diff_subset`; `cpython_itertools_repr_diff_subset`. | Full itertools module, pickling exactness, exact address repr, `tee()` cache compaction, and remaining public/helper types. |
| `json` | Pure in-memory `loads()` from `str` / `bytes` / `bytearray` values and subclasses, including UTF-8 BOM, UTF-16/UTF-32 encoded byte input, and short UTF-16 byte input, CPython-style string-input UTF-8 BOM rejection text, `loads(s=...)` keyword binding, CPython no-op `None` binding for optional `loads()` hook keywords, supported `parse_int` / `parse_float` / `parse_constant` hooks including arbitrary returned values and CPython-style non-callable hook `TypeError` text, `object_hook` post-processing for decoded objects including arbitrary returned values and non-callable hook `TypeError` text, and `object_pairs_hook` pair-list post-processing including arbitrary returned values and non-callable hook `TypeError` text, unsupported `loads()` `cls`, `loads()` / `dumps()` public function `__name__` / `__qualname__` / `__module__` / `__doc__` / `__type_params__` / `__annotate__` / `__closure__` / `__builtins__` / `__globals__` / `__dict__` / `__defaults__` / `__kwdefaults__` / `__annotations__` metadata plus `repr()` / `str()`, `__repr__` / `__str__`, `__call__`, `__dir__`, `__eq__` / `__ne__`, `__lt__` / `__le__` / `__gt__` / `__ge__`, `__format__`, `__hash__`, `__getattribute__`, `__setattr__`, `__delattr__`, `__init__`, `__init_subclass__`, and `__subclasshook__` wrappers, controlled `__doc__` / `__module__` assignment and deletion, controlled `__name__` / `__qualname__` assignment and deletion errors, controlled `__class__` assignment and deletion errors, controlled `__closure__` assignment and deletion errors, controlled `__builtins__` assignment and deletion errors, controlled `__globals__` assignment and deletion errors, controlled `__type_params__` assignment and deletion errors, controlled `__annotate__` assignment and deletion errors, controlled `__dict__` assignment, controlled `__annotations__` assignment and deletion, controlled `__defaults__` assignment and deletion, controlled `__kwdefaults__` assignment and deletion, ordinary `\uXXXX` escapes and valid surrogate-pair Unicode escapes, CPython default non-finite constants, `strict=False` raw control-character string parsing, invalid UTF-8 byte input `UnicodeDecodeError` type classification, duplicate-object-key last-value behavior, JSON whitespace, integer/float number grammar edges including leading-zero extra-data classification, `sys.set_int_max_str_digits()` enforcement for default parsed integer values, top-level scalars and empty containers, plus `dumps()` data model subset for common JSON values, `dumps(obj=...)` keyword binding, CPython no-op `None` binding for unsupported `dumps()` `cls` keyword and supported `default` hook fallback, `str` / `int` / `float` subclass, `IntEnum` values/keys, and `enum.StrEnum` values/keys, list/tuple containers including 200-level nested list encoding, non-empty dict subclass containers through public `items()` with CPython's 2-tuple item requirement and non-iterable `items()` result TypeError text, CPython's empty-dict-subclass `{}` fast path, `Counter` and Counter subclass mapping containers, and namedtuple containers, standard strings/escapes, `allow_nan` rejection of non-finite floats, `check_circular` cycle-error behavior, `ensure_ascii` string/key rendering, `indent` pretty-print formatting for int/string and `__index__` indent values, `skipkeys` omission of unsupported dict keys, `sort_keys` ordering for supported comparable keys, `separators` compact/custom rendering for two-string list/tuple values, subclasses, and general iterables plus CPython-style unpack length `ValueError` text and item/key separator element `TypeError` text, `default` hook handling for otherwise unsupported objects including CPython-style non-callable hook `TypeError` text, returned-self and returned-container circular detection, shared unsupported slice replacement circular detection, shared unsupported SimpleNamespace replacement circular detection, shared unsupported staticmethod replacement circular detection, shared unsupported classmethod replacement circular detection, shared unsupported property replacement circular detection, shared unsupported slot member_descriptor replacement circular detection, shared unsupported namedtuple field descriptor replacement circular detection, shared unsupported dict view replacement circular detection, shared unsupported exact-dict mappingproxy replacement circular detection, shared unsupported custom mappingproxy replacement circular detection, shared unsupported collections.abc KeysView replacement circular detection, shared unsupported io.BytesIO replacement circular detection, shared unsupported weakref replacement circular detection, shared unsupported deque replacement circular detection, shared unsupported UserList replacement circular detection, shared unsupported UserDict replacement circular detection, shared unsupported bytes-like replacement circular detection, shared unsupported functools.singledispatch replacement circular detection, shared unsupported functools.singledispatch register decorator replacement circular detection, shared unsupported functools.singledispatchmethod replacement circular detection, shared unsupported functools.singledispatchmethod bound callable replacement circular detection, shared unsupported operator.attrgetter replacement circular detection, shared unsupported operator.itemgetter replacement circular detection, shared unsupported operator.methodcaller replacement circular detection, shared unsupported functools.cached_property replacement circular detection, shared unsupported functools.cmp_to_key callable replacement circular detection, shared unsupported functools.cmp_to_key key-object replacement circular detection, shared unsupported frame-locals proxy replacement circular detection, `check_circular=False` default-hook recursion classification, and fresh unsupported replacement recursion as a Python-level `RecursionError`, `separators=None` preserving indent default item-separator behavior, finite and default non-finite float spelling, bool/null, lists/tuples/dicts, basic dict-key coercion, circular-reference rejection, and first-pass type/structural/literal/data error classification. Subset evidence: `cpython_json_module_package_metadata_subset`, `cpython_json_function_module_identity_metadata_subset`, `cpython_json_function_name_qualname_identity_metadata_subset`, `cpython_json_function_name_assignment_metadata_subset`, `cpython_json_function_qualname_assignment_metadata_subset`, `cpython_json_function_bound_method_name_dir_visibility_subset`, `cpython_json_function_bound_method_class_dir_visibility_subset`, `cpython_json_function_type_class_metadata_subset`, `cpython_json_function_class_assignment_metadata_subset`, `cpython_json_function_repr_str_wrapper_metadata_subset`, `cpython_json_function_call_wrapper_metadata_subset`, `cpython_json_function_init_wrapper_metadata_subset`, `cpython_json_function_init_subclass_wrapper_metadata_subset`, `cpython_json_function_subclasshook_wrapper_metadata_subset`, `cpython_json_function_format_wrapper_metadata_subset`, `cpython_json_function_hash_wrapper_metadata_subset`, `cpython_json_function_rich_compare_wrapper_metadata_subset`, `cpython_json_function_order_wrapper_metadata_subset`, `cpython_json_function_getattribute_wrapper_metadata_subset`, `cpython_json_function_setattr_delattr_wrapper_metadata_subset`, `cpython_json_function_doc_module_assignment_metadata_subset`, `cpython_json_function_dict_assignment_metadata_subset`, `cpython_json_function_type_params_metadata_subset`, `cpython_json_function_type_params_assignment_metadata_subset`, `cpython_json_function_annotate_metadata_subset`, `cpython_json_function_annotate_assignment_metadata_subset`, `cpython_json_function_closure_none_metadata_subset`, `cpython_json_function_closure_assignment_metadata_subset`, `cpython_json_function_builtins_metadata_subset`, `cpython_json_function_builtins_identity_metadata_subset`, `cpython_json_function_builtins_assignment_metadata_subset`, `cpython_json_function_globals_metadata_subset`, `cpython_json_function_globals_identity_metadata_subset`, `cpython_json_function_globals_assignment_metadata_subset`, `cpython_json_function_doc_identity_metadata_subset`, `cpython_json_function_dict_identity_metadata_subset`, `cpython_json_function_annotations_identity_metadata_subset`, `cpython_json_function_annotations_assignment_metadata_subset`, `cpython_json_function_defaults_assignment_metadata_subset`, `cpython_json_function_kwdefaults_assignment_metadata_subset`, `cpython_json_function_kwdefaults_identity_metadata_subset`, `cpython_json_function_dir_metadata_subset`, `cpython_json_function_get_descriptor_metadata_subset`, `cpython_json_function_get_missing_owner_error_subset`, `cpython_json_function_bound_method_repr_subset`, `cpython_json_function_bound_method_repr_str_wrapper_subset`, `cpython_json_function_bound_method_call_wrapper_subset`, `cpython_json_function_bound_method_get_wrapper_subset`, `cpython_json_function_bound_method_dir_wrapper_subset`, `cpython_json_function_bound_method_rich_compare_wrapper_subset`, `cpython_json_function_bound_method_rich_compare_operator_subset`, `cpython_json_function_bound_method_order_wrapper_subset`, `cpython_json_function_bound_method_format_wrapper_subset`, `cpython_json_function_bound_method_hash_wrapper_subset`, `cpython_json_function_bound_method_getattribute_wrapper_subset`, `cpython_json_function_bound_method_getattribute_missing_attr_subset`, `cpython_json_function_bound_method_doc_dir_metadata_subset`, `cpython_json_function_bound_method_text_signature_missing_attr_subset`, `cpython_json_function_bound_method_annotate_metadata_subset`, `cpython_json_function_bound_method_type_params_metadata_subset`, `cpython_json_function_bound_method_closure_none_metadata_subset`, `cpython_json_function_bound_method_defaults_metadata_subset`, `cpython_json_function_bound_method_kwdefaults_metadata_subset`, `cpython_json_function_bound_method_annotations_metadata_subset`, `cpython_json_function_bound_method_dict_metadata_subset`, `cpython_json_function_bound_method_globals_metadata_subset`, `cpython_json_function_bound_method_builtins_metadata_subset`, `cpython_json_loads_dumps_basic_subset`, `cpython_json_keyword_argument_binding_subset`, `cpython_json_loads_escape_and_duplicate_key_subset`, `cpython_json_loads_unicode_escape_roundtrip_subset`, `cpython_json_loads_strict_subset`, `cpython_json_option_truthiness_subset`, `cpython_json_dumps_string_escape_subset`, `cpython_json_dumps_key_coercion_subset`, `cpython_json_dumps_strenum_subset`, `cpython_json_dumps_strenum_member_type_error_subset`, `cpython_json_dumps_ordered_dict_subset`, `cpython_json_dumps_counter_subclass_subset`, `cpython_json_dumps_sequence_subclass_iter_subset`, `cpython_json_dumps_allow_nan_subset`, `cpython_json_dumps_check_circular_subset`, `cpython_json_dumps_ensure_ascii_subset`, `cpython_json_dumps_indent_subset`, `cpython_json_dumps_skipkeys_subset`, `cpython_json_dumps_sort_keys_subset`, `cpython_json_dumps_separators_subset`, `cpython_json_dumps_float_spelling_subset`, `cpython_json_dumps_default_hook_subset`, `cpython_json_dumps_default_hook_slice_identity_subset`, `cpython_json_dumps_default_hook_simplenamespace_identity_subset`, `cpython_json_dumps_default_hook_range_identity_subset`, `cpython_json_dumps_default_hook_complex_identity_subset`, `cpython_json_dumps_default_hook_function_identity_subset`, `cpython_json_dumps_default_hook_types_coroutine_function_identity_subset`, `cpython_json_dumps_default_hook_staticmethod_identity_subset`, `cpython_json_dumps_default_hook_classmethod_identity_subset`, `cpython_json_dumps_default_hook_property_identity_subset`, `cpython_json_dumps_default_hook_member_descriptor_identity_subset`, `cpython_json_dumps_default_hook_namedtuple_field_descriptor_identity_subset`, `cpython_json_dumps_default_hook_dict_view_identity_subset`, `cpython_json_dumps_default_hook_deque_identity_subset`, `cpython_json_dumps_default_hook_userlist_identity_subset`, `cpython_json_dumps_default_hook_userdict_identity_subset`, `cpython_json_dumps_default_hook_bound_method_identity_subset`, `cpython_json_dumps_default_hook_super_identity_subset`, `cpython_json_dumps_default_hook_type_identity_subset`, `cpython_json_dumps_default_hook_exception_identity_subset`, `cpython_json_dumps_default_hook_code_identity_subset`, `cpython_json_dumps_default_hook_cell_identity_subset`, `cpython_json_dumps_default_hook_traceback_identity_subset`, `cpython_json_dumps_default_hook_frame_identity_subset`, `cpython_json_dumps_default_hook_frame_locals_proxy_identity_subset`, `cpython_json_dumps_default_hook_coroutine_identity_subset`, `cpython_json_dumps_default_hook_coroutine_wrapper_identity_subset`, `cpython_json_dumps_default_hook_generator_identity_subset`, `cpython_json_dumps_default_hook_async_generator_identity_subset`, `cpython_json_dumps_default_hook_async_generator_asend_identity_subset`, `cpython_json_dumps_default_hook_list_iterator_identity_subset`, `cpython_json_dumps_default_hook_mappingproxy_identity_subset`, `cpython_json_dumps_default_hook_custom_mappingproxy_identity_subset`, `cpython_json_dumps_default_hook_mappingview_identity_subset`, `cpython_json_dumps_default_hook_bytesio_identity_subset`, `cpython_json_dumps_default_hook_weakref_identity_subset`, `cpython_json_dumps_default_hook_partial_identity_subset`, `cpython_json_dumps_default_hook_partialmethod_identity_subset`, `cpython_json_dumps_default_hook_partialmethod_bound_identity_subset`, `cpython_json_dumps_default_hook_lru_cache_identity_subset`, `cpython_json_dumps_default_hook_singledispatch_identity_subset`, `cpython_json_dumps_default_hook_singledispatch_register_identity_subset`, `cpython_json_dumps_default_hook_singledispatchmethod_identity_subset`, `cpython_json_dumps_default_hook_singledispatchmethod_bound_identity_subset`, `cpython_json_dumps_default_hook_attrgetter_identity_subset`, `cpython_json_dumps_default_hook_itemgetter_identity_subset`, `cpython_json_dumps_default_hook_methodcaller_identity_subset`, `cpython_json_dumps_default_hook_cached_property_identity_subset`, `cpython_json_dumps_default_hook_cmp_to_key_identity_subset`, `cpython_json_dumps_default_hook_cmp_to_key_object_identity_subset`, `cpython_json_loads_number_and_whitespace_subset`, `cpython_json_loads_int_digit_limit_subset`, `cpython_json_loads_top_level_scalar_and_empty_container_subset`, `cpython_json_loads_nonfinite_constants_subset`, `cpython_json_loads_parse_hooks_subset`, `cpython_json_loads_object_hook_subset`, `cpython_json_loads_object_pairs_hook_subset`, `cpython_json_loads_dumps_error_boundary_subset`, and `cpython_json_loads_string_error_boundary_subset`. | `cpython_json_loads_dumps_diff_subset`, `cpython_json_loads_dumps_basic_diff_subset`, `cpython_json_module_package_metadata_diff_subset`, `cpython_json_function_module_identity_metadata_diff_subset`, `cpython_json_function_name_qualname_identity_metadata_diff_subset`, `cpython_json_function_name_assignment_metadata_diff_subset`, `cpython_json_function_qualname_assignment_metadata_diff_subset`, `cpython_json_function_bound_method_name_dir_visibility_diff_subset`, `cpython_json_function_bound_method_class_dir_visibility_diff_subset`, `cpython_json_function_type_class_metadata_diff_subset`, `cpython_json_function_class_assignment_metadata_diff_subset`, `cpython_json_function_repr_str_wrapper_metadata_diff_subset`, `cpython_json_function_call_wrapper_metadata_diff_subset`, `cpython_json_function_init_wrapper_metadata_diff_subset`, `cpython_json_function_init_subclass_wrapper_metadata_diff_subset`, `cpython_json_function_subclasshook_wrapper_metadata_diff_subset`, `cpython_json_function_format_wrapper_metadata_diff_subset`, `cpython_json_function_hash_wrapper_metadata_diff_subset`, `cpython_json_function_rich_compare_wrapper_metadata_diff_subset`, `cpython_json_function_order_wrapper_metadata_diff_subset`, `cpython_json_function_getattribute_wrapper_metadata_diff_subset`, `cpython_json_function_setattr_delattr_wrapper_metadata_diff_subset`, `cpython_json_function_doc_module_assignment_metadata_diff_subset`, `cpython_json_function_dict_assignment_metadata_diff_subset`, `cpython_json_function_type_params_metadata_diff_subset`, `cpython_json_function_type_params_assignment_metadata_diff_subset`, `cpython_json_function_annotate_metadata_diff_subset`, `cpython_json_function_annotate_assignment_metadata_diff_subset`, `cpython_json_function_closure_none_metadata_diff_subset`, `cpython_json_function_closure_assignment_metadata_diff_subset`, `cpython_json_function_builtins_metadata_diff_subset`, `cpython_json_function_builtins_identity_metadata_diff_subset`, `cpython_json_function_builtins_assignment_metadata_diff_subset`, `cpython_json_function_globals_metadata_diff_subset`, `cpython_json_function_globals_identity_metadata_diff_subset`, `cpython_json_function_globals_assignment_metadata_diff_subset`, `cpython_json_function_doc_identity_metadata_diff_subset`, `cpython_json_function_dict_identity_metadata_diff_subset`, `cpython_json_function_annotations_identity_metadata_diff_subset`, `cpython_json_function_annotations_assignment_metadata_diff_subset`, `cpython_json_function_defaults_assignment_metadata_diff_subset`, `cpython_json_function_kwdefaults_assignment_metadata_diff_subset`, `cpython_json_function_kwdefaults_identity_metadata_diff_subset`, `cpython_json_function_dir_metadata_diff_subset`, `cpython_json_function_get_descriptor_metadata_diff_subset`, `cpython_json_function_get_missing_owner_error_diff_subset`, `cpython_json_function_bound_method_repr_diff_subset`, `cpython_json_function_bound_method_repr_str_wrapper_diff_subset`, `cpython_json_function_bound_method_call_wrapper_diff_subset`, `cpython_json_function_bound_method_get_wrapper_diff_subset`, `cpython_json_function_bound_method_dir_wrapper_diff_subset`, `cpython_json_function_bound_method_rich_compare_wrapper_diff_subset`, `cpython_json_function_bound_method_rich_compare_operator_diff_subset`, `cpython_json_function_bound_method_order_wrapper_diff_subset`, `cpython_json_function_bound_method_format_wrapper_diff_subset`, `cpython_json_function_bound_method_hash_wrapper_diff_subset`, `cpython_json_function_bound_method_getattribute_wrapper_diff_subset`, `cpython_json_function_bound_method_getattribute_missing_attr_diff_subset`, `cpython_json_function_bound_method_doc_dir_metadata_diff_subset`, `cpython_json_function_bound_method_text_signature_missing_attr_diff_subset`, `cpython_json_function_bound_method_annotate_metadata_diff_subset`, `cpython_json_function_bound_method_type_params_metadata_diff_subset`, `cpython_json_function_bound_method_closure_none_metadata_diff_subset`, `cpython_json_function_bound_method_defaults_metadata_diff_subset`, `cpython_json_function_bound_method_kwdefaults_metadata_diff_subset`, `cpython_json_function_bound_method_annotations_metadata_diff_subset`, `cpython_json_function_bound_method_dict_metadata_diff_subset`, `cpython_json_function_bound_method_globals_metadata_diff_subset`, `cpython_json_function_bound_method_builtins_metadata_diff_subset`, `cpython_json_keyword_argument_binding_diff_subset`, `cpython_json_loads_escape_and_duplicate_key_diff_subset`, `cpython_json_loads_unicode_escape_roundtrip_diff_subset`, `cpython_json_loads_strict_diff_subset`, `cpython_json_option_truthiness_diff_subset`, `cpython_json_dumps_string_escape_diff_subset`, `cpython_json_dumps_key_coercion_diff_subset`, `cpython_json_dumps_strenum_diff_subset`, `cpython_json_dumps_strenum_member_type_error_diff_subset`, `cpython_json_dumps_ordered_dict_diff_subset`, `cpython_json_dumps_counter_subclass_diff_subset`, `cpython_json_dumps_sequence_subclass_iter_diff_subset`, `cpython_json_dumps_allow_nan_diff_subset`, `cpython_json_dumps_check_circular_diff_subset`, `cpython_json_dumps_ensure_ascii_diff_subset`, `cpython_json_dumps_indent_diff_subset`, `cpython_json_dumps_skipkeys_diff_subset`, `cpython_json_dumps_sort_keys_diff_subset`, `cpython_json_dumps_separators_diff_subset`, `cpython_json_dumps_float_spelling_diff_subset`, `cpython_json_dumps_default_hook_diff_subset`, `cpython_json_dumps_default_hook_slice_identity_diff_subset`, `cpython_json_dumps_default_hook_simplenamespace_identity_diff_subset`, `cpython_json_dumps_default_hook_range_identity_diff_subset`, `cpython_json_dumps_default_hook_complex_identity_diff_subset`, `cpython_json_dumps_default_hook_function_identity_diff_subset`, `cpython_json_dumps_default_hook_types_coroutine_function_identity_diff_subset`, `cpython_json_dumps_default_hook_staticmethod_identity_diff_subset`, `cpython_json_dumps_default_hook_classmethod_identity_diff_subset`, `cpython_json_dumps_default_hook_property_identity_diff_subset`, `cpython_json_dumps_default_hook_member_descriptor_identity_diff_subset`, `cpython_json_dumps_default_hook_namedtuple_field_descriptor_identity_diff_subset`, `cpython_json_dumps_default_hook_dict_view_identity_diff_subset`, `cpython_json_dumps_default_hook_deque_identity_diff_subset`, `cpython_json_dumps_default_hook_userlist_identity_diff_subset`, `cpython_json_dumps_default_hook_userdict_identity_diff_subset`, `cpython_json_dumps_default_hook_bound_method_identity_diff_subset`, `cpython_json_dumps_default_hook_super_identity_diff_subset`, `cpython_json_dumps_default_hook_type_identity_diff_subset`, `cpython_json_dumps_default_hook_exception_identity_diff_subset`, `cpython_json_dumps_default_hook_code_identity_diff_subset`, `cpython_json_dumps_default_hook_cell_identity_diff_subset`, `cpython_json_dumps_default_hook_traceback_identity_diff_subset`, `cpython_json_dumps_default_hook_frame_identity_diff_subset`, `cpython_json_dumps_default_hook_frame_locals_proxy_identity_diff_subset`, `cpython_json_dumps_default_hook_coroutine_identity_diff_subset`, `cpython_json_dumps_default_hook_coroutine_wrapper_identity_diff_subset`, `cpython_json_dumps_default_hook_generator_identity_diff_subset`, `cpython_json_dumps_default_hook_async_generator_identity_diff_subset`, `cpython_json_dumps_default_hook_async_generator_asend_identity_diff_subset`, `cpython_json_dumps_default_hook_list_iterator_identity_diff_subset`, `cpython_json_dumps_default_hook_mappingproxy_identity_diff_subset`, `cpython_json_dumps_default_hook_custom_mappingproxy_identity_diff_subset`, `cpython_json_dumps_default_hook_mappingview_identity_diff_subset`, `cpython_json_dumps_default_hook_bytesio_identity_diff_subset`, `cpython_json_dumps_default_hook_weakref_identity_diff_subset`, `cpython_json_dumps_default_hook_partial_identity_diff_subset`, `cpython_json_dumps_default_hook_partialmethod_identity_diff_subset`, `cpython_json_dumps_default_hook_partialmethod_bound_identity_diff_subset`, `cpython_json_dumps_default_hook_lru_cache_identity_diff_subset`, `cpython_json_dumps_default_hook_singledispatch_identity_diff_subset`, `cpython_json_dumps_default_hook_singledispatch_register_identity_diff_subset`, `cpython_json_dumps_default_hook_singledispatchmethod_identity_diff_subset`, `cpython_json_dumps_default_hook_singledispatchmethod_bound_identity_diff_subset`, `cpython_json_dumps_default_hook_attrgetter_identity_diff_subset`, `cpython_json_dumps_default_hook_itemgetter_identity_diff_subset`, `cpython_json_dumps_default_hook_methodcaller_identity_diff_subset`, `cpython_json_dumps_default_hook_cached_property_identity_diff_subset`, `cpython_json_dumps_default_hook_cmp_to_key_identity_diff_subset`, `cpython_json_dumps_default_hook_cmp_to_key_object_identity_diff_subset`, `cpython_json_loads_number_and_whitespace_diff_subset`, `cpython_json_loads_int_digit_limit_diff_subset`, `cpython_json_loads_top_level_scalar_and_empty_container_diff_subset`, `cpython_json_loads_nonfinite_constants_diff_subset`, `cpython_json_loads_parse_hooks_diff_subset`, `cpython_json_loads_object_hook_diff_subset`, `cpython_json_loads_object_pairs_hook_diff_subset`, `cpython_json_loads_dumps_error_boundary_diff_subset`, `cpython_json_loads_string_error_boundary_diff_subset`. | File APIs, module `__all__` exports beyond the sandbox `loads` / `dumps` surface, non-`None` encoder/decoder hooks other than `object_hook` / `object_pairs_hook` / `parse_int` / `parse_float` / `parse_constant` / `default`, `loads()` hooks/options other than `strict` / `object_hook` / `object_pairs_hook`, `dumps()` hooks/options other than `allow_nan` / `check_circular` / `ensure_ascii` / `indent` / `skipkeys` / `sort_keys` / `separators` / `default` such as non-`None` `cls`, bytes/bytearray serialization, unpaired surrogate storage, and full `JSONDecodeError` compatibility. |

`cpython_io_bytesio_read_method_descriptor_subset`, backed by
`cpython_io_bytesio_read_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_read1_method_descriptor_subset`, backed by
`cpython_io_bytesio_read1_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_readline_method_descriptor_subset`, backed by
`cpython_io_bytesio_readline_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_readlines_method_descriptor_subset`, backed by
`cpython_io_bytesio_readlines_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_write_method_descriptor_subset`, backed by
`cpython_io_bytesio_write_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_writelines_method_descriptor_subset`, backed by
`cpython_io_bytesio_writelines_method_descriptor_diff_subset`, pins the
supported `io.BytesIO.writelines` method descriptor, including descriptor type
visibility, multi-line bytes-like descriptor writes for list and tuple inputs,
wrong-receiver descriptor `TypeError`, missing-receiver keyword behavior,
exact arity / keyword diagnostics, non-iterable input rejection, and bytes-like
item rejection.

`cpython_io_bytesio_getvalue_method_descriptor_subset`, backed by
`cpython_io_bytesio_getvalue_method_descriptor_diff_subset`, pins the supported
...
missing-receiver keyword behavior.

`cpython_io_bytesio_getbuffer_method_descriptor_subset`, backed by
`cpython_io_bytesio_getbuffer_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_tell_method_descriptor_subset`, backed by
`cpython_io_bytesio_tell_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_seek_method_descriptor_subset`, backed by
`cpython_io_bytesio_seek_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_truncate_method_descriptor_subset`, backed by
`cpython_io_bytesio_truncate_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_readable_method_descriptor_subset`, backed by
`cpython_io_bytesio_readable_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_writable_method_descriptor_subset`, backed by
`cpython_io_bytesio_writable_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_seekable_method_descriptor_subset`, backed by
`cpython_io_bytesio_seekable_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_isatty_method_descriptor_subset`, backed by
`cpython_io_bytesio_isatty_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_flush_method_descriptor_subset`, backed by
`cpython_io_bytesio_flush_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_check_closed_method_descriptor_subset`, backed by
`cpython_io_bytesio_check_closed_method_descriptor_diff_subset`, pins the
supported `io.BytesIO._checkClosed` method descriptor, including descriptor type
visibility, closed-state guard descriptor calls for open-stream no-op bound and
type-level calls plus closed-stream `ValueError`, `_io._IOBase` wrong-receiver
descriptor `TypeError`, exact extra-argument / keyword diagnostics, and `dir()`
visibility.

`cpython_io_bytesio_check_readable_method_descriptor_subset`, backed by
`cpython_io_bytesio_check_readable_method_descriptor_diff_subset`, pins the
supported `io.BytesIO._checkReadable` method descriptor, including descriptor
type visibility, readability guard descriptor calls for open-stream bound and
...
`cpython_io_bytesio_fileno_method_descriptor_subset`, backed by
`cpython_io_bytesio_fileno_method_descriptor_diff_subset`, pins the supported
...
`TypeError`, and missing-receiver keyword behavior.

`cpython_io_bytesio_detach_method_descriptor_subset`, backed by
`cpython_io_bytesio_detach_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_close_method_descriptor_subset`, backed by
`cpython_io_bytesio_close_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_enter_method_descriptor_subset`, backed by
`cpython_io_bytesio_enter_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_exit_method_descriptor_subset`, backed by
`cpython_io_bytesio_exit_method_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_iter_wrapper_descriptor_subset`, backed by
`cpython_io_bytesio_iter_wrapper_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_next_wrapper_descriptor_subset`, backed by
`cpython_io_bytesio_next_wrapper_descriptor_diff_subset`, pins the supported
...
`cpython_io_bytesio_state_method_descriptor_subset`, backed by
`cpython_io_bytesio_state_method_descriptor_diff_subset`, pins the supported
...
`cpython_json_dumps_default_hook_weakref_identity_subset`, backed by
`cpython_json_dumps_default_hook_weakref_identity_diff_subset`, keeps shared unsupported weakref replacement circular detection without treating fresh callback-bound weakrefs or proxies as circular.
...
plus `enum.StrEnum` values and keys,
...
`cpython_types_class_creation_init_subclass_keyword_error_subset`, backed by
`cpython_types_class_creation_init_subclass_keyword_error_diff_subset`, keeps
class-statement keyword errors that fall through to `object.__init_subclass__`
tied to CPython's public owner text, including
`C.__init_subclass__() takes no keyword arguments`, without expanding into
CPython object-layout internals.

`cpython_types_class_creation_init_subclass_return_value_subset`, backed by
`cpython_types_class_creation_init_subclass_return_value_diff_subset`, keeps
non-`None` `__init_subclass__` hook return values tied to CPython's public
behavior: once the hook call succeeds, the returned value is ignored and the
class is created.
...
keeps unexpected class-statement keywords passed to user-defined
`__init_subclass__` hooks tied to CPython's public defining-owner text,
including inherited hooks that report
`Base.__init_subclass__() got an unexpected keyword argument 'flag'`, without
expanding into CPython object-layout internals.

`cpython_types_class_creation_init_subclass_missing_required_owner_subset`,
backed by
`cpython_types_class_creation_init_subclass_missing_required_owner_diff_subset`,
keeps missing required arguments in user-defined `__init_subclass__` hooks tied
to CPython's public defining-owner text and positional-argument wording,
including inherited hooks that report
`Base.__init_subclass__() missing 1 required positional argument: 'flag'`,
without expanding into CPython object-layout internals.

`cpython_types_class_creation_init_subclass_bound_classmethod_subset`, backed by
`cpython_types_class_creation_init_subclass_bound_classmethod_diff_subset`,
keeps plain functions named `__init_subclass__` tied to CPython's implicit
classmethod binding: `Base.__init_subclass__` exposes a bound method with
`__self__ is Base`, and extra positional calls report
`Base.__init_subclass__() takes 1 positional argument but 2 were given`, without
expanding into CPython descriptor internals.
...
CPython's malformed `items()` result errors,
without adding arbitrary mapping-protocol serialization or custom encoder
classes.
...
truthiness dispatch for `loads(strict=...)` and the supported `dumps()`
boolean options, including propagated `__bool__` exceptions.
...
| Types module | `Lib/test/test_types.py::TypesTests` is now classified as `ported_public`: all portable public methods have Rust evidence, while the remaining locale, C-extension descriptor-crash, and CPython-internal object-layout/capsule/specialization rows are explicit sandbox stop-lines. | Keep extending `types` only through public pure-memory behavior with direct `cpython_diff` coverage; do not promote locale, C-extension, or CPython-internal rows into the default sandbox contract. |
...
| Completion status | The strict CPython test manifest still has `partial`, `blocked_by_runtime`, `blocked_by_ast_module`, and `blocked_by_cpython_internal` groups; full method-level parity has not been proven, and some rows need runtime or standard-library behavior MiniPython does not yet implement while CPython-internal rows are deliberately outside the sandbox product contract. | Keep those unfinished rows visible until each row is either ported, intentionally blocked by the sandbox stop line, classified as CPython-internal rather than public behavior, or removed by a deliberate scope change. |
...
- Added `cpython_compile_specifics_compile_filename_diff_subset` and tightened
  `cpython_compile_specifics_compile_filename_subset` to CPython's public
...
- Added `cpython_memoryview_getstate_subset`, backed by
  `cpython_memoryview_getstate_diff_subset`, for inherited
  `object.__getstate__` on memoryview instances and sliced views through
  `memoryview(b'abc').__getstate__()` and
...
- Added `cpython_array_module_and_constructor_public_surface_subset` and
  ...
  storage tests: `array.typecodes`, legacy typecode construction, str-subclass
  typecode arguments, invalid typecode `ValueError`, non-string and wrong-arity
  constructor `TypeError`s, string-length typecode diagnostics, keyword
  rejection, `extend()` positional argument diagnostics, and zero-length array
  self-slice / concat / repeat behavior. The runtime now rejects bad typecodes
  directly instead of producing a half-initialized array object.
- Added `cpython_array_module_package_metadata_subset` and
...
- Added `cpython_array_one_byte_public_mutation_methods_subset`,
  `cpython_array_one_byte_public_mutation_methods_diff_subset`, and the focused
...
- Added `cpython_array_one_byte_public_concat_repeat_subset` and
  ...
  between operator/augmented and direct-dunder non-integer diagnostics, and
  identity-preserving in-place concat/repeat.
- Added `cpython_array_inplace_repeat_error_subset` and
  `cpython_array_inplace_repeat_error_diff_subset`, pinning the exact CPython
  split between augmented/operator `array` in-place repeat TypeError text and
  direct `array.__imul__()` TypeError text for non-index repeat counts.
...
- Added `cpython_memoryview_cast_native_formats_subset`, backed by
  ...
  formats, CPython-style structure mismatch rejection for signed/unsigned or
  float-size mismatches, CPython-style non-byte-to-non-byte cast rejection, and
  `u` / `w`, doubled-`@`, and non-native-prefixed destination-format rejection.
...
- Added `cpython_operator_length_hint_subset`, migrating the public behavior
  ...
  defaults, preserves CPython non-integer default TypeError text, rejects non-integer and
  negative hint results while preserving CPython non-integer TypeError text, and re-reads sequence lengths
...
- Added `cpython_operator_comparison_predicate_subset`, migrating public
  ...
  comparison exceptions are preserved. Direct CPython diff evidence for the
...
- Added `cpython_operator_callable_helper_subset`, migrating public behavior
  ...
  Direct CPython diff evidence for the default-oracle stable getter/methodcaller
  slice is in `cpython_operator_callable_helper_diff_subset`; the newer
  `operator.call` helper has gated direct CPython evidence in
...
- Added `cpython_operator_index_normalization_subset`, and extended
  `cpython_operator_public_helpers_subset` plus
...
- Added `cpython_operator_module_metadata_subset`, migrating public behavior
  ...
  newer `is_none` and `is_not_none` `operator.__all__` entries have gated direct
  CPython evidence in `cpython_operator_is_none_predicates_diff_subset`, and the
  newer `operator.call` entry has gated direct CPython evidence in
  `cpython_operator_call_helper_diff_subset`. Helper instance `__module__`
  metadata has gated direct CPython evidence in
...
- Added `cpython_operator_helper_repr_subset`, migrating the public helper
  ...
  method args without copying CPython's internal helper object layout. Direct
...
- Migrated first-pass attribute-introspection builtin coverage from
  ...
  arity/keyword TypeError paths. MiniPython now also exposes
  `cpython_object_getstate_direct_subset`, backed by
  `cpython_object_getstate_direct_diff_subset`, for CPython's inherited
  `object.__getstate__` descriptor behavior on the default object instance:
  ...
  `cpython_object_getstate_builtin_instance_subset`, backed by
  `cpython_object_getstate_builtin_instance_diff_subset`, for inherited
  `object.__getstate__` on built-in pure-memory instances through
  ...
  extra-argument and keyword `TypeError` text. Pickle behavior, custom instance
  state, class object state, and CPython object-layout internals remain outside
  this sandbox contract. Ordinary `str()` conversion now separately dispatches
  class-level `__str__` for `str()`, `print()`, f-string `!s`,
  ...
  descriptor receivers. The super object attribute assignment/deletion errors slice
  now covers readonly public data attributes and missing-attribute no-`__dict__`
  errors through `cpython_super_attribute_assignment_errors_subset` and
  `cpython_super_attribute_assignment_errors_diff_subset` without adding super instance dictionaries.
  The super object dir() supported attributes slice now exposes the already-supported
  `__thisclass__`, `__self__`, `__self_class__`, `__get__`, and `__class__`
  names through `cpython_super_object_dir_supported_attributes_subset` and
  `cpython_super_object_dir_supported_attributes_diff_subset` without adding
  super instance dictionaries or target class methods to `dir(super_obj)`.
  The super type public descriptors slice now exposes `__thisclass__`, `__self__`,
  `__self_class__`, and `__get__` through `cpython_super_type_public_descriptors_subset`
  and `cpython_super_type_public_descriptors_diff_subset`, including descriptor metadata,
  direct readonly member descriptor access, and the `super.__get__` wrapper descriptor
  without requiring descriptor object identity caching.
  The super repr wrapper descriptor slice now exposes and dispatches `super.__repr__`
  through `cpython_super_repr_wrapper_descriptor_subset` and
  `cpython_super_repr_wrapper_descriptor_diff_subset`, including descriptor metadata,
  direct `super.__repr__()` calls, `repr(super(...))`, and CPython-compatible wrapper
  receiver/argument errors without promoting the rest of super's type-level slot surface.
  The super getattribute wrapper descriptor slice now exposes and dispatches
  `super.__getattribute__` through `cpython_super_getattribute_wrapper_descriptor_subset`
  and `cpython_super_getattribute_wrapper_descriptor_diff_subset`, including descriptor
  metadata, direct access to supported super attributes, and CPython-compatible
  receiver/name/argument errors without promoting unrelated super slot wrappers.
  The MRO slice now uses C3 linearization for
...
- Added `cpython_aiter_anext_builtin_subset` and
  `cpython_aiter_anext_builtin_diff_subset`, exposing the public `aiter()`
  builtin with one-argument arity, `__aiter__` dispatch, async-iterator return
  validation through `__anext__`, and CPython-compatible propagation of
  protocol errors.
- Added `cpython_aiter_anext_keyword_error_subset` and direct CPython evidence
  in `cpython_aiter_anext_keyword_error_diff_subset` for public keyword
  rejection on `aiter()` and `anext()`, including CPython's
  `aiter() takes no keyword arguments` and
  `anext() takes no keyword arguments` messages.
...
- Migrated the first `BaseException` attribute slice from
  ...
  exception calls. Direct CPython evidence is tracked by
...
- Migrated the first `BaseException.with_traceback` slice from
  ...
  traceback values including direct `__traceback__` assignment. Direct CPython evidence is
  tracked by `cpython_base_exception_with_traceback_diff_subset`.
- Migrated the `SystemExit` / `OSError` attribute slice from
  ...
  no-filename, one-filename, and two-filename cases. Direct CPython evidence is
  tracked by `cpython_system_exit_oserror_attributes_diff_subset`.
- Migrated the `SyntaxError` attribute slice from
  ...
  constructor-object attributes, not real parser traceback rendering. Direct CPython evidence is
  tracked by `cpython_syntax_error_attributes_diff_subset`.
- Migrated the Unicode exception attribute slice from
  ...
  when constructed from `bytearray`, while preserving the original `.args`. Direct
  CPython evidence is tracked by `cpython_unicode_error_attributes_diff_subset`.
- Migrated the `AttributeError` keyword-attribute slice from
  ...
  keywords. Capability-gated direct CPython evidence is tracked by
  `cpython_attribute_error_keyword_attributes_diff_subset` for oracles that
  expose this current public behavior.
...
- Extended bytes/bytearray CPython parity evidence for integer-length
  ...
  repetition including zero and negative counts, sandbox `MemoryError` handling
  for address-space-sized bytes/bytearray repeats, bool membership as integer
  byte membership, and membership over integer, bytes, bytearray, and
  memoryview needles. Direct CPython diff evidence is in
...
- Added `cpython_bytes_memoryview_contiguity_methods_subset` with direct
  ...
  `memoryview` needles.
- Added `cpython_bytes_memoryview_concat_subset` with direct
  `cpython_bytes_memoryview_concat_diff_subset` evidence for bytes/bytearray
  concatenation, bytearray in-place concatenation, and `bytearray.extend()`
  rejecting non-contiguous `memoryview` inputs with CPython TypeError text while
  still accepting contiguous sliced views.
...
Completed in the `format()` keyword error pass:

- Added `cpython_format_builtin_keyword_error_subset`, adapted from
  `Lib/test/test_builtin.py::BuiltinTest::test_format` keyword argument
  rejection.
- `format(value=1)`, `format(value=1, format_spec='')`, and
  `format(format_spec='', value=1)` now raise catchable `TypeError` objects
  with CPython's public `format() takes no keyword arguments` message.
- Direct CPython output parity for this keyword-error surface is covered in
  `cpython_format_builtin_keyword_error_diff_subset`.
...
- Added `cpython_float_bytes_like_input_types_subset`, adapted from CPython
  ...
  CPython.
- Added `sys.builtin_module_names` sandbox metadata, exposed as a sorted tuple
  of MiniPython builtin module names while CPython diff evidence only compares
  the stable public shape and required entries.
- Added `sys.getdefaultencoding()` as fixed in-memory UTF-8 metadata with
  CPython-compatible no-argument and no-keyword diagnostics; host filesystem
...
- Added MiniPython `sys.version_info` and `sys.implementation` metadata with
  ...
  compatibility. The same evidence now exposes CPython's public
  `__match_args__` on `sys.version_info`, keeps namedtuple-only helpers such as
  `_fields`, `_asdict`, `_replace`, `_make`, `_field_defaults`, and
  `__slots__` hidden, and pins the CPython public `__getnewargs__()` result
  shape.
- Added `sys.float_info` and `sys.hash_info` public attribute objects plus
  ...
  CPython-compatible structure counts, CPython-visible `__match_args__` is
  available on both the value and type, other namedtuple-only helpers remain hidden,
  `repr()` and type `repr()` use CPython's public `sys.<name>` spelling,
  their public type `__module__` is `sys`, their public type `__doc__` first
  lines follow CPython's documented sys structseq names, their type `__dict__`
  exposes the supported field descriptors, `__match_args__`, and structure
  metadata while keeping other namedtuple-only helper entries including
  `__slots__` hidden, and
  `__getnewargs__()` returns the CPython public nested tuple shape without
...
- Added `cpython_float_hash_and_sys_info_subset`, adapted from CPython
  ...
  `cpython_sys_module_package_metadata_subset` and
  `cpython_sys_module_package_metadata_diff_subset` separately pin sys module `__package__` metadata
  (`sys.__package__` is `''`).
  The same evidence now includes `sys.is_finalizing()` as a pure-memory
  runtime-state query that returns `False` during normal sandbox execution and
  rejects positional and keyword arguments with CPython-compatible diagnostics.
  It also includes `sys.exc_info()` for the current MiniPython VM exception
  state, returning `(None, None, None)` outside active handlers and
  `(type, value, traceback)` inside `except` blocks with CPython-compatible
  no-argument diagnostics. The same sys metadata evidence covers
  `sys.byteorder` as target-endian in-memory metadata constrained to CPython's
  ...
  with `sys.version_info`, its `__match_args__` mirrors its sequence field
  order, and its type `__dict__` exposes the supported version fields,
  `__match_args__`, and structseq metadata without other namedtuple helper
  entries such as `__slots__`, while preserving the public type-level
  `__repr__` descriptor call shape. It remains a sandbox-owned version tuple.
  ...
  and `sys.flags.dont_write_bytecode` as integer `0`;
  `sys.flags.debug` is exposed as integer `0` without enabling CPython debug
  internals or host debug side effects; `sys.flags.dev_mode` is
  exposed as `False` without enabling host debug/dev-mode side effects,
  `sys.flags.hash_randomization` is exposed as integer `0` without reading host
  hash seed configuration,
  `sys.flags.ignore_environment` is exposed as integer `0` without reading host
  ...
  counts, including CPython 3.14's 18 sequence fields and 21 public total
  fields. CPython-visible `__match_args__` is available on both the value and
  type, other namedtuple-only helpers remain hidden, `repr()` and type `repr()`
  use CPython's public `sys.flags` spelling, the public type `__module__` is
  `sys`, public type `__doc__` starts with `sys.flags`, type `__dict__`
  exposes supported flag fields, `__match_args__`, and structseq metadata
  without other namedtuple helper entries such as `__slots__`, and
  `__getnewargs__()` returns the CPython public nested tuple shape without
  reading host startup flags or implying full CPython internal structseq
  compatibility,
  ...
  `sys.flags.isolated` is exposed as integer `0` without reading or applying
  host isolated-startup policy, and
  `sys.flags.optimize` is exposed as the default integer `0` while explicit
  `compile(..., optimize=...)` still controls per-call optimization.
  `sys.flags.quiet` and `sys.flags.verbose` are exposed as integer `0`
  without enabling host startup diagnostics or output suppression.
  `sys.flags.utf8_mode` is exposed as default integer `0` without reading host
  locale or environment encoding policy.
  `sys.flags.warn_default_encoding` is exposed as integer `0`,
  `sys.flags.safe_path` as `False`, and `sys.flags.int_max_str_digits` as the
  sandbox integer digit limit without reading host startup flags. MiniPython
  still does not write bytecode cache files in the sandbox.
...
- Added `cpython_math_constants_and_classification_diff_subset`, gated for
  newer CPython oracles, and
  `cpython_math_constants_and_classification_subset`, adapted from CPython
...
- Added `cpython_math_integer_alias_diff_subset` as capability-gated direct
  CPython parity for the `math.integer` alias surface; older system CPython
  oracles that lack the submodule continue to use the local
  `cpython_math_integer_subset` evidence.
- Added `cpython_math_pow_diff_subset` and `cpython_math_pow_subset`, adapted
  from CPython
...
- Added `cpython_math_fma_diff_subset`, gated for newer CPython oracles, and
  `cpython_math_fma_subset`, adapted from CPython
...
- Added `cpython_math_fmax_fmin_diff_subset`, gated for newer CPython oracles,
  and `cpython_math_fmax_fmin_subset`, adapted from CPython
  ...
  classes.
- Added `cpython_math_fmod_remainder_diff_subset` and
  `cpython_math_fmod_remainder_subset`, adapted from CPython
...
- Added `cpython_math_frexp_ldexp_modf_diff_subset` and
  `cpython_math_frexp_ldexp_modf_subset`, adapted from CPython
  ...
  catchable error classes.
- Added `cpython_math_fsum_diff_subset` and `cpython_math_fsum_subset`,
  adapted from CPython
  ...
  classes.
- Added `cpython_math_sumprod_diff_subset`, gated for newer CPython oracles,
  and `cpython_math_sumprod_subset`, adapted from CPython
...
- Added `cpython_math_nextafter_ulp_diff_subset`, gated for newer CPython
  oracles with `nextafter(steps=...)`, and
  `cpython_math_nextafter_ulp_subset`, adapted from CPython
...
- Added `cpython_math_copysign_diff_subset` and
  `cpython_math_copysign_subset`, adapted from CPython
  `Lib/test/test_math.py::MathTests::testCopysign`. The existing `copysign()`
  math module entry now has direct CPython oracle coverage and local runtime
  coverage for sign transfer across zeroes, infinities, and NaNs, plus
  catchable `TypeError` and `OverflowError` cases for invalid arity/type,
  keyword arguments, and huge integer conversion.
- Added the newer-CPython-oracle-gated
  `cpython_math_signbit_diff_subset` and `cpython_math_signbit_subset`, adapted
  from CPython `Lib/test/test_math.py::MathTests::test_signbit`. The bundled
  `math` module now exposes `signbit()` for negative sign-bit detection across
  zeroes, finite values, infinities, NaNs, bool/int conversion, huge integer
...
- Added `cpython_math_degrees_radians_diff_subset` and
  ...
  bundled `math` module now exposes `degrees()` and `radians()` with direct
  CPython oracle coverage and local runtime coverage for float result
  semantics, non-finite propagation, `__float__` and `__index__` input
  conversion, huge-index overflow, propagated conversion exceptions, and
  TypeError cases supported by the current runtime.
- Added the newer-CPython-oracle-gated `cpython_math_cbrt_diff_subset` and
  `cpython_math_cbrt_subset`, adapted from CPython
  `Lib/test/test_math.py::MathTests::testCbrt`. The bundled `math` module now
  exposes `cbrt()` with direct CPython oracle coverage when the selected oracle
  provides `math.cbrt`, plus local runtime coverage for float result semantics,
  signed zero, non-finite propagation, `__float__` and `__index__` input
  conversion, huge-index overflow, propagated conversion exceptions, and
  TypeError cases supported by the current runtime.
- Added the newer-CPython-oracle-gated
  ...
  `::testExp2`. The bundled `math` module now exposes `exp()` and `exp2()`
  with direct CPython oracle coverage when the selected oracle provides
  `math.exp2`, plus local runtime coverage for float result semantics,
...
- Added `cpython_math_expm1_diff_subset` and `cpython_math_expm1_subset`,
  adapted from CPython `Lib/test/test_math.py::MathTests::test_expm1`. The
  bundled `math` module now exposes `expm1()` with direct CPython oracle
  coverage for public exponential-minus-one behavior, signed zero, non-finite
  propagation, finite-input overflow errors, `__float__` and `__index__` input
  conversion, huge-index overflow, propagated conversion exceptions, and
  TypeError cases supported by the current runtime.
- Added `cpython_math_erf_erfc_diff_subset` and
  `cpython_math_erf_erfc_subset`, adapted from CPython
  `Lib/test/test_math.py::MathTests` public error-function behavior. The
  bundled `math` module now exposes `erf()` and `erfc()` with direct CPython
  oracle coverage for representative six-decimal values, signed zero,
  non-finite propagation, `__float__` and `__index__` input conversion,
  huge-index overflow, propagated conversion exceptions, and TypeError cases.
  Exact platform libm special-function precision remains outside the current
  sandbox subset.
- Added `cpython_math_gamma_lgamma_diff_subset` and
  `cpython_math_gamma_lgamma_subset`, adapted from CPython
  `Lib/test/test_math.py::MathTests` public gamma-function behavior. The
  bundled `math` module now exposes `gamma()` and `lgamma()` with direct
  CPython oracle coverage for representative six-decimal values, non-finite
  propagation, pole/domain errors, `__float__` and `__index__` input
  conversion, huge-index overflow, propagated conversion exceptions, and
  TypeError cases. Exact platform libm special-function precision remains
  outside the current sandbox subset.
- Added `cpython_math_log_family_diff_subset` and
  `cpython_math_log_family_subset`, adapted from CPython
  ...
  `log()`, `log1p()`, `log2()`, and `log10()` with direct CPython oracle
  coverage and local runtime coverage for optional-base division, non-finite
  propagation, large-integer logarithms that avoid float-conversion overflow,
  `__float__` / `__index__` input conversion, and catchable error classes. The
  local runtime subset additionally keeps huge `__index__` inputs and
  OverflowError-to-`__index__` fallback for log helpers covered because the
  default CPython 3.9 oracle does not provide stable cross-version evidence for
  those boundaries.
- Added `cpython_math_trig_diff_subset` and `cpython_math_trig_subset`,
  adapted from CPython `Lib/test/test_math.py::MathTests::testAcos`,
  `::testAsin`, `::testAtan`, `::testAtan2`, `::testCos`, `::testSin`, and
  `::testTan`. The bundled `math` module now exposes `acos()`, `asin()`,
  `atan()`, `atan2()`, `cos()`, `sin()`, and `tan()` with direct CPython oracle
  coverage and local runtime coverage for domain errors, signed-zero `atan2()`
  behavior, non-finite propagation/rejection, `__float__` and `__index__` input
  conversion, huge-index overflow, propagated conversion exceptions, and
  catchable error classes.
- Added `cpython_math_hyperbolic_diff_subset` and
  `cpython_math_hyperbolic_subset`, adapted from CPython
  ...
  `sinh()`, and `tanh()` with direct CPython oracle coverage and local runtime
  coverage for domain errors, finite-input overflow errors, non-finite
...
- Added direct CPython output parity evidence
  ...
  bytearray `hex()` separator/grouping/overflow, `str` subclass separators,
  memoryview separator length/type error boundaries, and length/nibble-boundary
...
- Added direct CPython output parity evidence
  ...
  skipping, `str` subclass input, and representative `ValueError` / `TypeError`
  classes. Added capability-gated direct CPython evidence in
...
Completed in the staticmethod callable descriptor pass:

- Added `cpython_staticmethod_callable_subset` and gated direct CPython
  evidence in `cpython_staticmethod_callable_diff_subset` for current public
  `staticmethod` object callability.
- `callable(staticmethod(func))` now reports true, and direct calls to the
  `staticmethod` object forward positional and keyword arguments to the wrapped
  callable while preserving `__func__`.
- The existing class and instance descriptor path remains covered separately;
  the direct CPython diff skips older oracles whose `staticmethod` objects still
  expose the legacy non-callable behavior.

Completed in the staticmethod metadata descriptor pass:

- Added `cpython_staticmethod_metadata_subset` and gated direct CPython evidence
  in `cpython_staticmethod_metadata_diff_subset` for current public
  `staticmethod` wrapped-function metadata.
- `staticmethod()` now exposes the wrapped callable through `__wrapped__` and
  forwards supported metadata reads for `__name__`, `__qualname__`,
  `__module__`, `__doc__`, and `__annotations__`, with matching `dir()`
  visibility.
- Custom `staticmethod.__dict__` mutation and arbitrary descriptor attributes
  remain outside this focused metadata slice.

Completed in the classmethod metadata descriptor pass:

- Added `cpython_classmethod_metadata_subset` and gated direct CPython evidence
  in `cpython_classmethod_metadata_diff_subset` for current public
  `classmethod` wrapped-function metadata.
- `classmethod()` now exposes the wrapped callable through `__wrapped__` and
  forwards supported metadata reads for `__name__`, `__qualname__`,
  `__module__`, `__doc__`, and `__annotations__`, with matching `dir()`
  visibility.
- Custom `classmethod.__dict__` mutation and arbitrary descriptor attributes
  remain outside this focused metadata slice.

Completed in the staticmethod/classmethod abstract-method descriptor pass:

- Added `cpython_staticmethod_classmethod_abstractmethod_subset` and direct
  CPython evidence in
  `cpython_staticmethod_classmethod_abstractmethod_diff_subset` for public
  `__isabstractmethod__` behavior on `staticmethod` and `classmethod` objects.
- `staticmethod.__isabstractmethod__` and `classmethod.__isabstractmethod__`
  now dynamically reflect the wrapped callable's marker truthiness, reject
...
- Added `cpython_property_abstractmethod_subset` and direct CPython evidence in
  `cpython_property_abstractmethod_diff_subset` for public
  `property.__isabstractmethod__` behavior.
- `property` descriptors now aggregate truthiness from getter, setter, and
  deleter callable `__isabstractmethod__` attributes, with matching `dir()`
  visibility for the supported property descriptor surface.

Completed in the property attribute-assignment descriptor pass:

- Added `cpython_property_attribute_assignment_errors_subset` and direct
  CPython evidence in `cpython_property_attribute_assignment_errors_diff_subset`
  for public property object attribute assignment/deletion errors.
- read-only property slot attributes (`fget`, `fset`, `fdel`), read-only property method attributes,
  `__isabstractmethod__`, and missing attributes now match CPython's `AttributeError` text
  without adding property instance dictionaries.

Completed in the property name metadata descriptor pass:

- Added `cpython_property_name_metadata_subset` and gated direct CPython
  evidence in `cpython_property_name_metadata_diff_subset` for newer public
  `property.__name__` behavior.
- `property.__name__` now falls back to `fget.__name__`, tracks dynamic getter
  name updates such as `renamed_func`, accepts arbitrary assigned values,
  deletes back to the fallback state, copies explicit values through `setter()`,
  and appears in `dir()` for property objects.

Completed in the property set-name metadata descriptor pass:

- Added `cpython_property_set_name_metadata_subset` and gated direct CPython evidence
  in `cpython_property_set_name_metadata_diff_subset` for public
  `property.__set_name__` behavior.
- `property.__set_name__` now updates `property.__name__` for explicit calls
  and class creation, accepts arbitrary name values, rejects keywords, reports
  supported arity errors, and appears in `dir()` for property objects.

Completed in the property no-getter descriptor error pass:

- Added `cpython_property_no_getter_error_subset` and direct CPython evidence
  in `cpython_property_no_getter_error_diff_subset` for public property
  no-getter `AttributeError` text.
- Anonymous descriptors, class-bound descriptors, normal attribute access, and
  arbitrary `property.__name__` values now format no-getter errors like CPython.

Completed in the property no-setter/no-deleter descriptor error pass:

- Added `cpython_property_no_setter_deleter_error_subset` and direct CPython
  evidence in `cpython_property_no_setter_deleter_error_diff_subset` for public
  property no-setter/no-deleter `AttributeError` text.
- Class-bound descriptors, normal attribute assignment/deletion,
  `fget.__name__` fallback, anonymous descriptors, and arbitrary
  `property.__name__` values now format missing setter/deleter errors like
  CPython.

Completed in the property doc metadata descriptor pass:

- Added `cpython_property_doc_metadata_subset` and direct CPython evidence in
  `cpython_property_doc_metadata_diff_subset` for public `property.__doc__`
  behavior.
- `property.__doc__` now defaults from getter docs when the doc argument is
  omitted or `None`, accepts arbitrary assigned values, deletes to `None`,
  recomputes docs for derived `getter()` / `setter()` / `deleter()` properties
  when the original doc is getter-derived, preserves explicit docs through
  `getter()`, and appears in `dir()` for property objects.
...
Completed in the `test_builtin.py` object-constructor argument pass:

- Added `cpython_object_constructor_argument_error_subset`, adapted from
  `Lib/test/test_builtin.py::BuiltinTest::test_object` constructor argument
  checks.
- `object(1)`, `object(a=1)`, and `object(1, a=2)` now raise catchable
  `TypeError` objects with CPython's public `object() takes no arguments`
  message.
- Direct CPython output parity for this constructor-argument surface is covered
  in `cpython_object_constructor_argument_error_diff_subset`.

Completed in the `test_builtin.py` list-constructor keyword error pass:

- Added `cpython_list_constructor_keyword_error_subset`, adapted from
  `Lib/test/test_builtin.py::BuiltinTest::test_list` keyword argument
  rejection.
- `list(iterable=())`, `list(object=())`, and `list((), iterable=())` now
  raise catchable `TypeError` objects with CPython's public
  `list() takes no keyword arguments` message.
- Direct CPython output parity for this keyword-error surface is covered in
  `cpython_list_constructor_keyword_error_diff_subset`.

Completed in the `test_tuple.py` tuple-constructor keyword error pass:

- Added `cpython_tuple_constructor_keyword_error_subset`, adapted from
  `Lib/test/test_tuple.py::TupleTest::test_constructors` keyword argument
  rejection.
- `tuple(iterable=())`, `tuple(object=())`, `tuple((), iterable=())`, and the
  same keyword-rejection surface on plain `tuple` subclasses now raise
  catchable `TypeError` objects with CPython's public
  `tuple() takes no keyword arguments` message.
- Direct CPython output parity for this keyword-error surface is covered in
  `cpython_tuple_constructor_keyword_error_diff_subset`.

Completed in the `test_set.py` set-constructor keyword error pass:

- Added `cpython_set_constructor_keyword_error_subset`, adapted from
  `Lib/test/test_set.py::TestSet` constructor keyword argument rejection.
- `set(iterable=())`, `set(object=())`, `set((), iterable=())`, and the same
  keyword-rejection surface on plain `set` subclasses now raise catchable
  `TypeError` objects with CPython's public
  `set() takes no keyword arguments` message.
- Direct CPython output parity for this keyword-error surface is covered in
  `cpython_set_constructor_keyword_error_diff_subset`.

Completed in the `test_set.py` frozenset-constructor keyword error pass:

- Added `cpython_frozenset_constructor_keyword_error_subset`, adapted from
  `Lib/test/test_set.py::TestFrozenSet` constructor keyword argument rejection.
- `frozenset(iterable=())`, `frozenset(object=())`,
  `frozenset((), iterable=())`, and the same keyword-rejection surface on plain
  `frozenset` subclasses now raise catchable `TypeError` objects with
  CPython's public `frozenset() takes no keyword arguments` message.
- Direct CPython output parity for this keyword-error surface is covered in
  `cpython_frozenset_constructor_keyword_error_diff_subset`.

Completed in the `test_slice.py` slice-constructor keyword error pass:

- Added `cpython_slice_constructor_keyword_error_subset`, adapted from
  `Lib/test/test_slice.py` constructor keyword argument rejection.
- `slice(stop=3)`, `slice(start=1, stop=3)`, and `slice(1, stop=3)` now raise
  catchable `TypeError` objects with CPython's public
  `slice() takes no keyword arguments` message.
- Direct CPython output parity for this keyword-error surface is covered in
...
- `breakpoint()` now dynamically dispatches to the current
  ...
  custom-hook argument `TypeError`s, and raises
...
- Extended `functools.partial` with public `func`, `args`, `keywords`, and
  ...
  `__doc__` metadata, default and instance-overridden `__module__` metadata
  with deletion fallback, deletion of custom attributes, and readonly core
  attributes while leaving placeholder, weakref, pickle, and CPython-internal
  behavior for later slices.
- Added `cpython_functools_partial_instance_module_metadata_subset` and gated
  `cpython_functools_partial_instance_module_metadata_diff_subset` for newer
  CPython partial instance `__module__` metadata, including custom assignment
  and deletion fallback to the default `functools` module.
...
- Added `functools.cache`, `functools.lru_cache`, and
  ...
  size-one/size-two LRU statistics, cached `builtins.len` reentrancy, wrapper
  `__module__` metadata override and deletion fallback to `functools`,
  unbounded caches, and finite LRU eviction. Threading behavior, pickle identity, C
...
- Added `functools.cached_property`, generic descriptor `__set_name__` calls
  ...
  access, copied doc metadata, live public `__dict__` state for `func`,
...
- Added `cpython_functools_cached_property_module_metadata_subset` and gated
  ...
  function, including custom assignment and deletion fallback to `functools`.
...
- Added `functools.singledispatch` and
  ...
  classified for later runtime work. Direct CPython output parity for the
  union-registration surface is now in the CPython-version-gated
  `cpython_functools_singledispatch_union_diff_subset`, with local runtime
  evidence in `cpython_functools_singledispatch_subset`.
...
- Added `cpython_dict_view_membership_rich_compare_subset`, backed by
...
- Added `cpython_dict_view_iterable_set_operators_subset`, backed by
...
- Added `cpython_dict_view_direct_set_operator_methods_subset`, backed by
  ...
  on values views.
- Added `cpython_dict_view_direct_richcompare_methods_subset`, backed by
...
- Added `cpython_dict_view_direct_display_methods_subset`, backed by
  ...
  views.
- Added `cpython_dict_view_hash_methods_subset`, backed by
...
- Added `cpython_dict_view_reduce_methods_subset`, backed by
...
- Added `cpython_dict_view_class_attribute_subset`, backed by
...
- Added `cpython_types_mappingproxy_type_metadata_subset`, backed by
...
- Added `cpython_types_mappingproxy_instance_doc_attribute_subset`, backed by
...
- Added `cpython_types_mappingproxy_type_hierarchy_subset`, backed by
...
- Added `cpython_types_mappingproxy_type_subclasses_subset`, backed by
...
- Added `cpython_types_mappingproxy_type_dict_subset`, backed by
...
- Added `cpython_types_mappingproxy_type_richcompare_subset`, backed by
...
- Added `cpython_types_mappingproxy_keyword_constructor_subset`, backed by
  `cpython_types_mappingproxy_keyword_constructor_diff_subset`, for the public
  `MappingProxyType(mapping={'a': 1})` constructor path and CPython
  argument-binding errors for mixed positional/keyword calls, missing
...
- Added `cpython_types_mappingproxy_attribute_assignment_errors_subset`, backed
  by `cpython_types_mappingproxy_attribute_assignment_errors_diff_subset`, for
  mappingproxy attribute assignment errors on missing attributes and read-only
  mappingproxy method attributes without adding mappingproxy instance
  dictionaries.
...
- Added `cpython_copy_module_package_metadata_subset` and
  `cpython_copy_module_package_metadata_diff_subset`, covering copy module `__package__` metadata
  (`copy.__package__` is `''`).
- Added `cpython_copy_module_all_exports_subset` and
  `cpython_copy_module_all_exports_diff_subset`, covering copy module `__all__` exports
  (`copy.__all__` is `['Error', 'copy', 'deepcopy', 'replace']`).
- Added `cpython_copy_function_module_metadata_subset` and
  `cpython_copy_function_module_metadata_diff_subset`, covering copy public function `__module__` metadata
  (`copy.copy.__module__`, `copy.deepcopy.__module__`, and
  `copy.replace.__module__` are `copy`).
- Added `cpython_copy_function_qualname_metadata_subset` and
  `cpython_copy_function_qualname_metadata_diff_subset`, covering copy public function `__qualname__` metadata
  (`copy.copy.__qualname__`, `copy.deepcopy.__qualname__`, and
  `copy.replace.__qualname__` are `copy`, `deepcopy`, and `replace`).
- Added `cpython_copy_function_doc_metadata_subset` and
  `cpython_copy_function_doc_metadata_diff_subset`, covering copy public function `__doc__` metadata (`copy.copy.__doc__`, `copy.deepcopy.__doc__`, and
  `copy.replace.__doc__` are non-empty strings).
- Added `cpython_copy_function_type_params_metadata_subset` and
  `cpython_copy_function_type_params_metadata_diff_subset`, covering copy public function `__type_params__` metadata (`copy.copy.__type_params__`, `copy.deepcopy.__type_params__`, and
  `copy.replace.__type_params__` are `()`).
- Added `cpython_copy_function_annotate_metadata_subset` and
  `cpython_copy_function_annotate_metadata_diff_subset`, covering copy public function `__annotate__` metadata (`copy.copy.__annotate__`, `copy.deepcopy.__annotate__`, and
  `copy.replace.__annotate__` are `None`).
- Added `cpython_copy_function_closure_none_metadata_subset` and
  `cpython_copy_function_closure_none_metadata_diff_subset`, covering copy public function `__closure__` None metadata (`copy.copy.__closure__`, `copy.deepcopy.__closure__`, and
  `copy.replace.__closure__` are `None`).
- Added `cpython_copy_function_annotations_empty_metadata_subset` and
  `cpython_copy_function_annotations_empty_metadata_diff_subset`, covering copy public function `__annotations__` empty metadata (`copy.copy.__annotations__`, `copy.deepcopy.__annotations__`, and
  `copy.replace.__annotations__` are empty dicts).
- Added `cpython_copy_function_dict_empty_metadata_subset` and
  `cpython_copy_function_dict_empty_metadata_diff_subset`, covering copy public function `__dict__` empty metadata (`copy.copy.__dict__`, `copy.deepcopy.__dict__`, and
  `copy.replace.__dict__` are empty dicts).
- Added `cpython_copy_function_defaults_none_metadata_subset` and
  `cpython_copy_function_defaults_none_metadata_diff_subset`, covering copy public function `__defaults__` None metadata (`copy.copy.__defaults__` and
  `copy.replace.__defaults__` are `None`).
- Added `cpython_copy_function_deepcopy_defaults_shape_metadata_subset` and
  `cpython_copy_function_deepcopy_defaults_shape_metadata_diff_subset`, covering copy public function `deepcopy.__defaults__` shape metadata (`copy.deepcopy.__defaults__` has shape `(None, [])`).
- Added `cpython_copy_function_kwdefaults_metadata_subset` and
  `cpython_copy_function_kwdefaults_metadata_diff_subset`, covering copy public function `__kwdefaults__` metadata (`copy.copy.__kwdefaults__`, `copy.deepcopy.__kwdefaults__`, and
  `copy.replace.__kwdefaults__` are `None`).
- Added `cpython_copy_replace_custom_hook_subset`, adapted from CPython
  `Lib/copy.py` public `copy.replace()` behavior. The VM now dispatches custom
  user-class `__replace__` hooks through class-level lookup, ignores instance
  shadowing, and preserves CPython-style `staticmethod` / `classmethod` hook
  binding. Direct output parity is guarded by
  `cpython_copy_replace_custom_hook_diff_subset`.
- Added `cpython_copy_replace_unsupported_type_error_subset`, adapted from
  CPython `Lib/copy.py` public `copy.replace()` unsupported-object `TypeError`
  behavior. The VM now reports `replace() does not support T objects` for
  values outside the supported replacement protocol, including `object()`, user
  classes missing `__replace__`, user classes with `__replace__ = None`, and
  `int` values. Direct output parity is guarded by
  `cpython_copy_replace_unsupported_type_error_diff_subset`.
- Added `cpython_copy_replace_hook_unexpected_keyword_error_subset`, adapted
  from CPython `Lib/copy.py` public custom `__replace__` hook error behavior.
  The VM now reports class-qualified `__replace__()` unexpected-keyword
  `TypeError` text for bad custom hook signatures, e.g.
  `BadKeyword.__replace__() got an unexpected keyword argument 'x'`. Direct
  output parity is guarded by
  `cpython_copy_replace_hook_unexpected_keyword_error_diff_subset`.
- Added `cpython_copy_replace_classmethod_hook_arity_error_subset`, adapted
  from CPython `Lib/copy.py` public custom classmethod `__replace__` hook error
  behavior. The VM now reports classmethod `__replace__()` arity `TypeError`
  text for classmethod hooks that accept `**changes` but omit the object
  positional parameter, e.g. `ClassMissingObject.__replace__() takes 1
  positional argument but 2 were given`. Direct output parity is guarded by
  `cpython_copy_replace_classmethod_hook_arity_error_diff_subset`.
- Added `cpython_copy_replace_staticmethod_hook_arity_error_subset`, adapted
  from CPython `Lib/copy.py` public custom staticmethod `__replace__` hook
  error behavior. The VM now reports staticmethod hook arity `TypeError` text
  using the function `__qualname__`, preserving class-defined hook names such as
  `StaticFunction.hook()` and `StaticLambda.<lambda>()` while leaving external
  hook names such as `external()` unqualified. Direct output parity is guarded
  by `cpython_copy_replace_staticmethod_hook_arity_error_diff_subset`.
...
- Extended default `athrow()` handling to preserve CPython's public `typ`,
  explicit `val`, accepted real traceback object arguments with raised
  traceback replacement, `tb=None`, and invalid non-traceback `tb` behavior.
...
- Added `cpython_collections_deque_attribute_assignment_errors_subset`, backed
  by `cpython_collections_deque_attribute_assignment_errors_diff_subset`, for
  deque attribute assignment errors on missing attributes, read-only deque data
  attributes, and read-only deque method attributes without adding deque
  instance dictionaries.
- Added `cpython_collections_deque_mutating_eq_subset`, backed by
...
- Added `cpython_collections_module_package_metadata_subset`, backed by
...
- Added `cpython_collections_counter_instance_doc_attribute_subset`, backed by
  `cpython_collections_counter_instance_doc_attribute_diff_subset`, covering
...
- Added `cpython_collections_counter_type_base_metadata_subset`, backed by
  `cpython_collections_counter_type_base_metadata_diff_subset`, covering
  `Counter` direct base metadata through `__base__` and `__bases__`, pinned to
  `builtins.dict`.
- Added `cpython_collections_counter_type_mro_metadata_subset`, backed by
  `cpython_collections_counter_type_mro_metadata_diff_subset`, covering
...
- Added `cpython_collections_userdict_public_methods_diff_subset` and
  `cpython_collections_userdict_public_methods_subset`, adapted from CPython
  `Lib/test/test_collections.py::TestUserObjects`, covering
  ...
  `KeyError.args[0]`, empty `popitem()` no-arg `KeyError`, `.copy()`,
  and `copy.copy()` with shallow
  instance-attribute copying.
- Added `cpython_collections_userdict_instance_doc_attribute_subset`, backed by
  `cpython_collections_userdict_instance_doc_attribute_diff_subset`, covering
...
- Added `cpython_collections_userdict_type_doc_attribute_subset`, backed by
  `cpython_collections_userdict_type_doc_attribute_diff_subset`, covering
...
- Added `cpython_collections_userdict_type_base_metadata_subset`, backed by
  `cpython_collections_userdict_type_base_metadata_diff_subset`, covering
...
- Added `cpython_collections_userlist_type_base_metadata_subset`, backed by
  `cpython_collections_userlist_type_base_metadata_diff_subset`, covering
...
- Added `cpython_collections_userstring_type_base_metadata_subset`, backed by
  `cpython_collections_userstring_type_base_metadata_diff_subset`, covering
...
- Added `cpython_collections_abc_type_hierarchy_metadata_subset`, backed by
  `cpython_collections_abc_type_hierarchy_metadata_diff_subset`, covering
...
- Added `cpython_collections_userlist_instance_doc_attribute_subset`, backed by
  ...
  instance dictionaries.
- Added `cpython_collections_userlist_mutating_eq_diff_subset` and
  `cpython_collections_userlist_mutating_eq_subset`, covering UserList mutation during comparison
  for membership, `__contains__`, `count`, `index`, and `remove`:
  append-during-false-comparison remains visible to the search,
  clear-during-false-comparison stops the search, and true-match paths preserve
  CPython return values after the mutation.
...
- Added `cpython_collections_chainmap_instance_doc_attribute_subset`, backed by
  `cpython_collections_chainmap_instance_doc_attribute_diff_subset`, covering
  ...
  instance dictionaries.
- Added `cpython_collections_chainmap_type_base_metadata_subset`, backed by
  `cpython_collections_chainmap_type_base_metadata_diff_subset`, covering
  `ChainMap` direct base metadata through `__base__` and `__bases__`, pinned to
  `collections.abc.MutableMapping` without expanding full `__mro__` parity.
- Added `cpython_collections_chainmap_keyword_error_subset`, backed by
  `cpython_collections_chainmap_keyword_error_diff_subset`, for CPython's
  public `ChainMap.__init__() got an unexpected keyword argument` diagnostics
  on `ChainMap(mapping=...)`, multi-keyword, and positional-plus-keyword
  constructor calls.
- Added `cpython_collections_chainmap_constructor_lazy_mapping_subset`, backed
  by `cpython_collections_chainmap_constructor_lazy_mapping_diff_subset`, for
  CPython's ChainMap constructor lazy map-source storage: non-mapping sources
  such as `1`, `[]`, `None`, and `(1, 2)` are preserved in `.maps` rather than
  rejected at construction time.
- Added `cpython_collections_chainmap_constructor_source_repr_subset`, backed
  by `cpython_collections_chainmap_constructor_source_repr_diff_subset`, for
  ChainMap constructor source repr display. Stored string, tuple, and dict map
  sources now use CPython-style repr formatting, including quotes for
  `repr(ChainMap('abc'))` and `str(ChainMap('abc'))`.
- Added `cpython_collections_chainmap_subclass_source_repr_subset`, backed by
  `cpython_collections_chainmap_subclass_source_repr_diff_subset`, for
  ChainMap subclass source repr, string, and empty-format display across stored
  string, tuple, and dict sources using the subclass name.
- Added `cpython_collections_chainmap_subclass_display_type_methods_subset`,
  backed by
  `cpython_collections_chainmap_subclass_display_type_methods_diff_subset`, for
  ChainMap subclass display type methods inherited through the class object:
  `__repr__`, `__str__`, and `__format__`, while keeping `maps` absent from
  the type object.
- Added `cpython_collections_chainmap_constructor_source_truthiness_subset`,
  backed by
  `cpython_collections_chainmap_constructor_source_truthiness_diff_subset`, for
  ChainMap constructor source truthiness across empty, mapping, integer, list,
  string, and `None` map sources without eager mapping validation.
- Added `cpython_collections_chainmap_subclass_source_truthiness_subset`, backed
  by `cpython_collections_chainmap_subclass_source_truthiness_diff_subset`, for
  ChainMap subclass source truthiness across empty, mapping, integer, list,
  string, and `None` map sources without falling back to key length.
- Added `cpython_collections_chainmap_constructor_source_len_subset`, backed by
  `cpython_collections_chainmap_constructor_source_len_diff_subset`, for
  ChainMap constructor source length across mapping, list, tuple, string,
  integer, and `None` sources, including direct `ChainMap.__len__()` dispatch
  and CPython's non-iterable source TypeError text.
- Added `cpython_collections_chainmap_subclass_source_len_subset`, backed by
  `cpython_collections_chainmap_subclass_source_len_diff_subset`, for
  ChainMap subclass source length across mapping, list, tuple, string,
  integer, and `None` sources without eager mapping validation.
- Added `cpython_collections_chainmap_constructor_source_iter_subset`, backed by
  `cpython_collections_chainmap_constructor_source_iter_diff_subset`, for
  ChainMap constructor source iteration across dict, `OrderedDict`, `UserDict`
...
- Added `cpython_collections_chainmap_constructor_source_items_subset`, backed
  by `cpython_collections_chainmap_constructor_source_items_diff_subset`, for
  ChainMap constructor source item lookup across list and tuple sources through
  `items()`, direct `ChainMap.items(...)`, and `dict(...)` coercion without
  eager mapping-entry expansion.
...
- Added `cpython_collections_chainmap_iter_does_not_call_getitem_diff_subset`
  and `cpython_collections_chainmap_iter_does_not_call_getitem_subset`,
...
- Added `cpython_collections_chainmap_union_operators_diff_subset` and
  `cpython_collections_chainmap_union_operators_subset`, adapted from CPython
  `TestChainMap::test_union_operators`, covering ChainMap/mapping union,
...
- Added `cpython_collections_chainmap_new_child_custom_mapping_diff_subset`
  and `cpython_collections_chainmap_new_child_custom_mapping_subset`, adapted
...
- Added `cpython_collections_chainmap_order_preservation_diff_subset` and
  `cpython_collections_chainmap_order_preservation_subset`, adapted from
...
- Added `cpython_collections_defaultdict_core_diff_subset` and
  ...
  callable/`None` factory validation, keyword `default_factory` treatment as a
  mapping item, missing-key factory insertion, `get()` not invoking missing
  behavior, `default_factory` assignment/reset, shallow `copy()`, factory
  exception propagation, and `json.dumps()` mapping serialization. Full
  pickle/eval identity, merge operators, and subclass compatibility stay
  outside the default sandbox surface.
- Added `cpython_collections_defaultdict_instance_doc_attribute_diff_subset`
  and `cpython_collections_defaultdict_instance_doc_attribute_subset`, covering
...
- Added `cpython_collections_defaultdict_type_base_metadata_subset`, backed by
  `cpython_collections_defaultdict_type_base_metadata_diff_subset`, covering
  `defaultdict` direct base metadata through `__base__` and `__bases__`,
  pinned to `builtins.dict`.
- Added `cpython_collections_defaultdict_type_mro_metadata_subset`, backed by
  `cpython_collections_defaultdict_type_mro_metadata_diff_subset`, covering
...
- Added
  ...
  and `cpython_collections_defaultdict_attribute_assignment_errors_subset`,
  covering `defaultdict` instance attribute assignment/deletion errors for the
  supported pure-memory mapping: existing read-only method attributes such as
  `copy`, `update`, `get`, and `fromkeys` now use CPython's read-only
  `AttributeError` text, missing attributes keep the no-`__dict__` text, and
  `default_factory` remains writable and deletable. Full method-table parity,
...
- Added `cpython_collections_defaultdict_copy_module_diff_subset` and
  ...
  copy is a fresh `defaultdict`, keeps the same default factory, shallow-copies
  mapping entries, shares nested values, and isolates later missing-key writes
  to the copied mapping. `deepcopy`, pickle/eval identity, merge operators,
  and defaultdict subclass compatibility stay outside this slice.
- Added `cpython_collections_defaultdict_dunder_copy_diff_subset` and
  ...
  for the supported pure-memory mapping: the direct `__copy__` hook is visible
  through `dir()`, returns a fresh `defaultdict`, keeps the same default
  factory, shallow-copies mapping entries, shares nested values, and preserves
  missing-key behavior. descriptor type identity, `deepcopy`, pickle/eval
  identity, merge operators, and defaultdict subclass compatibility stay
  outside this slice.
- Added `cpython_collections_defaultdict_copy_descriptor_errors_diff_subset`
  ...
  calls still return fresh shallow copies, while `defaultdict.copy` and
  `defaultdict.__copy__` are exposed as `method_descriptor` values,
  `defaultdict.__dict__['copy']` and `defaultdict.__dict__['__copy__']` are
  available through the type mappingproxy, and unbound method text,
  method-descriptor self validation, no-extra-argument text, and keyword
  rejection now match CPython public behavior. Full `defaultdict.__dict__`
  method-table parity, `deepcopy`, pickle/eval identity, merge operators, and
  defaultdict subclass compatibility stay outside this slice.
- Added `cpython_collections_defaultdict_fromkeys_diff_subset` and
  ...
  pure-memory mapping: the classmethod-style hook is visible through `dir()`,
  returns `defaultdict(None, ...)` with `default_factory is None`, preserves
  shared explicit default values, raises `KeyError` on later missing-key
  lookups, and pins arity plus keyword rejection behavior. Pickle/eval
  identity, merge operators, and defaultdict subclass compatibility stay
  outside this slice.
- Added `cpython_collections_defaultdict_missing_descriptor_diff_subset` and
  `cpython_collections_defaultdict_missing_descriptor_subset`, covering
  `defaultdict.__missing__()` on bound instances and through
...
- Added `cpython_collections_defaultdict_format_error_diff_subset` and
  ...
  supported pure-memory mapping: empty format specs preserve the display text,
  and non-empty format spec values raise the CPython public
  `collections.defaultdict.__format__` TypeError text. descriptor identity,
  pickle/eval identity, merge operators, and defaultdict subclass
  compatibility stay outside this slice.
- Exposed a first-pass `collections.OrderedDict` constructor alias over
  ...
  `KeyError.args[0]`, empty `popitem()` errors preserving CPython's
  `KeyError(message)` string display, and public error classes for unexpected
...
- Added `cpython_collections_counter_comparison_diff_subset` as gated direct
  ...
  supports the post-3.10 Counter behavior; older system oracles skip this diff.
- Added `cpython_collections_counter_fromkeys_diff_subset` and
  `cpython_collections_counter_fromkeys_subset` for the stable CPython-oracle
  `Counter.fromkeys()` / instance `fromkeys()` public `NotImplementedError`
  behavior.
...
- Added `cpython_collections_counter_mapping_mutation_diff_subset` and
  ...
  `keys()` descriptor-style argument diagnostics, `values()` descriptor-style argument diagnostics, `popitem()`,
  empty `popitem()` `KeyError(message)` string display, `clear()`, `clear()` descriptor-style argument diagnostics,
...
- Extended `cpython_collections_counter_public_diff_subset` and
  ...
  count `__index__` conversion boundary, `Counter.elements()` public `self`
  and keyword binding diagnostics, and `Counter.total()` reflected addition
  dispatch plus public `self` and duplicate-keyword argument binding
  diagnostics.
...
- Added Counter multiset arithmetic support for current CPython-supported `+`,
  `-`, `|`, and `&` across binary operators and direct Counter dunder methods,
  with positive-count result filtering matching CPython.
- Added Counter in-place arithmetic support for current CPython-supported
  `__iadd__`, `__isub__`, `__ior__`, and `__iand__`, preserving the original
  Counter storage identity for `id()` and `is` checks.
...
- Added `cpython_collections_counter_inplace_operations_matrix_subset`,
  ...
  samples for current CPython-supported in-place Counter operations.
- Added
  `cpython_collections_counter_multiset_operations_equivalent_to_set_operations_diff_subset` and
  ...
  across the current CPython-supported part of the full 64-by-64 zero/one-count
  Counter matrix. The direct diff remains gated for CPython oracles that expose
  `Counter.__xor__`.
...
- Added `cpython_collections_namedtuple_factory_instance_diff_subset` as gated
  direct diff evidence for generated namedtuple factory metadata, tuple-like
  instance behavior, helper methods, construction errors, and zero/one-field
  edge cases when the configured CPython oracle supports generated match args.
...
- Added `cpython_collections_namedtuple_match_args_diff_subset` as gated direct
  diff evidence for generated namedtuple `__match_args__` metadata and
  executable class-pattern matching when the configured CPython oracle supports
  that public behavior; older system oracles skip this diff.
...
- Added `cpython_collections_namedtuple_new_builtins_globals_diff_subset` and
  `cpython_collections_namedtuple_new_builtins_globals_subset` as the stable
  direct CPython-oracle slice for the generated namedtuple
  `__new__.__globals__['__builtins__'] == {}` public behavior.
- Added `cpython_collections_namedtuple_new_builtins_issue_43102_diff_subset`
  as gated direct diff evidence for the newer `function.__builtins__`
  attribute assertion when the configured CPython oracle supports it.
...
- Added `cpython_types_nonetype_unacceptable_base_type_subset`, backed by
  `cpython_types_nonetype_unacceptable_base_type_diff_subset`, for CPython's
  `type 'NoneType' is not an acceptable base type` rejection through
  `types.NoneType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime None `__class__` bases while keeping
  supported `ModuleType` subclass creation working.
- Added `cpython_types_notimplementedtype_unacceptable_base_type_subset`, backed
  by `cpython_types_notimplementedtype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'NotImplementedType' is not an acceptable base type` rejection
  through `types.NotImplementedType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime NotImplemented
  `__class__` bases while keeping supported `ModuleType` subclass creation
  working.
- Added `cpython_types_ellipsistype_unacceptable_base_type_subset`, backed by
  `cpython_types_ellipsistype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'ellipsis' is not an acceptable base type` rejection through
  `types.EllipsisType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime Ellipsis `__class__` bases while keeping
  supported `ModuleType` subclass creation working.
- Added `cpython_types_capsuletype_unacceptable_base_type_subset`, backed by
  `cpython_types_capsuletype_unacceptable_base_type_diff_subset`, for CPython's
  `type 'PyCapsule' is not an acceptable base type` rejection through public
  `types.CapsuleType` class statements, `type(...)`, `type.__new__(...)`, and
  `types.new_class(...)` while keeping supported `ModuleType` subclass creation
  working and leaving the capsule C API out of scope.
- Added `cpython_types_capsuletype_module_metadata_subset`, backed by
  `cpython_types_capsuletype_module_metadata_diff_subset`, for CPython's public
  `types.CapsuleType.__module__ == 'builtins'` metadata returning `builtins`
  through direct lookup and `object.__getattribute__`. The slice keeps
  `__module__` out of `dir(types.CapsuleType)` and the fallback `__dict__`
  probe, so it does not promote capsule C API behavior or a writable type dictionary
  into the sandbox contract.
- Added `cpython_types_capsuletype_qualname_metadata_subset`, backed by
  `cpython_types_capsuletype_qualname_metadata_diff_subset`, for CPython's
  public `types.CapsuleType.__qualname__ == 'PyCapsule'` metadata returning
  `PyCapsule` through direct lookup and `object.__getattribute__`. The slice
  keeps `__qualname__` out of `dir(types.CapsuleType)` and the fallback
  `__dict__` probe, so it does not promote capsule C API behavior or a writable
  type dictionary into the sandbox contract.
- Added `cpython_types_capsuletype_doc_metadata_subset`, backed by
  `cpython_types_capsuletype_doc_metadata_diff_subset`, for CPython's public
  `types.CapsuleType.__doc__` metadata as a string through direct lookup and
  `object.__getattribute__`, plus `dir(types.CapsuleType)` visibility. The
  slice does not promote capsule construction, capsule C API behavior, or a
  writable type dictionary into the sandbox contract.
- Added `cpython_types_capsuletype_text_signature_metadata_subset`, backed by
  `cpython_types_capsuletype_text_signature_metadata_diff_subset`, for CPython's
  public `types.CapsuleType.__text_signature__ is None` metadata through direct
  lookup and `object.__getattribute__`. The slice keeps `__text_signature__`
  out of `dir(types.CapsuleType)` and the fallback `__dict__` probe, so it does
  not promote capsule C API behavior, constructor-signature support, or a
  writable type dictionary into the sandbox contract.
- Added `cpython_types_module_package_metadata_subset`, backed by
  `cpython_types_module_package_metadata_diff_subset`, for types module `__package__` metadata
  (`types.__package__` is CPython's empty string).
- Added `cpython_types_accelerator_module_package_metadata_subset`, backed by
  `cpython_types_accelerator_module_package_metadata_diff_subset`, for _types module `__package__` metadata
  (`_types.__package__` is CPython's empty string).
...
- Added `cpython_types_functiontype_unacceptable_base_type_subset`, backed by
  `cpython_types_functiontype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'function' is not an acceptable base type` rejection through
  `types.FunctionType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime function `__class__` bases, while keeping
  supported `ModuleType` subclass creation working and without expanding into
  the broader runtime type-object matrix.
- Added `cpython_types_codetype_unacceptable_base_type_subset`, backed by
  `cpython_types_codetype_unacceptable_base_type_diff_subset`, for CPython's
  `type 'code' is not an acceptable base type` rejection through
  `types.CodeType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime code object `__class__` bases, while
  keeping supported `ModuleType` subclass creation working and without
  expanding into the broader runtime type-object matrix.
- Added `cpython_types_tracebacktype_unacceptable_base_type_subset`, backed by
  `cpython_types_tracebacktype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'traceback' is not an acceptable base type` rejection through
  `types.TracebackType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime traceback `__class__` bases, while
  keeping supported `ModuleType` subclass creation working and without
  expanding into the broader runtime type-object matrix.
- Added `cpython_types_frametype_unacceptable_base_type_subset`, backed by
  `cpython_types_frametype_unacceptable_base_type_diff_subset`, for CPython's
  `type 'frame' is not an acceptable base type` rejection through
  `types.FrameType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and `sys._getframe()` runtime frame `__class__`
  bases, while keeping supported `ModuleType` subclass creation working and
  without expanding into the broader runtime type-object matrix.
- Added `cpython_types_generatortype_unacceptable_base_type_subset`, backed by
  `cpython_types_generatortype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'generator' is not an acceptable base type` rejection through
  `types.GeneratorType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime generator `__class__` bases, while
  keeping supported `ModuleType` subclass creation working and without
  expanding into the broader runtime type-object matrix.
- Added `cpython_types_coroutinetype_unacceptable_base_type_subset`, backed by
  `cpython_types_coroutinetype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'coroutine' is not an acceptable base type` rejection through
  `types.CoroutineType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime coroutine `__class__` bases, while
  keeping supported `ModuleType` subclass creation working and without
  expanding into the broader runtime type-object matrix.
- Added `cpython_types_asyncgeneratortype_unacceptable_base_type_subset`,
  backed by `cpython_types_asyncgeneratortype_unacceptable_base_type_diff_subset`,
  for CPython's `type 'async_generator' is not an acceptable base type`
  rejection through `types.AsyncGeneratorType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime async generator
  `__class__` bases, while keeping supported `ModuleType` subclass creation
  working and without expanding into the broader runtime type-object matrix.
- Added `cpython_types_builtinfunctiontype_unacceptable_base_type_subset`,
  backed by `cpython_types_builtinfunctiontype_unacceptable_base_type_diff_subset`,
  for CPython's `type 'builtin_function_or_method' is not an acceptable base type`
  rejection through `types.BuiltinFunctionType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime builtin function
  `__class__` bases, while keeping supported `ModuleType` subclass creation
  working and without expanding into the broader runtime type-object matrix.
- Added `cpython_types_methodtype_unacceptable_base_type_subset`, backed by
  `cpython_types_methodtype_unacceptable_base_type_diff_subset`, for CPython's
  `type 'method' is not an acceptable base type` rejection through
  `types.MethodType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and runtime method `__class__` bases, while keeping
  supported `ModuleType` subclass creation working and without expanding into
  the broader runtime type-object matrix.
- Added `cpython_types_methoddescriptortype_unacceptable_base_type_subset`,
  backed by
  `cpython_types_methoddescriptortype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'method_descriptor' is not an acceptable base type` rejection
  through `types.MethodDescriptorType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime method descriptor
  `__class__` bases, while keeping supported `ModuleType` subclass creation
  working and without expanding into the broader runtime type-object matrix.
- Added `cpython_types_classmethoddescriptortype_unacceptable_base_type_subset`,
  backed by
  `cpython_types_classmethoddescriptortype_unacceptable_base_type_diff_subset`,
  for CPython's `type 'classmethod_descriptor' is not an acceptable base type`
  rejection through `types.ClassMethodDescriptorType` class statements,
  `type(...)`, `type.__new__(...)`, `types.new_class(...)`, and runtime classmethod descriptor
  `__class__` bases, while keeping supported
  `ModuleType` subclass creation working and without expanding into the broader
  runtime type-object matrix.
- Added `cpython_types_wrapperdescriptortype_unacceptable_base_type_subset`,
  backed by
  `cpython_types_wrapperdescriptortype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'wrapper_descriptor' is not an acceptable base type`
  rejection through `types.WrapperDescriptorType` class statements,
  `type(...)`, `type.__new__(...)`, `types.new_class(...)`, and runtime wrapper descriptor
  `__class__` bases, while keeping supported `ModuleType` subclass
  creation working and without expanding into the broader runtime type-object
  matrix.
- Added `cpython_types_methodwrappertype_unacceptable_base_type_subset`, backed
  by `cpython_types_methodwrappertype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'method-wrapper' is not an acceptable base type` rejection
  through `types.MethodWrapperType` class statements, `type(...)`,
  `type.__new__(...)`, `types.new_class(...)`, and runtime method-wrapper
  `__class__` bases, while keeping supported `ModuleType` subclass creation
  working and without expanding into the broader runtime type-object matrix.
- Added `cpython_types_getsetdescriptortype_unacceptable_base_type_subset`,
  backed by
  `cpython_types_getsetdescriptortype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'getset_descriptor' is not an acceptable base type`
  rejection through `types.GetSetDescriptorType` class statements,
  `type(...)`, `type.__new__(...)`, `types.new_class(...)`, and runtime getset descriptor
  `__class__` bases, while keeping supported `ModuleType` subclass
  creation working and without expanding into the broader runtime type-object
  matrix.
- Added `cpython_types_memberdescriptortype_unacceptable_base_type_subset`,
  backed by
  `cpython_types_memberdescriptortype_unacceptable_base_type_diff_subset`, for
  CPython's `type 'member_descriptor' is not an acceptable base type`
  rejection through `types.MemberDescriptorType` class statements,
  `type(...)`, `type.__new__(...)`, `types.new_class(...)`, and runtime member descriptor
  `__class__` bases, while keeping supported `ModuleType` subclass
  creation working and without expanding into the broader runtime type-object
  matrix.
...
- Added public `types` runtime aliases for `LambdaType`, `GeneratorType`,
  ...
  classified.
- Added `cpython_types_celltype_module_metadata_subset`, backed by
  `cpython_types_celltype_module_metadata_diff_subset`, for CPython's public
  `types.CellType.__module__ == 'builtins'` metadata through direct lookup and
  `object.__getattribute__`. The slice keeps `__module__` out of
  `dir(types.CellType)` and the fallback `__dict__` probe, so broader
  `CellType` type-object metadata and a writable type dictionary remain outside
  the sandbox contract.
- Added `cpython_types_celltype_base_metadata_subset`, backed by
  `cpython_types_celltype_base_metadata_diff_subset`, for CPython's public
  ...
  `object.__getattribute__`. The slice keeps `__base__` and `__bases__` out of
  ...
  slice.
- Added `cpython_types_celltype_display_metadata_subset`, backed by
  `cpython_types_celltype_display_metadata_diff_subset`, for CPython's public
  `repr(types.CellType)`, `str(types.CellType)`, `format(types.CellType, '')`,
  and `types.CellType.__mro__` display shape as `<class 'cell'>`. The slice
  reuses the existing `object` display for `types.CellType.__base__` while
  keeping CPython object-layout internals and a writable type dictionary outside
  this sandbox contract slice.
- Added `cpython_types_celltype_name_metadata_subset`, backed by
  `cpython_types_celltype_name_metadata_diff_subset`, for CPython's public
  `types.CellType.__name__` metadata returning `cell` through direct lookup and
  `object.__getattribute__`. The slice also keeps `__name__` out of
  `dir(types.CellType)` and the fallback `__dict__` probe while leaving broader
  CellType constructor behavior, CPython implementation fields such as
  `__basicsize__`, and a writable type dictionary outside this sandbox contract
  slice.
- Added `cpython_types_celltype_qualname_metadata_subset`, backed by
  `cpython_types_celltype_qualname_metadata_diff_subset`, for CPython's public
  `types.CellType.__qualname__` metadata returning `cell` through direct lookup
  and `object.__getattribute__`. The slice keeps `__qualname__` out of
  `dir(types.CellType)` and the fallback `__dict__` probe, so broader
  `CellType` type-object metadata and a writable type dictionary remain outside
  this sandbox contract slice.
- Added `cpython_types_celltype_text_signature_metadata_subset`, backed by
  `cpython_types_celltype_text_signature_metadata_diff_subset`, for CPython's
  public `types.CellType.__text_signature__` metadata returning `([contents])`
  through direct lookup and `object.__getattribute__`. The slice keeps
  `__text_signature__` out of `dir(types.CellType)` and the fallback `__dict__`
  probe, so broader CellType constructor behavior and a writable type
  dictionary remain outside this sandbox contract slice.
- Added `cpython_types_celltype_doc_metadata_subset`, backed by
  `cpython_types_celltype_doc_metadata_diff_subset`, for CPython's public
  `types.CellType.__doc__` metadata as a string through direct lookup and
  `object.__getattribute__`, plus `dir(types.CellType)` visibility. The slice
  avoids fallback `__dict__` parity and keeps broader CellType constructor
  behavior and a writable type dictionary outside this sandbox contract slice.
- Added `cpython_types_celltype_keyword_error_subset`, backed by
  `cpython_types_celltype_keyword_error_diff_subset`, for the public
  `types.CellType` keyword-rejection path. `types.CellType(x=1)`,
  `types.CellType(value=1)`, and `types.CellType(1, x=2)` now raise catchable
  `TypeError` objects with CPython's public `cell() takes no keyword arguments`
  text while keeping the broader cell constructor behavior as a separate
  surface.
- Added `cpython_types_celltype_constructor_behavior_subset`, backed by
  `cpython_types_celltype_constructor_behavior_diff_subset`, for CPython's
  public `types.CellType()`, `types.CellType(value)`, `cell_contents` read,
  assignment, deletion, empty-cell `Cell is empty` errors, and positional arity
  text `cell expected at most 1 argument, got N`. The slice keeps CPython
  object-layout internals, pickle identity matrices, and a writable type
  dictionary outside this sandbox contract.
- Added `cpython_types_celltype_cell_contents_descriptor_subset`, backed by
  `cpython_types_celltype_cell_contents_descriptor_diff_subset`, for CPython's
  public `types.CellType.cell_contents` getset descriptor metadata, visibility
  through `dir(types.CellType)` / the type `__dict__`, and
  `descriptor.__get__(cell, types.CellType)`, `descriptor.__set__(cell, value)`,
  and `descriptor.__delete__(cell)` for the pure-memory descriptor over cell
  contents. Pickle behavior, `__sizeof__`, and CPython object-layout internals
  remain outside this sandbox contract.
- Added `cpython_types_celltype_attribute_errors_subset`, backed by
  `cpython_types_celltype_attribute_errors_diff_subset`, for CPython's public
  missing-attribute diagnostics on `cell.missing`, `object.__getattribute__`,
  `setattr(cell, 'missing', 1)`, and `delattr(cell, 'missing')`, including the
  `no __dict__ for setting new attributes` suffix for mutation attempts. The
  slice preserves `cell_contents` deletion behavior while keeping a writable instance dictionary
  and CPython object-layout internals outside the sandbox contract.
- Added `cpython_types_celltype_dir_surface_subset`, backed by
  `cpython_types_celltype_dir_surface_diff_subset`, for CPython's public
  `dir(cell)` and `object.__dir__(cell)` name surface on closure cells,
  ...
  `__sizeof__`. This slice covers name visibility only; pickle behavior,
  `__sizeof__` object-layout values, and other CPython object-layout internals
  remain outside this sandbox contract.
- Added `cpython_types_celltype_getstate_subset`, backed by
  `cpython_types_celltype_getstate_diff_subset`, for CPython's public inherited
  `object.__getstate__` behavior on closure cells. `cell.__getstate__()`,
  `object.__getstate__(cell)`, and `types.CellType.__getstate__(cell)` now
  return the pure-memory no-state value `None`, and the extra positional
  argument, keyword argument, and missing receiver paths preserve CPython's
  public `TypeError` text. Pickle behavior, custom instance state, and CPython
  object-layout internals remain outside this sandbox contract.
- Added `cpython_types_celltype_hash_semantics_subset`, backed by
  `cpython_types_celltype_hash_semantics_diff_subset`, for CPython's public
  cell hash contract: `hash(cell)` raises `unhashable type: 'cell'`,
  ...
  align dict/set key error text for cells or any CPython object-layout details
  behind the identity hash.
- Added `cpython_types_celltype_instance_display_subset`, backed by
  `cpython_types_celltype_instance_display_diff_subset`, for CPython's public
  address shape of `repr(cell)`, `str(cell)`, `format(cell, '')`, and
  `object.__repr__(cell)` on empty and populated cells. This slice pins display
  shape only; exact object addresses and CPython object-layout internals remain
  outside the sandbox contract.
- Added `cpython_types_celltype_rich_compare_subset`, backed by
  `cpython_types_celltype_rich_compare_diff_subset`, for CPython's public
  cell-vs-cell content comparison behavior across `cell == other_cell`, order
  operators, `cell.__eq__(other)`, and
  `types.CellType.__lt__(cell, other)`. The empty cell ordering and non-cell
  direct-dunder `NotImplemented` results are covered; descriptor type metadata
  and CPython object-layout internals remain outside this sandbox slice.
- Added `cpython_types_celltype_unacceptable_base_type_subset`, backed by
  `cpython_types_celltype_unacceptable_base_type_diff_subset`, for CPython's
  `type 'cell' is not an acceptable base type` rejection through
  `types.CellType` class statements, `type(...)`, `type.__new__(...)`,
  `types.new_class(...)`, and closure cell `__class__` bases, while keeping
  supported `ModuleType` subclass creation working and without expanding into
  the broader runtime type-object matrix.
...
- Promoted `Lib/test/test_types.py::TypesTests::test_dunder_get_signature`
...
- Classified `Lib/test/test_types.py::TypesTests` as `ported_public` for the
  default sandbox `types` contract: all portable public methods have direct
  Rust evidence, while locale formatting, `_io` / `_queue` descriptor-crash
  coverage, CPython object-layout sizes, C-extension capsules, and
  specialization child-process crash rows remain explicit stop-lines.
...
- Added `cpython_itertools_groupby_subset` and
  ...
  key functions, generator-backed input, shared `_grouper` invalidation when
  the parent iterator advances, keyword binding for `iterable` / `key`, and
  public TypeError classes.
- Extended `cpython_itertools_core_iterator_subset` and
...
- Added `cpython_itertools_count_bool_arithmetic_subset` and
  `cpython_itertools_count_bool_arithmetic_diff_subset`, keeping CPython's
  public `count()` distinction between a preserved bool first value and
  subsequent numeric addition results covered by direct differential evidence.
- Extended `cpython_itertools_repr_subset` and
  `cpython_itertools_repr_diff_subset` with public iterator/helper type
  `__qualname__` / `__module__` / `__doc__` metadata, including the
  `_grouper.__doc__ is None` boundary, without expanding into pickling,
  exact-address repr, or cache-compaction internals.
...
- Added `cpython_ast_literal_eval_public_diff_subset`, direct CPython output
  ...
  diff covers safe literal containers, bytes, `set()`, numeric signs, negative
