...
- Added `cpython_collections_counter_mapping_mutation_diff_subset` and
  ...
  `pop()` descriptor-style argument diagnostics, `setdefault()`, `popitem()`,
  empty `popitem()` `KeyError(message)` string display, `clear()`, `clear()` descriptor-style argument diagnostics,
  zero lookup after deletion behavior plus `popitem()` descriptor-style argument diagnostics out of
  the broader Counter basics coverage.
