...
- Added `cpython_types_celltype_qualname_metadata_subset`, backed by
  ...
  `dir(types.CellType)` and the fallback `__dict__` probe, so `__name__`,
...
- Added `cpython_types_celltype_text_signature_metadata_subset`, backed by
  ...
  probe, so `__name__`, `__doc__`, broader CellType constructor behavior, and a
  writable type dictionary remain outside this sandbox contract slice.
