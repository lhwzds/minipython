...
| `json` | Pure in-memory `loads()` from `str` / `bytes` / `bytearray` values and subclasses, including UTF-8 BOM and UTF-16/UTF-32 encoded byte input, plus `dumps()` data model subset for common JSON values, `str` / `int` / `float` subclass and `IntEnum` values/keys, list/tuple/dict subclass and namedtuple containers, strings/escapes, finite and default non-finite numbers, bool/null, lists/tuples/dicts, basic dict-key coercion, circular-reference rejection, and first-pass error classification. | `cpython_json_loads_dumps_diff_subset`, `cpython_json_loads_dumps_error_boundary_diff_subset`. | File APIs, hooks, keyword options such as `allow_nan`, bytes/bytearray serialization, unpaired surrogate storage, and full `JSONDecodeError` compatibility. |
