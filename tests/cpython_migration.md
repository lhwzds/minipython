...
| `copy` | `copy()`, `deepcopy()`, and `replace()` for supported runtime values and AST/simple namespace use cases. Subset evidence: `cpython_copy_public_subset`. | `cpython_copy_public_diff_subset`, `cpython_array_one_byte_public_copy_byteswap_compare_diff_subset`. | Full pickle protocol byte compatibility and arbitrary extension-object copy hooks. |
