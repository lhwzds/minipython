...
- Added direct public-surface evidence for the minimal `deque` sandbox slice in
  ...
  `extend` / `extendleft` / `pop` / `popleft` / `count` / `index` /
  `rotate` / `reverse` / `clear` / `copy` operations. Broader deque APIs,
  pickling, performance, lifetime, and thread-safety semantics remain outside
  the sandbox default scope.
