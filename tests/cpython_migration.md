...
| `operator` | Arithmetic/comparison helpers, sequence/member helpers, attrgetter/itemgetter/methodcaller, selected signature/module/qualname/doc metadata, and helper repr subsets. Subset evidence: `cpython_operator_public_helpers_subset`, `cpython_operator_length_hint_subset`, `cpython_operator_comparison_predicate_subset`, `cpython_operator_is_none_predicates_subset`, `cpython_operator_arithmetic_bitwise_subset`, `cpython_operator_sequence_member_subset`, `cpython_operator_callable_helper_subset`, `cpython_operator_call_helper_subset`, `cpython_operator_inplace_helper_subset`, `cpython_operator_module_metadata_subset`, `cpython_operator_signature_helper_subset`, and `cpython_operator_helper_repr_subset`. | `cpython_operator_public_helpers_diff_subset`, `cpython_operator_length_hint_diff_subset`, `cpython_operator_comparison_predicate_diff_subset`, `cpython_operator_is_none_predicates_diff_subset` is gated for CPython oracles with `operator.is_none`, `cpython_operator_arithmetic_bitwise_diff_subset`, `cpython_operator_sequence_member_diff_subset`, `cpython_operator_callable_helper_diff_subset`, `cpython_operator_call_helper_diff_subset` is gated for CPython oracles with `operator.call`, `cpython_operator_inplace_helper_diff_subset`, `cpython_operator_module_metadata_diff_subset`, `cpython_operator_signature_helper_diff_subset`, `cpython_operator_helper_repr_diff_subset`. | Full pickle metadata and every CPython helper edge case until separately migrated. |
...
- Added `cpython_operator_module_metadata_subset`, migrating public behavior
  ...
  for this slice. Direct CPython diff evidence for the default-oracle stable
...
- Extended the scope-backed namespace mappings with CPython-compatible
  `copy()`, `get()`, `pop()`, and `setdefault()` behavior for supported
  string-key globals and locals snapshots, including default values, snapshot
  copies, write-through insertion, and missing-key `KeyError` type parity.
- Added a CPython/MiniPython differential parity case for the supported
