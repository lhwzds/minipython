...
| `io.BytesIO` | In-memory bytes read/read1/write/writelines/readline/readlines/getvalue/getbuffer/readinto/readinto1/tell/seek/truncate plus io.SEEK_SET/SEEK_CUR/SEEK_END constants behavior used by arrays, memoryview, copy, and byte-oriented tests, including writable `bytearray` / memoryview targets, contiguous memoryview `write()` / `writelines()` and non-contiguous memoryview `BufferError` rejection, `getbuffer()` writable memoryview mutation back into the stream buffer and active-export `BufferError` protection for `write()` / all `truncate()` requests / `close()` across direct and derived memoryviews, release of function-scoped, deleted-binding, and expression-temporary exported views, sequential bytes-like line writes, sparse write NUL filling, non-extending `truncate()` growth requests, post-EOF empty reads/readinto results, position tracking, size/sizehint line reads, line iteration through `__iter__()` / `__next__()` with EOF `StopIteration`, `readable()` / `writable()` / `seekable()` / `isatty()` / `flush()` IOBase query methods, `fileno()` / `detach()` `io.UnsupportedOperation` stubs, type metadata, instance `__dict__` and custom attributes, `copy.copy()` / `copy.deepcopy()` behavior for open in-memory streams, `close()` / `closed` state handling, context-manager `__enter__()` / `__exit__()` lifecycle behavior, supported method visibility through `dir()`, and `None` / bytes-like `initial_bytes=` construction plus CPython-style unknown-keyword constructor diagnostics. Subset evidence: `cpython_io_bytesio_public_subset` and `cpython_memoryview_bytesio_readinto_subset`. | `cpython_io_bytesio_public_diff_subset`, `cpython_memoryview_bytesio_readinto_diff_subset`, `cpython_array_one_byte_public_file_methods_diff_subset`, `cpython_copy_public_diff_subset`. | Real files, buffering layers, text I/O, file descriptors, and OS-backed stream semantics. |
