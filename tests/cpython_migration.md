...
| Completion status | The strict CPython test manifest still has `partial`, `blocked_by_runtime`, and `blocked_by_ast_module` groups; full method-level parity has not been proven, and some rows need runtime or standard-library behavior MiniPython does not yet implement. | Keep those unfinished rows visible until each row is either ported, intentionally blocked by the sandbox stop line, or removed by a deliberate scope change. |
