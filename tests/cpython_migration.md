...
| `sys` | In-memory module metadata including `builtin_module_names`, `byteorder`, `hexversion`, `implementation`, `version_info`, `warnoptions`, `modules`, stdio placeholders, `maxsize`, float/hash info, `getdefaultencoding()`, `is_finalizing()`, `exc_info()`, int digit-limit controls, frame inspection subset, breakpoint hook metadata, and sandbox default no-op breakpoint hook. Subset evidence: `cpython_float_hash_and_sys_info_subset`, `cpython_builtin_negation_sys_maxsize_subset`, `cpython_int_max_str_digits_runtime_subset`, `cpython_attribute_introspection_builtins_subset`, `cpython_builtin_breakpoint_custom_hook_subset`, `cpython_builtin_breakpoint_default_stub_subset`, `cpython_builtin_breakpoint_passthru_error_subset`, and `cpython_types_frame_locals_proxy_type_subset`. | `cpython_globals_locals_builtin_diff_subset`, `cpython_attribute_introspection_builtins_diff_subset`, `cpython_builtin_negation_sys_maxsize_diff_subset`, `cpython_int_max_str_digits_runtime_diff_subset` is gated for CPython oracles with sys.get_int_max_str_digits() / sys.set_int_max_str_digits(), `cpython_builtin_breakpoint_custom_hook_diff_subset`, `cpython_builtin_breakpoint_passthru_error_diff_subset`, `cpython_float_hash_and_sys_info_diff_subset`, `cpython_types_frame_locals_proxy_type_diff_subset`. | Real argv/process state, real stdin/stdout/stderr streams, implementation refcount/GC/debug APIs, host filesystem encoding policy, host warning/environment option injection, and cross-thread interpreter state. |
