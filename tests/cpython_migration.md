...
- Kept `cpython_collections_counter_comparison_subset` as runtime-subset
  evidence because the local `cpython_diff` oracle predates `Counter.total()`
  and the newer zero-count rich-comparison rules.
