...
| `copy` | `copy()`, `deepcopy()`, and `replace()` for supported runtime values and AST/simple namespace use cases. | `pure-memory-stdlib-core`, `cpython_copy_public_diff_subset`, `cpython_array_one_byte_public_copy_byteswap_compare_diff_subset`. | Full pickle protocol byte compatibility and arbitrary extension-object copy hooks. |
| `io.BytesIO` | In-memory bytes read/write/getvalue behavior used by arrays, memoryview, and byte-oriented tests. | `pure-memory-stdlib-core`, `cpython_io_bytesio_public_diff_subset`, `array-one-byte-public-file-methods`. | Real files, buffering layers, text I/O, file descriptors, and OS-backed stream semantics. |
