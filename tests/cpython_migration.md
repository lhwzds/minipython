...
| `io.BytesIO` | In-memory bytes read/write/writelines/readline/readlines/getvalue/readinto/tell/seek/truncate behavior used by arrays, memoryview, and byte-oriented tests, including writable `bytearray` / memoryview targets, sequential bytes-like line writes, sparse write NUL filling, position tracking, size/sizehint line reads, `readable()` / `writable()` / `seekable()` / `isatty()` / `flush()` IOBase query methods, `close()` / `closed` state handling, context-manager `__enter__()` / `__exit__()` lifecycle behavior, and `initial_bytes=` construction. Subset evidence: `cpython_io_bytesio_public_subset` and `cpython_memoryview_bytesio_readinto_subset`. | `cpython_io_bytesio_public_diff_subset`, `cpython_memoryview_bytesio_readinto_diff_subset`, `cpython_array_one_byte_public_file_methods_diff_subset`. | Real files, buffering layers, text I/O, file descriptors, and OS-backed stream semantics. |
