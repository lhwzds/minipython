...
- Added
  ...
  across the full 64-by-64 zero/one-count Counter matrix. The direct diff is
  gated for CPython oracles that expose `Counter.__xor__`.
- Added `cpython_collections_counter_symmetric_difference_diff_subset` and
  `cpython_collections_counter_symmetric_difference_subset`, porting CPython
  `TestCounter::test_symmetric_difference` across the full 9^4 population
  matrix for `^` and `^=`. The direct diff is gated for CPython oracles that
  expose `Counter.__xor__`.
- Added
  `cpython_collections_counter_update_reentrant_add_clears_counter_subset`,
