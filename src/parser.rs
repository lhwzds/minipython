...
impl Parser<'_> {
    ...
    fn parse_as_pattern_capture_target(&mut self) -> Result<String, String> {
        ...
        if self.is_parenthesized_list_as_pattern_target() {
            return Err("cannot use list as pattern target".to_string());
        }
        ...
        if self.is_parenthesized_none_as_pattern_target() {
        ...
        if self.is_parenthesized_false_as_pattern_target() {
            return Err("cannot use False as pattern target".to_string());
        }

        if self.is_parenthesized_ellipsis_as_pattern_target() {
            return Err("cannot use ellipsis as pattern target".to_string());
        }

        if self.is_parenthesized_f_string_as_pattern_target() {
            return Err("cannot use f-string expression as pattern target".to_string());
        }

        if self.is_parenthesized_t_string_as_pattern_target() {
            return Err("cannot use t-string expression as pattern target".to_string());
        }
        ...
        if self.is_parenthesized_expression_as_pattern_target() {
            return Err("cannot use expression as pattern target".to_string());
        }
    ...
    fn is_parenthesized_list_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
    ...
    fn is_parenthesized_none_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
    ...
    fn is_parenthesized_false_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
        ...
        matches!(self.tokens.get(start), Some(Token::False)) && start + 1 == end
    }

    fn is_parenthesized_ellipsis_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
        ...
        matches!(self.tokens.get(start), Some(Token::Ellipsis)) && start + 1 == end
    }

    fn is_parenthesized_f_string_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
        ...
        matches!(self.tokens.get(start), Some(Token::FString(_))) && start + 1 == end
    }

    fn is_parenthesized_t_string_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
        ...
        matches!(self.tokens.get(start), Some(Token::TString(_))) && start + 1 == end
    }
    ...
    fn is_set_display_as_pattern_target(&self) -> bool {
        let (set_start, set_end) = if matches!(self.peek(), Some(Token::LeftBrace)) {
            let Some(set_end) = self.find_matching_brace(self.current) else {
                return false;
            };
        ...
        self.is_set_display(set_start, set_end)
    }

    fn is_set_display(&self, set_start: usize, set_end: usize) -> bool {
        if set_end == set_start + 1 {
            return false;
        }
    ...
    fn is_parenthesized_conditional_expression_as_pattern_target(&self) -> bool {
        ...
        for index in (self.current + 1)..outer_end {
            match self.tokens.get(index) {
                ...
                Some(Token::If) if bracket_depth == 0 && brace_depth == 0 => has_if = true,
                Some(Token::Else) if bracket_depth == 0 && brace_depth == 0 && has_if => {
                    has_else = true
                }
                _ => {}
            }
    ...
    fn is_parenthesized_comparison_as_pattern_target(&self) -> bool {
        ...
        has_top_level_comparison
    }

    fn is_parenthesized_expression_as_pattern_target(&self) -> bool {
        let Some(outer_end) = self.find_matching_paren(self.current) else {
            return false;
        };
        if !matches!(
        ...
        let mut bracket_depth = 0usize;
        let mut brace_depth = 0usize;
        let mut has_top_level_expression_operator = false;
        ...
        has_top_level_expression_operator
    }
    ...
    fn parse_mapping_rest_capture_target(&mut self) -> Result<String, String> {
        ...
        match self.advance().cloned() {
            Some(Token::Identifier(name)) => Ok(name),
            _ => unreachable!("mapping rest capture target starts with an identifier"),
        }
    }
